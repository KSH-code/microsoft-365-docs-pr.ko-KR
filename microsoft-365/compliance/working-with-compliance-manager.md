...
3. 서비스 신뢰 포털에서 가장 최근 기능이 포함 된 미리 보기 버전인 **준수 관리자**를 선택 하는 것이 좋습니다. **준수 관리자 (클래식)** 는 준수 관리자의 이전 버전으로 이동 합니다.

4. 공개 계약이 표시 되지 않는 경우에는 해당 계약 내용을 읽고 **동의**를 선택한 다음 준수 관리자 대시보드를 표시 합니다.

시작 하기 위해 Office 365에 대 한 ISO/IEC 27001:2103 평가가 조직에 기본적으로 표시 됩니다.

## <a name="administration"></a>관리

전역 관리자만 사용할 수 있으며 전역 관리자 계정으로 로그인 한 경우에만 표시 되는 특정 관리 기능이 있습니다. 전역 관리자는 사용자 권한을 할당할 수 있으며 모든 작업에 대해 자동 보안 점수 업데이트를 설정할 수 있습니다.
...
> [!NOTE]
> 새 평가를 추가 *하기 전에 먼저* 조직에 대 한 그룹화 전략을 결정 하는 것이 좋습니다. 기본적으로 초기 평가를 위해 "기본 그룹" 이라는 그룹을 사용할 수 있습니다.

### <a name="how-to-create-a-group"></a>그룹을 만드는 방법

그룹은 독립 실행형 엔터티로 만들 수 없습니다. 그룹에는 항상 하나 이상의 평가를 포함 해야 하므로 그룹을 만들려면 먼저 그룹에 배치할 평가를 만들어야 합니다. 그룹을 만들려면 다음 단계를 수행 합니다.
...
5. **저장**을 선택합니다. 완료 되 면 플라이 아웃 창이 닫히고 새 그룹이 대시보드에 자동으로 표시 됩니다.
...
차원은 템플릿, 평가 또는 작업 항목에 대 한 정보를 제공 하는 메타 데이터 집합입니다. 차원은 차원 키가 속성을 나타내는 키 및 값의 개념을 사용 하 고, 차원 값은 속성의 유효한 값을 나타냅니다. 예를 들어 준수 관리자에서 다음 세 가지 유형의 작업을 수행할 수 있습니다. **설명서**, **운영**및 **기술**에 대 한 **작업 유형** 및 차원 값의 차원 키로 정의 됩니다. 기존 차원을 편집 하거나 삭제할 수 있습니다.

> [!IMPORTANT]
> 새 차원을 추가할 수 있으며 이미 가져온 서식 파일에 할당할 수 있습니다. 새로 만든 템플릿에 새 차원을 추가할 수도 있습니다.
...
- **controlType:** CC를 사용 하 여 고객 관리 되는 컨트롤 임을 나타냅니다.
...
- **Actiondimensionvalues:** 다음 형식을 사용 하 여 차원 탭에서 적용 가능한 차원에 대 한 두 개의 세미콜론으로 구분 된 목록입니다.
    ...
    사용자 지정 서식 파일에 사용 되는 모든 차원은 차원 대시보드에 이미 나열 되어 있더라도 가져오기 파일의 차원 탭에 나열 되어야 합니다.
...
1. **서식 파일** 대시보드를 열고 **+ 서식 파일 추가**를 선택 합니다.
2. **서식 파일 이름 입력** 필드에 서식 파일의 고유 이름을 지정 합니다.
3. 사용 가능한 목록에서 차원을 하나 이상 선택 합니다.
...
> [!NOTE]
> 서식 파일에 대 한 변경 내용은 기존 평가에 반영 되지 않습니다. 변경 내용이 표시 되도록 먼저 서식 파일을 업데이트 한 다음 새 평가에 적용 해야 합니다.
