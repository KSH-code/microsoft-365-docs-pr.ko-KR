...
Office 365의 DLP (데이터 손실 방지)를 통해 중요 한 정보를 식별, 모니터링 및 보호할 수 있습니다. 중요 한 정보 식별 간혹 키워드를 찾으려는 경우, 특히 일반 콘텐츠 (예: 의료 관련 통신)를 식별 하거나 부적절 한 언어나 명시적 언어를 사용 해야 하는 경우가 있습니다. 키워드 목록은 중요 한 정보 유형으로 만들 수 있지만 키워드 목록은 크기를 제한 하며 XML을 수정 하 여 만들거나 편집 해야 합니다. 키워드 사전은 보다 간단 하 게 키워드를 관리할 수 있도록 하 여 사전 당 최대 10만 용어를 지원 합니다.
...
1. 보안 & 준수 센터 ([https://protection.office.com](https://protection.office.com))에 연결 합니다.
...
사전을 수정 하려면 `.split(',')` 메서드를 사용 하 여 용어 문자열을 다시 배열로 변환 해야 합니다. 그런 다음 `.trim()` 메서드를 사용 하 여 키워드와 함께 사용 하는 불필요 한 공백을 정리 하 여 사용할 키워드만 남겨 둡니다. 
