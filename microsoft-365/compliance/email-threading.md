...
전자 메일 스레딩은 각 전자 메일을 분석 하 고 개별 메시지에 deconstructs 합니다. 각 전자 메일은 개별 메시지의 연쇄입니다. 그런 다음 작업 집합의 모든 전자 메일을 분석 하 여 전자 메일이 고유한 콘텐츠를 포함 하는지 또는 체인이 다른 전자 메일에 완전히 포함 되어 있는지를 확인 합니다. 최종 전자 메일은 다음 네 가지 범주로 나뉩니다.
