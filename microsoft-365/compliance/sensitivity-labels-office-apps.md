...
각 기능에 대해 다음 표에는 기본 제공 레이블 지정을 사용하여 민감도 레이블을 지원하는 데 필요한 최소 Office 버전이 나열되어 있습니다. 또는 레이블 기능이 공개 미리 보기에 있는 경우 또는 향후 릴리스에 대한 검토 중입니다. 향후 릴리스에 대한 자세한 내용은 [Microsoft 365](https://aka.ms/MIPC/Roadmap) 로드맵을 사용 합니다.
...
iOS 및 Android용 Office: 민감도 레이블이 Office 앱에 [기본 제공됩니다.](https://www.microsoft.com/en-us/microsoft-365/blog/2020/02/19/new-office-app-android-ios-available/)
...
|레이블이 [지정되고](https://support.office.com/article/6d6bd723-ebfd-4e40-b5f6-ae6e8088f7a5) 암호화된 문서에 대한 자동 저장 및 공동 작성 지원 [](https://support.office.com/article/ee1509b4-1f6e-401e-b04a-782d26f564a4) | 검토 중 | 검토 중 | 검토 중 | 검토 중 | [예 - 옵트인](sensitivity-labels-sharepoint-onedrive-files.md) |
|
...
|[사용자가 전자 메일 및 문서에 레이블을 적용해야 합니다.](#require-users-to-apply-a-label-to-their-email-and-documents)   | 미리 보기: [현재 채널(미리 보기)](https://office.com/insider)                        | 16.43+                     | 검토 중            | 검토 중                | 예                |
|[데이터 분류 시작](data-classification-overview.md) 및 관리자를 위한 데이터 보내기                      | 2011+ | 검토 중 | 검토 중           | 검토 중               | 검토 중 |
|[민감도 레이블을 콘텐츠에 자동으로 적용](apply-sensitivity-label-automatically.md)                    | 2009+                      | 16.44+                    | 검토 중           | 검토 중               | 예 |
|
...
또한 외부 사용자는 Windows 및 Microsoft 365 앱(이전의[Office 365](https://docs.microsoft.com/deployoffice/name-change)앱) 또는 Office 2019의 독립 실행형 에디션을 사용할 때 Microsoft 계정을 사용하여 암호화된 문서를 열 수도 있습니다. 다른 플랫폼에서 더 최근에 지원되는 Microsoft 계정은 macOS(Microsoft 365 앱, 버전 16.42+), Android(버전 16.0.13029+) 및 iOS(버전 2.42+)에서 암호화된 문서를 여는 데도 지원됩니다. 예를 들어 조직의 사용자가 암호화된 문서를 조직 외부의 사용자와 공유하고 암호화 설정은 외부 사용자의 Gmail 전자 메일 주소를 지정합니다. 이 외부 사용자는 Gmail 전자 메일 주소를 사용하는 자체 Microsoft 계정을 만들 수 있습니다. 그런 다음 이 계정으로 로그인한 후 지정된 사용 제한에 따라 문서를 열고 편집할 수 있습니다. 이 시나리오의 Walkthrough example, [see Opening and editing the protected document.](https://docs.microsoft.com/azure/information-protection/secure-collaboration-documents#opening-and-editing-the-protected-document)

> [!NOTE]
> Microsoft 계정의 전자 메일 주소는 암호화 설정에 대한 액세스를 제한하기 위해 지정된 전자 메일 주소와 일치해야 합니다.

Microsoft 계정이 있는 사용자가 이러한 방식으로 암호화된 문서를 열면 동일한 이름의 게스트 계정이 없는 경우 테넌트에 대한 게스트 계정을 자동으로 만듭니다. 게스트 계정이 있는 경우 지원되는 데스크톱 및 모바일 Office 앱에서 암호화된 문서를 여는 것 외에도 웹용 Office를 사용하여 SharePoint 및 OneDrive에서 문서를 여는 데 사용할 수 있습니다.

그러나 복제 대기 시간 때문에 이 시나리오에서는 자동 게스트 계정이 즉시 만들어지지 않습니다. 레이블 암호화 설정의 일부로 개인 전자 메일 주소를 지정하는 경우 Azure Active Directory에서 해당 게스트 계정을 만드는 것이 좋습니다. 그런 다음 이러한 사용자가 이 계정을 사용하여 조직에서 암호화된 문서를 열 수 있어야 하다는 것을 알립니다.

> [!TIP]
> 외부 사용자가 지원되는 Office 클라이언트 앱을 사용하게 될지 확신할 수 없는 경우 게스트 계정을 만들거나(특정 사용자용) SharePoint 및 [OneDrive와 Azure AD B2B와의 SharePoint 및 OneDrive](https://docs.microsoft.com/sharepoint/sharepoint-azureb2b-integration-preview) 통합을 사용하는 경우(인증된 사용자에 대해) 외부 사용자와의 보안 공동 작업을 지원하는 보다 안정적인 방법입니다.
...
이 게스트 계정이 없는 경우 암호화된 문서를 열 수 없습니다. 오류 메시지가 표시됩니다. 이 시나리오에서 다른 **Azure Active Directory** 사용자 계정으로 다시 로그인하려면 해당 계정을 테넌트의 외부 사용자로 추가해야 하도록 메시지 텍스트가 표시될 수 있습니다.
...
민감도 레이블에서 사용하는 암호화 서비스인 조건부 액세스 및 Azure Information Protection에 대한 자세한 내용은 Azure Information Protection이 조건부 액세스에 사용할 수 있는 클라우드 앱으로 나열되는 것을 참조하세요. 이 방식은 어떻게 [작동하나요?](https://docs.microsoft.com/azure/information-protection/faqs#i-see-azure-information-protection-is-listed-as-an-available-cloud-app-for-conditional-accesshow-does-this-work)
...
> 
> [Azure Information Protection](https://docs.microsoft.com/azure/information-protection/rms-client/install-unifiedlabelingclient-app) 통합 레이블 클라이언트는 필수 레이블 지정을 지원하고 Office 앱에 기본 [](#support-for-sensitivity-label-capabilities-in-apps) 제공된 레이블 지정을 위해 이 페이지의 기능 섹션에 있는 표를 참조하세요.
...
- Office 앱에 기본 제공되는 레이블:
    - 문서의 경우(Word, Excel, PowerPoint): 표시되지 않은 문서를 열거나 저장할 때
...
이 설정을 사용하는 경우를 위한 지침은 정책 설정에 대한 [정보를 참조하세요.](sensitivity-labels.md#what-label-policies-can-do)
