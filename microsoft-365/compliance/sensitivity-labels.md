...
여러 레이블 정책에 한 사용자를 포함시키면 해당 사용자에게 해당 정책의 모든 민감도 레이블 및 설정이 표시됩니다. 여러 정책의 설정에서 충돌이 있는 경우 우선 순위가 가장 높은 정책의 설정이 적용됩니다. 즉, 각 설정에서 우선 순위가 가장 높은 우선 순위가 우선합니다.

사용자 또는 그룹에 대해 예상되는 레이블 또는 레이블 정책 설정 동작이 표시되지 않는다면 민감도 레이블 정책의 순서를 확인하십시오. 레이블 정책의 순서를 다시 지정하려면, 민감도 레이블 정책 선택 > 오른쪽에서 줄임표 선택 > **아래로 이동** 또는 **위로 이동** 합니다.
