...
*성인*, *Racy*및 *Gory* 이미지 분류자는에서 파일을 검색 합니다. JPEG,. PNG,. GIF 및입니다. BMP 형식 이미지 파일의 크기는 4mb 미만 이어야 하 고 이미지 크기는 50x50 픽셀 보다 커야 하며, 이미지가 평가를 위해 자격을 얻을 수 있도록 50 킬로바이트 (KB) 보다 커야 합니다. 이미지 식별은 Exchange Online 전자 메일 메시지 및 Microsoft 팀 채널과 채팅에 지원 됩니다.
...
Microsoft 365의 trainable 분류자에 대 한 자세한 내용은 [trainable 분류자 시작](classifier-getting-started-with.md)을 참조 하십시오.
...
- **익명 버전의**사용자 이름 표시 안 함: 통신 준수 경고에 대 한 모든 현재 및 이전의 정책 일치 항목에 대해 User 이름이 표시 됩니다. 모든 참가자 위험 관리 경고 및 사례에 대해 사용자에 대해 사용자 프로필 정보 (이름, 제목, 별칭, 조직 또는 부서)가 표시 됩니다.
...
새 **보고서** 대시보드는 모든 통신 준수 보고서를 볼 수 있는 중앙 위치입니다. 보고서 위젯은 통신 준수 작업의 상태에 대 한 전반적인 평가를 위해 가장 필요한 정보를 신속 하 게 보여 줍니다. 보고서 위젯에 포함 된 정보는 내보낼 수 없습니다.

**Reports** 대시보드에는 다음과 같은 보고서 widget이 포함 되어 있습니다.

- **최근에 사용한 정책 일치**: 시간에 따른 활성 정책의 일치 수를 표시 합니다.
- **정책에 따라 확인 된 항목**: 정책이 시간에 따라 확인 한 정책 일치 경고 수를 표시 합니다.
- **대부분의 정책이 일치**하는 사용자 (또는 익명 사용자 이름) 및 지정 된 기간에 대 한 정책 일치 수를 표시 합니다.
- **대부분의 일치**하는 정책: 지정 된 기간에 대해 일치 하는 정책과 일치 하는 항목의 수를 표시 합니다.
