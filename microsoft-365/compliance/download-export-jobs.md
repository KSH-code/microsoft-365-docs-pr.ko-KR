...
6. 연결 **요약 페이지에서** 연결 정보를 검토하고 연결 을 **클릭합니다.**
    ...
    이 컨테이너에는 4단계의 표시 이름으로 이름이 지정된 컨테이너가 포함되어 있습니다. 이 컨테이너에는 만든 각 내보내기 작업의 폴더가 포함되어 있습니다. 이러한 폴더의 이름은 내보내기 작업의 ID에 해당하는 ID로 지정됩니다. 작업 탭에 나열된 각 내보내기 작업 준비  데이터에 대한 플라이아웃 페이지의  지원 정보에서 이러한 내보내기 IDS(내보내기 이름)를 찾을 **수** 있습니다.
