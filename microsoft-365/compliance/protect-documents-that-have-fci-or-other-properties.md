...
예를 들어 조직에서는 Windows Server FCI를 사용하여 주민 등록 번호와 같은 개인 데이터가 있는  항목을 식별한 다음, 개인 식별 정보 속성을 문서에서 찾은 개인 데이터의 유형과 횟수에 따라 높음, 보통, 낮음, 공개 또는 **PII** 아예로 설정하여 문서를 분류할 수 있습니다.  
...
DLP 정책은 단순히 특정 속성 이름/값 쌍을 검색합니다. SharePoint 검색에 해당하는 관리 속성이 있는 경우 모든 문서 속성을 사용할 수 있습니다. 예를 들어 SharePoint 사이트 모음은 이름이 **Customer인** 필수 필드와 함께 Trip Report라는 콘텐츠 형식을 **사용할 수 있습니다.** 사용자가 여행 보고서를 만드는 경우 고객 이름을 입력해야 합니다. 이 속성 이름/값 쌍은 DLP 정책에서도 사용할 수 있습니다. 예를 들어 **고객** 필드에 **Contoso가** 포함되어 있을 때 게스트의 문서 액세스를 차단하는 규칙이 필요한 경우
...
2. 왼쪽 탐색에서 관리 **센터** \> **SharePoint 를 선택 합니다.** 이제 사용자는 SharePoint 관리 센터에 있습니다.
...
9. **크롤링** 속성 선택 대화 상자에서 Windows Server FCI 속성 또는 DLP 정책 확인에서 사용할 기타 속성에 해당하는 크롤링 속성을 찾아 \> \> **선택합니다.**
