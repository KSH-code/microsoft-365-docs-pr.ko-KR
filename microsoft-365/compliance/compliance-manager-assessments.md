...
이 프로세스를 준비하려면 먼저 특수한 서식이 있는 템플릿 Excel 어셈블하여 필요한 템플릿 데이터를 가져와야 합니다. 확장 프로세스에서 사용되는 서식이 Excel [파일에](compliance-manager-templates.md#formatting-your-template-data-with-excel) 대한 특별한 요구 사항이 있습니다. 가져오기 프로세스에서 오류를 방지하는 데 도움이 되는 추가 지점을 참조합니다.
...
준수 관리자에서 사용자 지정 평가를 만들려면 자체 템플릿을 만들어야 합니다. 템플릿을 직접 만들 수 있도록 먼저 서식 있는 템플릿을 Excel 서식 있는 템플릿 데이터를 가져오게 됩니다. 또한 평가를 만들 때 평가를 할당할 그룹을 미리 결정하는 데도 도움이 됩니다(그룹에 대한 자세한 [내용은).](#what-are-groups)

**아래 단계에 따라 사용자 지정 평가를 만드세요.**

1. **파일 형식을 Excel 지정합니다.** 먼저 다음 지침을 사용하여 템플릿 데이터를 Excel 스프레드시트에 [서식을 지정합니다.](compliance-manager-templates.md#formatting-your-template-data-with-excel)
