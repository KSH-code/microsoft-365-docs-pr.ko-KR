...
준수 관리자는 조직에 적용되는 산업 및 지역별 규정 준수를 평가하는 평가를 만들 수 있도록 합니다. 평가는 평가를 완료하기 위한 필수 컨트롤, 개선 작업 및 Microsoft 작업을 포함하는 평가 템플릿의 프레임워크를 사용하여 작성됩니다. 조직에 대해 가장 관련성이 높은 평가를 설정하면 정책 및 운영 절차를 구현하여 규정 준수 위험을 제한하는 데 도움이 될 수 있습니다.
...
동일한 그룹에서 두 가지 다른 평가가 사용자가 관리하는 개선 작업을 공유하면 작업의 구현 세부 정보 또는 상태에 대한 업데이트가 그룹 전체에서 자동으로 동기화됩니다. 이 동기화를 통해 하나의 개선 작업을 구현하고 여러 요구 사항을 동시에 충족할 수 있습니다.
...
4. **이름 및 그룹:** 이러한 속성을 설정하여 평가를 식별하고 그룹에 할당합니다.
