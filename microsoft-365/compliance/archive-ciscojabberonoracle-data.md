...
Microsoft 365 규정 준수 센터의 Veritas 커넥터를 사용하여 Oracle 플랫폼의 Cisco Jabber에서 조직의 사용자 사서함으로 데이터를 Microsoft 365 보관합니다. Veritas는 타사 데이터 원본의 항목을 정기적으로 캡처하고 해당 항목을 가져오도록 구성된 Oracle 커넥터에 [Cisco Jabber를](https://www.veritas.com/insights/merge1/jabber) Microsoft 365. 커넥터는 파일 및 파일 작업, 설명 및 공유 콘텐츠와 같은 콘텐츠를 Oracle의 Cisco Jabber에서 전자 메일 메시지 형식으로 변환한 다음 해당 항목을 사용자의 사서함으로 Microsoft 365.
...
3. Microsoft 365 규정 준수 센터에서 만든 Oracle 커넥터의 Cisco Jabber는 매일 Veritas Merge1 사이트에 연결하고 Jabber 콘텐츠를 Microsoft 클라우드의 안전한 Azure Storage 위치로 전송합니다.
...
Oracle 커넥터에서 Cisco Jabber를 만든 후 규정 준수 센터에서 커넥터 상태를 Microsoft 365 있습니다.
