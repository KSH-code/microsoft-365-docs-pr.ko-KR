...
UI에서 trainable 분류자에 액세스 하려면: 
- 전역 관리자가 테 넌 트를 옵트인 해야 함
- 분류자를 교육 하려면 준수 관리자 역할 또는 준수 데이터 관리자가 필요 합니다.

다음 시나리오에서 trainable 분류자를 사용 하려면 이러한 사용 권한이 있는 계정이 필요 합니다.

- 보존 레이블 정책 시나리오: RecordManagement and 보존 관리 역할 
