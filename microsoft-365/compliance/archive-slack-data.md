...
## <a name="overview-of-archiving-slack-data"></a>보관 여유 시간 데이터 개요

다음 개요에서는 커넥터를 사용 하 여 Microsoft 365에서 여유 시간 정보를 보관 하는 프로세스에 대해 설명 합니다.

![여유 시간 보관 워크플로](../media/SlackConnectorWorkflow.png)

1. 조직에서 여유 시간 사이트를 설정 및 구성 하는 데 사용할 수 있습니다.

2. 24 시간 마다 한 번씩에는 Globanet Merge1 사이트에 보충 되는 채팅 메시지가 복사 됩니다. 또한이 커넥터는 채팅 메시지의 콘텐츠를 전자 메일 메시지 형식으로 변환 합니다.

3. Microsoft 365 준수 센터에서 만드는 여유 커넥터는 매일 Globanet Merge1 site에 연결 하 고 채팅 메시지를 Microsoft 클라우드의 안전한 Azure Storage 위치로 전송 합니다.

4. 커넥터는 3 단계에서 설명한 것 처럼, *전자 메일* 속성 및 자동 사용자 매핑 값을 사용 하 여 변환 된 채팅 메시지 항목을 특정 사용자의 사서함으로 가져옵니다. 사용자 사서함에는 **여유** 편지함 폴더의 새 하위 폴더가 만들어지고 해당 폴더로 채팅 메시지 항목을 가져옵니다. 커넥터는 *Email* 속성 값을 사용 하 여이를 수행 합니다. 모든 채팅 메시지에는 채팅 메시지의 모든 참가자의 전자 메일 주소로 채워지는이 속성이 포함 되어 있습니다.
...
- 조직의 여유 시간 엔터프라이즈 계정에 대 한 사용자 이름 및 암호를 가져옵니다. 여유 시간을 구성 하는 경우 2 단계에서이 계정으로 로그인 해야 합니다.

- 1 단계에서 여유 시간 커넥터를 만든 사용자 (3 단계에서 완료)는 Exchange Online의 사서함 가져오기 내보내기 역할에 할당 되어야 합니다. 이 역할은 Microsoft 365 준수 센터의 **데이터 커넥터** 페이지에 커넥터를 추가 하는 데 필요 합니다. 기본적으로이 역할은 Exchange Online의 어떠한 역할 그룹에도 할당 되지 않습니다. Exchange Online의 조직 관리 역할 그룹에 사서함 가져오기 내보내기 역할을 추가할 수 있습니다. 또는 역할 그룹을 만들고 사서함 가져오기 내보내기 역할을 할당 한 다음 해당 사용자를 구성원으로 추가할 수 있습니다. 자세한 내용은 "Exchange Online에서 역할 그룹 관리" 문서의 [역할 그룹 만들기](https://docs.microsoft.com/Exchange/permissions-exo/role-groups#create-role-groups) 또는 [역할 그룹 수정](https://docs.microsoft.com/Exchange/permissions-exo/role-groups#modify-role-groups) 섹션을 참조 하세요.

## <a name="step-1-set-up-the-slack-connector"></a>1 단계: 여유 시간 커넥터 설정

첫 번째 단계는 Microsoft 365 준수 센터의 **데이터 커넥터** 페이지에 액세스 하 여 여유 시간 데이터를 위한 커넥터를 만드는 것입니다.

1. 로 이동한 [https://compliance.microsoft.com](https://compliance.microsoft.com/) 후 **데이터 커넥터**  >  **여유 시간**을 클릭 합니다.

2. **여유 시간** 제품 설명 페이지에서 **커넥터 추가**를 클릭 합니다.
...
## <a name="step-2-configure-slack"></a>2 단계: 여유 시간 구성

두 번째 단계는 Merge1 사이트에서 여유 시간 커넥터를 구성 하는 것입니다. Globanet Merge1 사이트에서 여유 시간 커넥터를 구성 하는 방법에 대 한 자세한 내용은 [Merge1 타사 커넥터 사용자 가이드](https://docs.ms.merge1.globanetportal.com/Merge1%20Third-Party%20Connectors%20Slack%20eDiscovery%20User%20Guide.pdf)를 참조 하십시오.
...
1. **Microsoft 365 사용자에 게 외부 사용자 매핑** 페이지에서 자동 사용자 매핑을 사용 하도록 설정 합니다.
   ...
   여유 시간 항목에는 조직 내 사용자의 전자 메일 주소를 포함 하는 *email*이라는 속성이 포함 되어 있습니다. 커넥터가이 주소를 Microsoft 365 사용자와 연결할 수 있으면 해당 사용자의 사서함으로 항목을 가져옵니다.
...
## <a name="step-4-monitor-the-slack-connector"></a>4 단계: 여유 시간 커넥터 모니터링

여유 시간 커넥터를 만든 후에는 Microsoft 365 준수 센터에서 커넥터 상태를 볼 수 있습니다.

1. 으로 이동 하 여 [https://compliance.microsoft.com](https://compliance.microsoft.com) 왼쪽 탐색 창에서 **데이터 커넥터** 를 클릭 합니다.

2. **커넥터** 탭을 클릭 한 다음 **여유 시간** 커넥터를 선택 하 여 커넥터에 대 한 속성 및 정보가 포함 된 플라이 아웃 페이지를 표시 합니다.
