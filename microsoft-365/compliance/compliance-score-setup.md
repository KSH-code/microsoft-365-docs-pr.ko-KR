...
조직의 Microsoft 365 전역 관리자는 준수 점수에 액세스 하는 첫 번째 사용자가 될 수 있습니다. 먼저 규정 준수 점수를 검색할 때 전역 관리자 로그인 및 아래에 설명 된 대로 사용자 권한을 설정 하는 것이 좋습니다.
...
액세스 준수 점수에 대 한 직접 링크는 [https://compliance.microsoft.com/compliancescore](https://compliance.microsoft.com/compliancescore)다음과 같습니다.
...
기본적으로 모든 새 테 넌 트에는 [보안 점수](../security/mtp/microsoft-secure-score.md) 자동 업데이트가 설정 되어 있습니다. 보안 점수를 통해 모니터링 되는 모든 작업에서는 준수 점수가 같은 작업에 대 한 상태를 자동으로 업데이트 합니다.
...
4. 최상위 메뉴에서 **준수 관리자** 를 선택 합니다 (레거시 제품인 "준수 관리자 (클래식)"는 선택 하지 않음).
