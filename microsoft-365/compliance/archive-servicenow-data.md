...
Microsoft 365 규정 준수 센터의 Veritas 커넥터를 사용하여 ServiceNow 플랫폼에서 조직 내 사용자 사서함으로 데이터를 Microsoft 365 보관합니다. Veritas는 타사 데이터 원본의 항목을 캡처하고 해당 항목을 가져오는 [ServiceNow](https://globanet.com/servicenow/) 커넥터를 Microsoft 365. 커넥터는 라이브 메시지, 첨부 파일 및 게시물과 같은 콘텐츠를 ServiceNow에서 전자 메일 메시지 형식으로 변환한 다음 해당 항목을 전자 메일 메시지의 사용자 사서함으로 Microsoft 365.

ServiceNow 데이터가 사용자 사서함에 저장되고 나면 소송 보존, eDiscovery Microsoft 365 보존 정책 및 보존 레이블과 같은 Microsoft 365 준수 기능을 적용할 수 있습니다. ServiceNow 커넥터를 사용하여 조직에서 데이터를 가져오고 Microsoft 365 정부 및 규정 정책을 준수하는 데 도움이 될 수 있습니다.
...
3. Microsoft 365 준수 센터에서 만드는 ServiceNow 커넥터는 매일 Veritas Merge1 사이트에 연결하고 ServiceNow 콘텐츠를 Microsoft 클라우드의 보안 Azure Storage 위치로 전송합니다.
...
1. 사용자에 **대한 서비스 사용자** Microsoft 365 매핑 페이지에서 자동 사용자 매핑을 사용하도록 설정합니다. ServiceNow 항목에는 조직의 사용자에 대한 전자 메일 주소가 포함된 *Email이라는* 속성이 포함됩니다. 커넥터가 이 주소를 Microsoft 365 사용자 사서함으로 항목을 가져올 수 있습니다.
...
ServiceNow 커넥터를 만든 후 서비스 준수 센터에서 커넥터 상태를 Microsoft 365 수 있습니다.
