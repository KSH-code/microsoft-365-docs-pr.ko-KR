...
> [!NOTE]
> 자료 보존 라이브러리는 보존 정책 또는 보존 레이블을 적용할 때가 아닌 필요한 경우에만 작성되므로, 이 기능이 작동하는 걸 보려면 먼저 보존 대상 항목을 편집하거나 삭제해야 합니다. 그다음 자료 보존 라이브러리로 이동해서 보존된 복사본을 확인하세요.
