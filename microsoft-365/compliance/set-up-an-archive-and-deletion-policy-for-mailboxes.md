...
- 새 사용자 계정을 만들고 사용자에 게 Exchange Online 라이선스를 할당 하면 사용자에 대해 사서함이 자동으로 만들어집니다. 사서함을 만들 때 기본 MRM 정책 이라는 기본 보존 정책이 자동으로 할당 됩니다. 이 문서에서는 새 보존 정책을 만들고이를 사용자 사서함에 할당 하 여 기본 MRM 정책을 대체 합니다. 사서함에는 한 번에 하나의 보존 정책만 할당할 수 있습니다.
...
> [!NOTE]
> 이 프로세스 중에는 프로세스를 완료 하기 전에 일정 시점이 되 면 언제 든 지 보관 사서함을 사용 하도록 설정할 수 있습니다. 보관 사서함이 사용 하도록 설정 되지 않은 경우에는 보관 또는 삭제 정책이 할당 된 모든 항목에 대해 아무 작업도 수행 되지 않습니다.
  
1. [https://protection.office.com](https://protection.office.com)으로 이동합니다.

2. 전역 관리자 계정을 사용 하 여 로그인 합니다.
...
새 보존 태그를 만들려면 Exchange Online 조직에서 EAC (Exchange 관리 센터)를 사용 합니다. 이전 버전의 EAC를 사용 해야 합니다.
  
1. 으로 이동 하 여 [https://admin.protection.outlook.com/ecp/](https://admin.protection.outlook.com/ecp/) 자격 증명을 사용 하 여 로그인 합니다.
  
2. EAC에서 **준수 관리**  >  **보존 태그로** 이동 합니다.
...
7. 새 보존 정책이 사서함에 할당 되었는지 확인 하려면 다음을 수행할 수 있습니다.
...
4 단계의 사서함에 새 보존 정책을 적용 하 고 나면 Exchange Online에서 새 보존 설정을 사서함에 적용 하는 데 최대 7 일이 걸릴 수 있습니다. 이는 *관리 되는 폴더 도우미* 라는 프로세스가 사서함을 최소한 7 일 마다 한 번씩 처리 하기 때문입니다. 관리 되는 폴더 도우미가 실행 될 때까지 기다리지 않고 Exchange Online PowerShell에서 **시작 관리자 (ManagedFolderAssistant cmdlet)** 를 실행 하 여이 작업이 강제로 수행 되도록 할 수 있습니다.
...
> [!NOTE]
> 앞에서 설명한 것 처럼 관리 되는 폴더 도우미는 최소한 7 일 마다 한 번씩 사서함을 처리 합니다. 따라서 관리 되는 폴더 도우미에서 사서함을 더 자주 처리할 수 있습니다. 또한 관리자는 다음에 관리 되는 폴더 도우미가 사서함을 처리할 때를 예측할 수 없으며,이에 따라 수동으로 실행 해야 하는 이유가 있습니다. 그러나 일시적으로 관리 되는 폴더 도우미가 사서함에 새 보존 설정을 적용 하지 못하게 하려면 `Set-Mailbox -ElcProcessingDisabled $true` 명령을 실행 하 여 관리 되는 폴더 도우미가 사서함을 처리 하지 못하도록 일시적으로 사용 하지 않도록 설정할 수 있습니다. 사서함에 대해 관리 되는 폴더 도우미를 다시 사용 하도록 설정 하려면 `Set-Mailbox -ElcProcessingDisabled $false` 명령을 실행 합니다.
...
4. 2 단계에서이 명령을 다시 실행 하 여 기본 사서함 계획에 할당 된 보존 정책이 변경 되었는지 확인할 수 있습니다.
...
- 다음 표에는이 항목의 단계를 수행 하 여 만든 사용자 지정 보존 정책에 추가 되는 각 보존 태그에 대 한 자세한 정보가 나와 있습니다.
    ...
    | 보존 태그 | 이 태그의 역할 | 기본 제공 또는 사용자 지정 여부 | 타이핑 |
    |:-----|:-----|:-----|:-----|
    ...
    |삭제 안 함  <br/> |이 태그는 보존 정책에 따라 항목이 삭제 되지 않도록 합니다.  <br/> |기본 제공  <br/> |비공개 이 태그는 사용자가 적용할 수 있습니다.  <br/> |
