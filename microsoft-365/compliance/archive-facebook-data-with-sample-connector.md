...
Microsoft 365 규정 준수 센터의 커넥터를 사용하여 Facebook 비즈니스 페이지에서 Microsoft 365로 데이터를 가져오고 보관합니다. 커넥터를 설정하고 구성한 후 일정에 따라 Facebook 비즈니스 페이지에 연결하고 Facebook 항목의 콘텐츠를 전자 메일 메시지 형식으로 변환한 다음 Microsoft 365의 사서함으로 가져올 수 있습니다.
...
Microsoft 365 규정 준수 센터에서 커넥터를 설정하고 구성하여 조직의 Facebook 비즈니스 페이지에서 데이터를 가져오고 보관하기 전에 다음 선행 요구를 완료합니다. 
...
- Facebook 비즈니스 페이지의 커넥터는 하루 동안 총 200,000개 항목을 가져올 수 있습니다. 하루 200,000개가 넘는 Facebook 비즈니스 항목이 있는 경우 이러한 항목 중 Microsoft 365로 가져오지 않습니다.
...
첫 번째 단계는 AAD(Azure Active Directory)에 새 앱을 등록하는 것입니다. 이 앱은 Facebook 커넥터의 4단계 및 5단계에서 구현하는 웹 앱 리소스에 해당합니다. 
...
- TenantId: 1단계에서 Azure Active Directory에서 Facebook 커넥터 앱을 만들고 나서 복사한 Microsoft 365 조직의 테넌트 ID입니다.
...
마지막 단계는 Facebook 비즈니스 페이지에서 Microsoft 365의 지정된 사서함으로 데이터를 가져오는 Microsoft 365 규정 준수 센터에서 커넥터를 설정하는 것입니다. 이 단계를 완료하면 Microsoft 365 가져오기 서비스가 Facebook 비즈니스 페이지에서 Microsoft 365로 데이터를 가져오기 시작하게 됩니다.

단계별 지침은 [5단계: Microsoft 365](deploy-facebook-connector.md#step-5-set-up-a-facebook-connector-in-the-microsoft-365-compliance-center)규정 준수 센터에서 Facebook 커넥터 설정 을 참조하세요. 
