...
Office 365의 보안 & 준수 센터에서 데이터 커넥터를 사용 하 여 Facebook Business 페이지에서 Office 365로 데이터를 가져오고 보관 합니다. 커넥터를 설정 및 구성 하 고 나면 일정에 따라 Facebook Business 페이지에 연결 하 고, Facebook 항목의 콘텐츠를 전자 메일 메시지 형식으로 변환한 다음, 해당 항목을 Office 365의 사서함으로 가져옵니다.
...
조직의 Facebook Business 페이지에서 데이터를 가져오고 보관 하기 위해 보안 & 준수 센터에서 커넥터를 설정 및 구성 하려면 먼저 다음 필수 구성 요소를 완료 합니다. 
...
단계별 지침에 따라이 단계를 완료 하는 동안 웹 앱 리소스를 만들 때 다음 정보 (이전 단계를 완료 한 후에 텍스트 파일로 복사)를 제공 합니다.
...
이 단계를 완료 한 후에는 앱 서비스 URL (예:을 https://fbconnector.azurewebsites.net)복사 해야 합니다. 5 단계, 6 단계 및 7 단계를 완료 하려면이 URL을 사용 해야 합니다.
...
마지막 단계에서는 Facebook Business 페이지의 데이터를 Office 365의 지정 된 사서함으로 가져올 보안 & 준수 센터에서 사용자 지정 커넥터를 설정 합니다. 이 단계를 완료 하면 Office 365 가져오기 서비스가 Facebook Business 페이지의 데이터를 Office 365로 가져오는 것을 시작 합니다. 
