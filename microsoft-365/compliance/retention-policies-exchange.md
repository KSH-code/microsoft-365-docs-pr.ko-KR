...
사용자가 지운 편지함 폴더 이외의 폴더에서 메시지를 삭제하면 해당 메시지는 기본적으로 지운 편지함 폴더로 이동합니다. 사용자가 지운 편지함 폴더에서 항목을 삭제하면 메시지가 복구할 수 있는 항목 폴더로 이동합니다. 그러나 사용자는 Shift+Delete를 눌러 항목을 일시적으로 삭제할 수 있으며, 이 경우 항목이 지운 편지함 폴더를 거치지 않고 복구할 수 있는 항목 폴더로 바로 이동합니다.
