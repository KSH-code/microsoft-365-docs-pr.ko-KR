...
> [!NOTE]
> 자동 정책은 서비스측 라벨을 조건과 함께 사용하여 보존 라벨을 자동으로 적용합니다. 다음 작업을 수행할 때 레이블 정책을 사용하여 보존 레이블을 자동으로 적용할 수도 있습니다. 
>
> - SharePoint 및 Outlook의 기본 보존 레이블을 적용하여 레이블이 지정되지 않은 콘텐츠에 자동으로 레이블을 지정합니다.
...
먼저 보존 레이블을 만듭니다. 그런 다음 자동 정책을 만들어 해당 레이블을 적용합니다. 보존 레이블을 이미 생성한 경우 [자동 정책 생성](#step-2-create-an-auto-apply-policy)로 건너뜁니다.

탐색 지침은 [ 레코드 관리](records-management.md) 사용 여부에 따라 달라집니다. 두 시나리오 모두에 대한 지침이 제공됩니다.
...
중요한 정보에 대한 보존 레이블 자동 적용 정책을 생성하면 DLP(데이터 유실 방지) 정책을 생성할 때와 동일한 정책 템플릿 목록이 표시됩니다. 각 템플릿은 특정 유형의 중요한 정보를 찾도록 미리 구성됩니다. 예를 들어, 여기에 표시된 템플릿은 **개인정보 보호** 범주 및 **U.S PII(개인 식별 가능 정보) 데이터 템플릿** 에서 미국 ITIN, SSN 및 여권 번호를 찾습니다.
...
- 탐지된 중요한 정보의 유형은 일치 정확도(또는 신뢰 수준)가 75 이상입니다. 많은 중요한 정보 유형은 여러 패턴으로 정의되는데, 여기서 일치 정확도가 높은 패턴은 키워드, 날짜 또는 주소와 같이 더 많은 증거를 찾아야 하는 반면 일치 정확도가 낮은 패턴은 증거가 더 적게 필요합니다. **min** 의 일치 정확도가 낮을수록 컨텐츠가 조건과 일치하기 쉽습니다.

- 콘텐츠에 이러한 세 가지 중요한 정보 유형 중 어느 하나의 1~9개 인스턴스가 포함되어 있습니다. **to** 값을 삭제하여 **any** 로 변경할 수 있습니다.
