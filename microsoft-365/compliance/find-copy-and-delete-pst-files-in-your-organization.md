...
- 다음 단계에 따라 PST 컬렉션 도구를 로컬 컴퓨터에 다운로드 합니다. 
    ...
    2. 팝업 창 **에서 다른** \> **이름으로 저장** 을 클릭 하 여 로컬 컴퓨터의 폴더에 PSTCollectionTool 파일을 저장 합니다. 
...
- ** \< JobName \> _FIND_ \< \> datema** csv 파일에는 검색 된 각 PST 파일에 대 한 행이 포함 되어 있습니다. 각 PST에는 PST 파일이 있는 컴퓨터, pst 파일의 전체 경로 위치, pst 파일의 소유자, PST 파일의 크기 (킬로바이트, Kb)가 포함 되어 있습니다. 이 파일은 매개 변수에 지정 된 폴더에 만들어집니다 `LogLocation` . 
...
- ** \< JobName \> _find_ \< Datetimestamp \> ** -xml 파일에는 찾기 모드에서 도구를 실행할 때 사용 되는 매개 변수 값에 대 한 정보가 들어 있습니다. 이 파일에는 찾은 모든 PST 파일에 대 한 정보도 포함 됩니다. 이 파일의 데이터는 동일한 작업에 대해 도구를 다시 실행 하 여 검색 된 PST 파일을 차단, 수집 또는 삭제 하는 데 사용 됩니다. 이 파일은 매개 변수에 지정 된 폴더에 만들어집니다 `ConfigurationLocation` . 
...
Collect 모드에서 DataCollectorMaster를 실행 한 후에는 `LogLocation` 및 매개 변수로 지정 된 폴더에 다음 파일이 만들어지고 저장 됩니다 `ConfigurationLocation` . 
  
- ** \< JobName \> _Collect_ \< datetimestamp \> . .log** -로그 파일에는 표시 된 상태 메시지가 포함 됩니다. 이 파일은 매개 변수에 지정 된 폴더에 만들어집니다 `LogLocation` . 
    
- ** \< JobName에서 \> _Collect_ \< datetimestamp \> ** .xml-xml 파일에는 도구에서 사용 하는 수집 모드에서 실행 된 매개 변수 값에 대 한 정보만 포함 됩니다. DataCollectorMaster 도구를 다시 실행 하 여 PST 파일을 삭제 하면이 파일의 데이터가 사용 됩니다. [5 단계](#step-5-delete-the-pst-files-found-on-your-network)를 참조 하세요.
...
삭제 모드에서 DataCollectorMaster를 실행 한 후에는 `LogLocation` 및 매개 변수에 지정 된 폴더에 다음 파일이 만들어지고 저장 됩니다 `ConfigurationLocation` . 
  
- ** \< JobName \> _Delete_ \< datetimestamp \> . .log** -로그 파일에 표시 된 상태 메시지가 포함 됩니다. 이 파일은 매개 변수에 지정 된 폴더에 만들어집니다 `LogLocation` . 
