...
---
# <a name="collection-statistics-and-reports-in-advanced-ediscovery"></a>Advanced eDiscovery의 컬렉션 통계 및 보고서
...
- **위치 유형:** 쿼리 통계를 적용할 수 있는 콘텐츠 위치의 유형입니다. **Exchange** 값은 사서함 위치를 나타냅니다. **SharePoint** 값은 사이트 위치를 나타냅니다.
...
- **컬렉션 구성 옵션.** 검토 집합에 초안 컬렉션을 커밋할 때 대화 스레드, 클라우드 첨부 파일 및 문서 버전을 포함해야 합니다. 검토 집합에 추가되는 이러한 항목은 초안 컬렉션의 예상 항목에 포함되지 않습니다. 컬렉션을 커밋할 때만 식별되고 수집됩니다. 이러한 옵션을 선택하면 검토 집합에 추가되는 항목 수가 늘어날 가능성이 높습니다. 
    ...
    예를 들어 초안 컬렉션의 예상 버전에는 여러 버전의 SharePoint 문서가 포함되지 않습니다. 그러나 검색 결과를 내보낼 때 모든 문서 버전을 포함하기 위한 옵션을 선택하면 검토 집합에 추가된 항목의 실제 수와 총 크기가 늘어날 수 있습니다. 
...
- **초안 컬렉션의 결과를 예측하는 방법** 초안 컬렉션에서 반환되는 검색 결과의 예상치는 컬렉션 쿼리 조건을 충족하는 항목의 예상 수가 아니라 실제 수입니다. 예상 전자 메일 항목 수를 컴파일하기 위해 Exchange 데이터베이스에서 검색 조건을 충족하는 메시지의 목록이 요청됩니다. 그러나 컬렉션을 검토 집합으로 커밋하면 컬렉션이 다시 시작되어 Exchange 데이터베이스에서 실제 메시지가 검색됩니다. 따라서 예상 항목 수와 실제 항목 수가 결정되는 방식 때문에 차이가 있을 수 있습니다.
