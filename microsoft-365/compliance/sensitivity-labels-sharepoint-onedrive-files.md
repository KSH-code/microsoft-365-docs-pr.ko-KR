...
- SharePoint 또는 OneDrive에서이 파일을 다운로드 하거나 액세스할 때 레이블에 있는 민감도 레이블과 모든 암호화 설정이 파일에 다시 적용 되며, 이러한 설정은 파일을 저장할 때마다 적용 됩니다. 이 동작으로 인해 문서를 보호 하기 위해 레이블을 단독으로 사용할 수 있는 사용자 지침을 제공 해야 합니다. 자세한 내용은 [정보 권한 관리 (테두리) 옵션 및 민감도 레이블을](sensitivity-labels-office-apps.md#information-rights-management-irm-options-and-sensitivity-labels)참조 하세요.
