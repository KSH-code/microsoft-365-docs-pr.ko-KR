...
Contoso의 책임자 정보 보안 책임자입니다. 직원이 전자 메일을 통해 여러 사용자와 비밀 유지 문서를 알 수 없는 공유하는 데이터 유출 상황을 알 수 있습니다. 이 문서를 받은 인원을 내부 및 외부적으로 빠르게 평가하려는 경우 확인되면 다른 조사자들과 사례 결과를 공유하여 검토한 다음 Office 365에서 데이터를 영구적으로 제거합니다. 조사가 완료되면 영구적으로 제거된 증거와 이후 참조에 대한 기타 사례 세부 정보가 있는 보고서를 생성해야 합니다.
...
수급자 또는 최종 사용자에게 Office 365 E5 라이선스가 할당된 경우 Advanced eDiscovery를 사용하여 한 번만 검색 결과를 10,000개까지 검사할 수 있습니다. 검토할 전자 메일 메시지가 10,000개보다 많은 경우 검색 쿼리를 날짜 범위로 나누고 검색 결과가 날짜별로 정렬될 때 각 결과를 개별적으로 검토할 수 있습니다. Advanced eDiscovery에서 미리 보기 패널의 **레이블** 기능을 사용하여 검색 결과에 태그를 지정하고 레이블을 지정한 태그로 검색 결과를 필터링할 수 있습니다. 보조 검토자와 공동 작업할 때 유용합니다. Advanced eDiscovery에서 광학 문자 인식, 전자 메일 스레딩 및 예측 코딩과 같은 추가 분석 도구를 사용하면 수천 개의 메시지를 신속하게 처리하고 검토하고 추가 검토를 위해 태그를 지정할 수 있습니다. [Advanced eDiscovery에 대한 빠른 설정을 참조합니다.](./get-started-with-advanced-ediscovery.md)

유출된 데이터가 포함된 전자 메일 메시지를 찾은 경우 메시지 받는 사람을 확인하여 외부에서 공유된 메시지인지를 파악합니다. 메시지를 더 추적하려면 5단계에 설명된 메시지 추적 로그를 사용할 수 있도록 보낸 사람 정보 및 날짜 범위를 [수집할 수 있습니다.](#step-5-use-message-trace-log-to-check-how-spilled-data-was-shared)

검색 결과를 확인한 후 다른 사용자와 찾은 결과를 보조 검토를 위해 공유할 수 있습니다. 1단계에서 사례에 할당한 사람은 eDiscovery 및 Advanced eDiscovery 둘 다에서 사례 콘텐츠를 검토하고 사례 결과를 승인할 수 있습니다. 실제 콘텐츠를 내보내지 않고도 보고서를 생성할 수 있습니다. 8단계에 설명된 동일한 보고서를 지우기 증명으로 사용할 [수도 있습니다.](#step-8-verify-provide-a-proof-of-deletion-and-audit)
...
4. eDiscovery 사례에서 내보내기 **를** 클릭하여 내보내기 작업 목록을 표시합니다. 방금 만든  내보내기 작업을 표시하려면 새로 고침을 클릭하여 목록을 업데이트해야 할 수 있습니다.
...
유출된 메시지를 삭제하려면 전자 메일 메시지 검색 및 삭제의 & 3단계를 [참조하세요.](./search-for-and-delete-messages-in-your-organization.md)
...
워크플로에서 데이터 유출 사고를 관리하는 마지막 단계는 eDiscovery 사례로 이동한 후 해당 데이터를 삭제하는 데 사용된 동일한 검색 쿼리를 다시 실행하여 결과가 반환되지 않는지 확인하여 유출된 데이터가 사서함에서 영구적으로 제거된지 확인하는 것입니다. 유출된 데이터가 영구적으로 제거된 후 보고서를 내보내고 원본 보고서와 함께 삭제 증명으로 포함할 수 있습니다. 그런 다음 [사례를](close-reopen-delete-core-ediscovery-cases.md) 닫을 수 있습니다. 그러면 향후 참조한 경우 다시 열 수 있습니다. 또한 사서함을 이전 상태로 되찾고, 유출된 데이터를 찾는 데 사용되는 검색 쿼리를 삭제하고, 데이터 유출 사고를 관리할 때 수행된 작업의 감사 레코드를 검색할 수도 있습니다.
