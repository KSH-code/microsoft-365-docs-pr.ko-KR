...
> [!CAUTION]
> 이 문서에서 설명 하는 절차에 따라 Exchange Online 사서함에서 데이터가 영구적으로 삭제 (제거) 됩니다. 즉, 복구 가능한 항목 폴더에서 삭제 한 메시지는 복구할 수 없으며, 법적 검색 이나 다른 규정 준수 용도로는 사용할 수 없습니다. 보안 및 준수 센터에서 만든 소송 보존, In-Place 보존, eDiscovery 보존 또는 보관 정책의 일부로 보류 중인 메시지를 삭제 하려는 경우에는 보존을 제거 하기 전에 레코드 관리 또는 법률 부서에 문의 하십시오. 조직에 보류 중인 사서함 또는 데이터 유출 인시던트가 우선적으로 적용 되는지 여부를 정의 하는 정책이 있을 수 있습니다.
...
- 보존 잠금을 사용 하 여 잠긴 정책이 할당 된 사서함에 대해서는이 절차를 수행할 수 없습니다. 이 잠금은 정책에서 사서함을 제거 하거나 제외 하 고 사서함에서 관리 되는 폴더 도우미를 사용 하지 않도록 설정 하는 것을 방지 하기 때문입니다. 보존에 대 한 정책 잠금에 대 한 자세한 내용은 [Use 보존이 Lock을 사용 하 여 보존 정책 및 보존 레이블 정책에 대 한 변경을 제한](retention-preservation-lock.md)합니다 .를 참조 하세요.
...
Exchange Online PowerShell에서 다음 명령을 실행 하 여 사서함에 있는 In-Place 보존을 식별 합니다. 1 단계에서 확인 한 In-Place 보존에 대 한 GUID를 사용 합니다.

```powershell
Get-MailboxSearch -InPlaceHoldIdentity <hold GUID> | FL Name
```

In-Place 보류를 확인 한 후 EAC (Exchange 관리 센터) 또는 Exchange Online PowerShell을 사용 하 여 보류에서 사서함을 제거할 수 있습니다. 자세한 내용은 [만들기 또는 In-place Hold 제거](https://go.microsoft.com/fwlink/?linkid=852668) 항목을 참조하십시오.
...
마지막 단계는 사서함을 다시 이전 구성으로 되돌리는 것입니다. 즉, 2 단계에서 변경한 속성을 다시 설정 하 고 3 단계에서 제거한 보존을 재적용 합니다. 해당 활동은 다음과 같습니다.
...
4. 3 단계에서 제거한 보존을 다시 적용 합니다. 보존 유형에 따라 다음 절차 중 하나를 사용 합니다.
    ...
    **원본 위치 유지**
    ...
    EAC (또는 Exchange Online PowerShell)를 사용 하 여 사서함을 다시 In-Place 보류에 추가 합니다.
...
|원본 위치 유지  <br/> | `c0ba3ce811b6432a8751430937152491` <br/> |*InPlaceHolds* 속성은 사서함에 저장 된 In-Place 보류의 GUID를 포함 합니다. GUID가 접두사로 시작 하지 않으므로이를 In-Place 보류 라고 말할 수 있습니다.  <br/> `Get-MailboxSearch -InPlaceHoldIdentity <hold GUID> | FL`Exchange Online PowerShell의 명령을 사용 하 여 사서함의 In-Place 보류에 대 한 정보를 가져올 수 있습니다.  <br/> |
