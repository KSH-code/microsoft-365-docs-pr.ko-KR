...
구성하는 보존 설정은 이러한 모든 목표를 달성하는 데 도움이 됩니다. 일반적으로 내용을 관리하려면 다음 두 가지 작업이 필요합니다.
...
이러한 보존 설정은 규정 준수를 이유로 컨텐츠를 보존해야 할 때 추가 스토리지를 생성하고 구성하는 데 드는 추가 오버헤드를 덜어주는 사용 중인 컨텐츠와 함께 작동합니다. 또한 이 데이터를 복사하고 동기화하기 위해 사용자 지정된 프로세스를 구현할 필요가 없습니다.
...
> [!NOTE]
> 보존 보류 라이브러리는 사이트의 스토리지 할당량에서 제외되지 않는 스토리지를 사용합니다. SharePoint 및 Microsoft 365 그룹에 대한 보존 설정을 사용할 경우 스토리지를 늘려야 할 수 있습니다.
> 
이러한 보안 위치 및 보존 콘텐츠는 대부분의 사용자에게 표시되지 않습니다. 대부분의 경우, 사람들은 자신의 콘텐츠가 보존 설정의 영향을 받는다는 사실조차 알 필요가 없습니다.
...
예를 들어 SharePoint 사이트의 모든 문서를 5년 동안 보존해야 하는 경우 해당 사이트의 모든 문서에 동일한 보존 레이블을 적용하는 것보다 보존 정책을 사용하여 보존하는 것이 더 효율적입니다. 그러나 해당 사이트의 일부 문서를 5년 동안 보관하고 다른 문서는 10년 동안 보관해야 하는 경우 보존 정책은 이를 수행할 수 없습니다. 항목 수준에서 보존 설정을 지정해야 하는 경우 보존 레이블을 사용합니다. 

보존 정책과 달리 보존 레이블의 보존 설정은 Microsoft 365 테넌트 내의 다른 위치로 이동하면 컨텐츠와 함께 이동합니다. 또한 보존 레이블에는 보존 정책이 지원하지 않는 다음과 같은 기능이 있습니다. 
