...
- 모니터링에서 잡음이 있는 파일 경로를 제외해야 하는 경우
...
- ‘\로 끝나는 올바른 파일 경로입니다. 이는 폴더 바로 아래에 있는 파일만을 의미합니다. 예시: C:\Temp\
- ‘\*’로 끝나는 올바른 파일 경로입니다. 이는 폴더 바로 아래에 있는 파일 외에 하위 폴더 바로 아래에 있는 파일만을 의미합니다. 예시: C:\Temp\*
- ‘\’ 또는 ‘\*’로 끝나는 올바른 파일 경로입니다. 이는 폴더 및 모든 하위 폴더 바로 아래에 있는 모든 파일을 의미합니다. 예시: C:\Temp
- 각 면의 '\' 사이에 와일드카드가 있는 경로입니다. 예시: C:\Users\*\Desktop\
- 정확한 하위 폴더 수를 제공하기 위해 각 면의 '\' 사이에 와일드카드와 '(숫자)'가 있는 경로입니다. 예시: C:\Users\*(1)\Downloads\
- 시스템 환경 변수를 사용하는 경로입니다. 예시: %SystemDrive%\Test\*
- 위의 모든 것을 혼합한 것입니다. 예시: %SystemDrive%\Users\*\Documents\*(2)\Sub\
...
목록 모드가 **허용**으로 설정되어 있으면 사용자가 ***오직*** 해당 도메인에만 중요한 항목을 업로드할 수 있으며, 다른 모든 도메인에 대한 업로드는 허용되지 않습니다.
...
클라우드 서비스 제한에 업로드가 차단 또는 재설정을 차단하는 것으로 설정되어 있는 적용된 DLP 정책의 조건과 일치하는 파일에 액세스하지 못하도록 차단된 프로세스 이름으로 식별되는 브라우저를 추가합니다. 이 브라우저가 파일에 액세스하지 못하도록 차단되면 최종 사용자에게 Edge Chromium를 통해 파일을 열도록 요청하는 알림 메시지가 표시됩니다.
...
1. 중요한 항목이 공유되지 않도록 하려는 위치(서비스, 도메인, IP 주소) 지정 
2. DLP 정책이 일치하는 경우 특정 중요한 항목에 액세스할 수 없는 브라우저 추가
...
6. 기본 설정인 **이 콘텐츠가 공유되는 경우에 검색**과 **조직 외부의 사용자와** 선택을 사용하고 **다음**을 선택합니다.
7. 기본 설정인 **보호 작업** 값을 적용하고 **다음**을 선택합니다.
8. **Windows 장치에서 활동 감사 또는 제한**을 선택하고 작업을 **감사 전용**으로 유지합니다. **다음**을 선택합니다.
9. 기본 설정인 **먼저 테스트하고 싶습니다**를 적용하고 **테스트 모드에서 정책 팁 표시**를 선택합니다. **다음**을 선택합니다.
10. 설정을 검토하고 **제출**을 선택합니다.
11. 새 DLP 정책이 정책 목록에 표시됩니다.
12. 모니터링되는 끝점에서 데이터에 대한 활동 탐색기를 확인합니다. 장치에 대한 위치 필터를 설정하고 정책을 추가한 다음 정책 이름을 기준으로 필터링하여 정책의 영향을 확인합니다. 필요한 경우 [활동 탐색기로 시작](data-classification-activity-explorer.md)을 참조하세요.
13. 조직 외부의 사용자와 미국 PII(개인 식별 정보) 데이터 조건을 트리거하는 콘텐츠가 포함된 테스트를 공유합니다. 이 경우 정책이 트리거되어야 합니다.
14. 이벤트에 대한 활동 탐색기를 확인합니다.
...
- [Windows 10 컴퓨터에 대한 온보딩 도구 및 방법](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints)
