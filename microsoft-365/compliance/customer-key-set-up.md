...
<span data-ttu-id="3460b-128">설명서 이외에 추가 지원이 필요한 경우 MCS(Microsoft Consulting Services), PFE(프리미어 필드 엔지니어링) 또는 Microsoft 파트너에게 지원을 요청하세요.</span><span class="sxs-lookup"><span data-stu-id="3460b-128">If you need additional support beyond the documentation, contact Microsoft Consulting Services (MCS), Premier Field Engineering (PFE), or a Microsoft partner for assistance.</span></span> <span data-ttu-id="3460b-129">설명서를 포함하여 고객 키에 대한 피드백을 제공하기 위해 사용자 의견, 제안 및 관점을 customerkeyfeedback@microsoft.com.</span><span class="sxs-lookup"><span data-stu-id="3460b-129">To provide feedback on Customer Key, including the documentation, send your ideas, suggestions, and perspectives to customerkeyfeedback@microsoft.com.</span></span>
...
- [<span data-ttu-id="3460b-142">각 구독에서 프리미엄 Azure Key Vault 만들기</span><span class="sxs-lookup"><span data-stu-id="3460b-142">Create a premium Azure Key Vault in each subscription</span></span>](#create-a-premium-azure-key-vault-in-each-subscription)
...
### <a name="submit-a-request-to-activate-customer-key-for-office-365"></a><span data-ttu-id="3460b-175">Office 365에 대한 고객 키 정품 인증 요청 제출</span><span class="sxs-lookup"><span data-stu-id="3460b-175">Submit a request to activate Customer Key for Office 365</span></span>
...
5. <span data-ttu-id="3460b-188">제품 세부 정보를 검토한 후 **2단계를 계속합니다.**</span><span class="sxs-lookup"><span data-stu-id="3460b-188">Once you've reviewed the offer details, choose **Continue to step 2**.</span></span>
...
2. <span data-ttu-id="3460b-205">Register-AzProviderFeature cmdlet을 실행하여 필수 보존 기간을 사용하기 위해 구독을 등록합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-205">Run the Register-AzProviderFeature cmdlet to register your subscriptions to use a mandatory retention period.</span></span> <span data-ttu-id="3460b-206">각 구독에 대해 이 작업을 수행하십시오.</span><span class="sxs-lookup"><span data-stu-id="3460b-206">Perform this action for each subscription.</span></span>
...
5. <span data-ttu-id="3460b-218">프로세스를 완료하기 위해 Register-AzResourceProvider 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-218">To complete the process, run the Register-AzResourceProvider command.</span></span> <span data-ttu-id="3460b-219">각 구독에 대해 이 작업을 수행하십시오.</span><span class="sxs-lookup"><span data-stu-id="3460b-219">Perform this action for each subscription.</span></span>
...
### <a name="check-the-recovery-level-of-your-keys"></a><span data-ttu-id="3460b-315">키의 복구 수준 확인</span><span class="sxs-lookup"><span data-stu-id="3460b-315">Check the recovery level of your keys</span></span>
...
<span data-ttu-id="3460b-362">Exchange Online 및 비즈니스용 Skype에 대한 고객 키를 설정하려면 Exchange Online에 원격으로 연결하여 이러한 단계를 Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="3460b-362">To set up Customer Key for Exchange Online and Skype for Business, you will need to perform these steps by remotely connecting to Exchange Online with Windows PowerShell.</span></span>
  
### <a name="create-a-data-encryption-policy-dep-for-use-with-exchange-online-and-skype-for-business"></a><span data-ttu-id="3460b-363">Exchange Online 및 비즈니스용 Skype에서 사용할 DEP(데이터 암호화 정책) 만들기</span><span class="sxs-lookup"><span data-stu-id="3460b-363">Create a data encryption policy (DEP) for use with Exchange Online and Skype for Business</span></span>

<span data-ttu-id="3460b-364">DEP는 Azure Key Vault에 저장된 키 집합과 연결됩니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-364">A DEP is associated with a set of keys stored in Azure Key Vault.</span></span> <span data-ttu-id="3460b-365">Microsoft 365의 사서함에 DEP를 할당합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-365">You assign a DEP to a mailbox in Microsoft 365.</span></span> <span data-ttu-id="3460b-366">그런 다음 Microsoft 365는 정책에 식별된 키를 사용하여 사서함을 암호화합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-366">Microsoft 365 will then use the keys identified in the policy to encrypt the mailbox.</span></span> <span data-ttu-id="3460b-367">DEP를 만들하려면 앞서 획득한 키 자격 증명 모음 UR이 필요합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-367">To create the DEP, you need the Key Vault URIs you obtained earlier.</span></span> <span data-ttu-id="3460b-368">지침은 [각 Azure Key Vault 키에](#obtain-the-uri-for-each-azure-key-vault-key) 대한 URI 획득을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="3460b-368">See [Obtain the URI for each Azure Key Vault key](#obtain-the-uri-for-each-azure-key-vault-key) for instructions.</span></span>
  
<span data-ttu-id="3460b-369">기억하세요!</span><span class="sxs-lookup"><span data-stu-id="3460b-369">Remember!</span></span> <span data-ttu-id="3460b-370">DEP를 만들 때 두 개의 서로 다른 Azure Key Vault에 있는 두 개의 키를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-370">When you create a DEP, you specify two keys that reside in two different Azure Key Vaults.</span></span> <span data-ttu-id="3460b-371">지리적 중복을 보장하기 위해 이러한 키가 두 개의 별도 Azure 지역에 위치해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-371">Ensure that these keys are located in two separate Azure regions to ensure geo-redundancy.</span></span>
  
<span data-ttu-id="3460b-372">DEP를 만들 수 있는 단계는 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-372">To create the DEP, follow these steps:</span></span>
  
1. <span data-ttu-id="3460b-373">로컬 컴퓨터에서 조직의 전역 관리자 권한이 있는 직장 또는 학교 계정을 사용하여 Exchange [Online PowerShell에](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell) Windows PowerShell 창에 연결합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-373">On your local computer, using a work or school account that has global administrator permissions in your organization, [connect to Exchange Online PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell) in a Windows PowerShell window.</span></span>

2. <span data-ttu-id="3460b-374">DEP를 만들 New-DataEncryptionPolicy 명령을 입력하여 이 cmdlet을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-374">To create a DEP, use the New-DataEncryptionPolicy cmdlet by typing the following command.</span></span>

   ```powershell
   New-DataEncryptionPolicy -Name <PolicyName> -Description "Policy Description" -AzureKeyIDs <KeyVaultURI1>, <KeyVaultURI2>
   ```

   <span data-ttu-id="3460b-375">여기서,</span><span class="sxs-lookup"><span data-stu-id="3460b-375">Where:</span></span>

   - <span data-ttu-id="3460b-376">*PolicyName은* 정책에 사용할 이름입니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-376">*PolicyName* is the name you want to use for the policy.</span></span> <span data-ttu-id="3460b-377">이름에는 공백을 포함할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-377">Names cannot contain spaces.</span></span> <span data-ttu-id="3460b-378">예를 들어 USA_mailboxes.</span><span class="sxs-lookup"><span data-stu-id="3460b-378">For example, USA_mailboxes.</span></span>

   - <span data-ttu-id="3460b-379">*정책 설명은* 정책의 정보를 기억하는 데 도움이 되는 정책에 대한 사용자 친숙한 설명입니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-379">*Policy Description* is a user friendly description of the policy that will help you remember what the policy is for.</span></span> <span data-ttu-id="3460b-380">설명에 공백을 포함할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-380">You can include spaces in the description.</span></span> <span data-ttu-id="3460b-381">예를 들어 "미국의 사서함과 해당 지역의 루트 키"를 예로 들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-381">For example, "Root key for mailboxes in USA and its territories".</span></span>

   - <span data-ttu-id="3460b-382">*KeyVaultURI1은* 정책의 첫 번째 키에 대한 URI입니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-382">*KeyVaultURI1* is the URI for the first key in the policy.</span></span> <span data-ttu-id="3460b-383">예를 들면 <https://contoso_EastUSvault01.vault.azure.net/keys/USA_key_01>와 같습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-383">For example, <https://contoso_EastUSvault01.vault.azure.net/keys/USA_key_01>.</span></span>

   - <span data-ttu-id="3460b-384">*KeyVaultURI2는* 정책의 두 번째 키에 대한 URI입니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-384">*KeyVaultURI2* is the URI for the second key in the policy.</span></span> <span data-ttu-id="3460b-385">예를 들면 <https://contoso_EastUS2vault01.vault.azure.net/keys/USA_Key_02>와 같습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-385">For example, <https://contoso_EastUS2vault01.vault.azure.net/keys/USA_Key_02>.</span></span> <span data-ttu-id="3460b-386">2개의 URIS를 COMMA와 공백으로 구분합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-386">Separate the two URIs by a comma and a space.</span></span>

   <span data-ttu-id="3460b-387">예제:</span><span class="sxs-lookup"><span data-stu-id="3460b-387">Example:</span></span>
  
   ```powershell
   New-DataEncryptionPolicy -Name USA_mailboxes -Description "Root key for mailboxes in USA and its territories" -AzureKeyIDs https://contoso_EastUSvault01.vault.azure.net/keys/USA_key_01, https://contoso_EastUS2vault01.vault.azure.net/keys/USA_Key_02
   ```

<span data-ttu-id="3460b-388">구문과 매개 변수에 대한 자세한 내용은 [New-DataEncryptionPolicy를 참조하십시오.](https://docs.microsoft.com/powershell/module/exchange/new-data-encryptionpolicy)</span><span class="sxs-lookup"><span data-stu-id="3460b-388">For detailed syntax and parameter information, see [New-DataEncryptionPolicy](https://docs.microsoft.com/powershell/module/exchange/new-data-encryptionpolicy).</span></span>

### <a name="assign-a-dep-to-a-mailbox"></a><span data-ttu-id="3460b-389">사서함에 DEP 할당</span><span class="sxs-lookup"><span data-stu-id="3460b-389">Assign a DEP to a mailbox</span></span>

<span data-ttu-id="3460b-390">이 cmdlet을 사용하여 사서함에 DEP를 Set-Mailbox 합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-390">Assign the DEP to a mailbox by using the Set-Mailbox cmdlet.</span></span> <span data-ttu-id="3460b-391">정책을 할당하면 Microsoft 365는 DEP에 지정된 키를 사용하여 사서함을 암호화할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-391">Once you assign the policy, Microsoft 365 can encrypt the mailbox with the key designated in the DEP.</span></span>
  
```powershell
Set-Mailbox -Identity <MailboxIdParameter> -DataEncryptionPolicy <PolicyName>
```

<span data-ttu-id="3460b-392">여기서 *MailboxIdParameter는* 사서함을 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-392">Where *MailboxIdParameter* specifies a mailbox.</span></span> <span data-ttu-id="3460b-393">이 cmdlet에 대한 Set-Mailbox [Set-Mailbox를 참조하십시오.](https://docs.microsoft.com/powershell/module/exchange/set-mailbox)</span><span class="sxs-lookup"><span data-stu-id="3460b-393">For more information about the Set-Mailbox cmdlet, see [Set-Mailbox](https://docs.microsoft.com/powershell/module/exchange/set-mailbox).</span></span>

<span data-ttu-id="3460b-394">하이브리드 최신 인증을 사용하는 iOS 및 [Android용 Outlook을](https://docs.microsoft.com/exchange/clients/outlook-for-ios-and-android/use-hybrid-modern-auth)사용하는온-프레미스 사서함의 경우 Exchange Online 테넌트와 동기화된온-프레미스 사서함 데이터에는 Set-MailUser cmdlet을 사용하여 DEP를 할당할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-394">For [on-premises mailboxes using Outlook for iOS and Android with hybrid Modern Authentication](https://docs.microsoft.com/exchange/clients/outlook-for-ios-and-android/use-hybrid-modern-auth), the on-premises mailbox data that is synchronized into your Exchange Online tenant can have DEP assigned using the Set-MailUser cmdlet.</span></span>

```powershell
Set-MailUser -Identity <MailUserIdParameter> -DataEncryptionPolicy <PolicyName>
```

<span data-ttu-id="3460b-395">여기서 *MailUserIdParameter는* 메일 사용자(메일 사용이 가능한 사용자라고도 합니다)를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-395">Where *MailUserIdParameter* specifies a mail user (also known as a mail-enabled user).</span></span> <span data-ttu-id="3460b-396">이 cmdlet에 대한 Set-MailUser [Set-MailUser를 참조하십시오.](https://docs.microsoft.com/powershell/module/exchange/set-mailuser)</span><span class="sxs-lookup"><span data-stu-id="3460b-396">For more information about the Set-MailUser cmdlet, see [Set-MailUser](https://docs.microsoft.com/powershell/module/exchange/set-mailuser).</span></span>
  
### <a name="validate-mailbox-encryption"></a><span data-ttu-id="3460b-397">사서함 암호화 유효성 검사</span><span class="sxs-lookup"><span data-stu-id="3460b-397">Validate mailbox encryption</span></span>

<span data-ttu-id="3460b-398">사서함 암호화에는 시간이 걸릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-398">Encrypting a mailbox can take some time.</span></span> <span data-ttu-id="3460b-399">정책 할당을 처음 수행하려면 서비스에서 사서함을 암호화하기 전에 사서함도 데이터베이스에서 다른 데이터베이스로 완전히 이동해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-399">For first time policy assignment, the mailbox must also completely move from one database to another before the service can encrypt the mailbox.</span></span> <span data-ttu-id="3460b-400">DEP를 변경하거나 사서함에 DEP를 처음 할당할 때 암호화 유효성을 검사하기 전에 72시간을 기다리는 것이 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-400">We recommend that you wait 72 hours before you attempt to validate encryption after you change a DEP or the first time you assign a DEP to a mailbox.</span></span>
  
<span data-ttu-id="3460b-401">이 Get-MailboxStatistics cmdlet을 사용하여 사서함이 암호화되어 있는지 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-401">Use the Get-MailboxStatistics cmdlet to determine if a mailbox is encrypted.</span></span>
  
```powershell
Get-MailboxStatistics -Identity <GeneralMailboxOrMailUserIdParameter> | fl IsEncrypted
```

<span data-ttu-id="3460b-402">사서함이 암호화된 경우 IsEncrypted 속성은 **true** 값을 반환하고 사서함이 암호화되지 않은 경우 **false** 값을 반환합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-402">The IsEncrypted property returns a value of **true** if the mailbox is encrypted and a value of **false** if the mailbox is not encrypted.</span></span>

<span data-ttu-id="3460b-403">사서함 이동을 완료하는 시간은 처음으로 DEP를 할당하는 사서함의 수와 사서함의 크기에 따라 달라 습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-403">The time to complete mailbox moves depends on the number of mailboxes to which you assign a DEP for the first time, as well as the size of the mailboxes.</span></span> <span data-ttu-id="3460b-404">DEP를 할당한 후 1주일 후에 사서함이 암호화되지 않은 경우 Microsoft에 문의하십시오.</span><span class="sxs-lookup"><span data-stu-id="3460b-404">If the mailboxes have not been encrypted after a week from the time you assigned the DEP, contact Microsoft.</span></span>

## <a name="office-365-setting-up-customer-key-for-sharepoint-online-onedrive-for-business-and-teams-files"></a><span data-ttu-id="3460b-405">Office 365: SharePoint Online, 비즈니스용 OneDrive 및 Teams 파일의 고객 키 설정</span><span class="sxs-lookup"><span data-stu-id="3460b-405">Office 365: Setting up Customer Key for SharePoint Online, OneDrive for Business, and Teams files</span></span>

<span data-ttu-id="3460b-406">시작하기 전에 Azure Key Vault를 설정하는 데 필요한 작업을 완료해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-406">Before you begin, ensure that you have completed the tasks required to set up Azure Key Vault.</span></span> <span data-ttu-id="3460b-407">자세한 [내용은 Azure Key Vault 및 고객 키에 대한 Microsoft FastTrack의 전체 작업을](#complete-tasks-in-azure-key-vault-and-microsoft-fasttrack-for-customer-key) 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="3460b-407">See [Complete tasks in Azure Key Vault and Microsoft FastTrack for Customer Key](#complete-tasks-in-azure-key-vault-and-microsoft-fasttrack-for-customer-key) for information.</span></span>
  
<span data-ttu-id="3460b-408">SharePoint Online, 비즈니스용 OneDrive 및 Teams 파일에 대한 고객 키를 설정하려면 SharePoint Online에 원격으로 연결하여 이러한 단계를 Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="3460b-408">To set up Customer Key for SharePoint Online, OneDrive for Business, and Teams files you will need to perform these steps by remotely connecting to SharePoint Online with Windows PowerShell.</span></span>
  
### <a name="create-a-data-encryption-policy-dep-for-each-sharepoint-online-and-onedrive-for-business-geo"></a><span data-ttu-id="3460b-409">각 SharePoint Online 및 비즈니스용 OneDrive 지리적 데이터에 대한 DEP(데이터 암호화 정책) 만들기</span><span class="sxs-lookup"><span data-stu-id="3460b-409">Create a data encryption policy (DEP) for each SharePoint Online and OneDrive for Business geo</span></span>

<span data-ttu-id="3460b-410">DEP를 Azure Key Vault에 저장된 키 집합과 연결합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-410">You associate a DEP with a set of keys stored in Azure Key Vault.</span></span> <span data-ttu-id="3460b-411">지역이라고도 하는 한 지리적 위치에 있는 모든 데이터에 DEP를 적용합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-411">You apply a DEP to all of your data in one geographic location, also called a geo.</span></span> <span data-ttu-id="3460b-412">Office 365의 Multi-Geo 기능을 사용하는 경우 지역마다 다른 키를 사용할 수 있는 기능을 사용하여 지역당 DEP를 하나씩 만들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-412">If you use the multi-geo feature of Office 365, you can create one DEP per geo with the capability to use different keys per geo.</span></span> <span data-ttu-id="3460b-413">Multi-Geo를 사용하지 않는 경우 조직에서 SharePoint Online, 비즈니스용 OneDrive 및 Teams 파일에서 사용할 DEP를 하나 만들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-413">If you are not using multi-geo, you can create one DEP in your organization for use with SharePoint Online, OneDrive for Business, and Teams files.</span></span> <span data-ttu-id="3460b-414">Microsoft 365는 DEP에서 식별된 키를 사용하여 지리적으로 데이터를 암호화합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-414">Microsoft 365 uses the keys identified in the DEP to encrypt your data in that geo.</span></span> <span data-ttu-id="3460b-415">DEP를 만들하려면 앞서 획득한 키 자격 증명 모음 UR이 필요합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-415">To create the DEP, you need the Key Vault URIs you obtained earlier.</span></span> <span data-ttu-id="3460b-416">지침은 [각 Azure Key Vault 키에](#obtain-the-uri-for-each-azure-key-vault-key) 대한 URI 획득을 참조하세요.</span><span class="sxs-lookup"><span data-stu-id="3460b-416">See [Obtain the URI for each Azure Key Vault key](#obtain-the-uri-for-each-azure-key-vault-key) for instructions.</span></span>
  
<span data-ttu-id="3460b-417">기억하세요!</span><span class="sxs-lookup"><span data-stu-id="3460b-417">Remember!</span></span> <span data-ttu-id="3460b-418">DEP를 만들 때 두 개의 서로 다른 Azure Key Vault에 있는 두 개의 키를 지정합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-418">When you create a DEP, you specify two keys that reside in two different Azure Key Vaults.</span></span> <span data-ttu-id="3460b-419">지리적 중복을 보장하기 위해 이러한 키가 두 개의 별도 Azure 지역에 위치해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-419">Ensure that these keys are located in two separate Azure regions to ensure geo-redundancy.</span></span>
  
<span data-ttu-id="3460b-420">DEP를 만들 수 있도록 원격으로 SharePoint Online에 연결해야 Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="3460b-420">To create a DEP, you need to remotely connect to SharePoint Online by using Windows PowerShell.</span></span>
  
1. <span data-ttu-id="3460b-421">로컬 컴퓨터에서 조직의 전역 관리자 권한이 있는 직장 또는 학교 계정을 사용하여 [SharePoint Online PowerShell에 연결합니다.](https://docs.microsoft.com/powershell/sharepoint/sharepoint-online/connect-sharepoint-online?view=sharepoint-ps)</span><span class="sxs-lookup"><span data-stu-id="3460b-421">On your local computer, using a work or school account that has global administrator permissions in your organization, [Connect to SharePoint Online PowerShell](https://docs.microsoft.com/powershell/sharepoint/sharepoint-online/connect-sharepoint-online?view=sharepoint-ps).</span></span>

2. <span data-ttu-id="3460b-422">Microsoft SharePoint Online 관리 셸에서 다음과 Register-SPODataEncryptionPolicy cmdlet을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-422">In the Microsoft SharePoint Online Management Shell, run the Register-SPODataEncryptionPolicy cmdlet as follows:</span></span>
   ...
   ```
   <span data-ttu-id="3460b-423">예제:</span><span class="sxs-lookup"><span data-stu-id="3460b-423">Example:</span></span>
  
 ```powershell
Register-SPODataEncryptionPolicy -PrimaryKeyVaultName 'stageRG3vault' -PrimaryKeyName 'SPKey3' -PrimaryKeyVersion 'f635a23bd4a44b9996ff6aadd88d42ba' -SecondaryKeyVaultName 'stageRG5vault' -SecondaryKeyName 'SPKey5' -SecondaryKeyVersion '2b3e8f1d754f438dacdec1f0945f251a’
```

   <span data-ttu-id="3460b-424">DEP를 등록하면 지리적 데이터에 대한 암호화가 시작됩니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-424">When you register the DEP, encryption begins on the data in the geo.</span></span> <span data-ttu-id="3460b-425">이 경우 시간이 걸릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-425">This can take some time.</span></span> <span data-ttu-id="3460b-426">이 매개 변수 사용에 대한 자세한 내용은 [Register-SPODataEncryptionPolicy를 참조하세요.](https://docs.microsoft.com/powershell/module/sharepoint-online/register-spodataencryptionpolicy?view=sharepoint-ps)</span><span class="sxs-lookup"><span data-stu-id="3460b-426">For more information on using this parameter, see [Register-SPODataEncryptionPolicy](https://docs.microsoft.com/powershell/module/sharepoint-online/register-spodataencryptionpolicy?view=sharepoint-ps).</span></span>

### <a name="validate-file-encryption"></a><span data-ttu-id="3460b-427">파일 암호화 유효성 검사</span><span class="sxs-lookup"><span data-stu-id="3460b-427">Validate file encryption</span></span>

 <span data-ttu-id="3460b-428">SharePoint Online, 비즈니스용 OneDrive 및 Teams 파일의 암호화 유효성을 검사하려면 [SharePoint Online PowerShell에](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell)연결한 다음 Get-SPODataEncryptionPolicy cmdlet을 사용하여 테넌트의 상태를 검사합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-428">To validate encryption of SharePoint Online, OneDrive for Business, and Teams files, [connect to SharePoint Online PowerShell](https://docs.microsoft.com/powershell/exchange/connect-to-exchange-online-powershell), and then use the Get-SPODataEncryptionPolicy cmdlet to check the status of your tenant.</span></span> <span data-ttu-id="3460b-429">고객 키 암호화를 사용하도록  설정하고 모든 사이트의 모든 파일이 암호화된 경우 _State_ 속성은 등록된 값을 반환합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-429">The _State_ property returns a value of **registered** if Customer Key encryption is enabled and all files in all sites have been encrypted.</span></span> <span data-ttu-id="3460b-430">암호화가 아직 진행 중이면 이 cmdlet은 완료된 사이트의 백분율에 대한 정보를 제공합니다.</span><span class="sxs-lookup"><span data-stu-id="3460b-430">If encryption is still in progress, this cmdlet provides information on what percentage of sites is complete.</span></span>

## <a name="related-articles"></a><span data-ttu-id="3460b-431">관련 문서</span><span class="sxs-lookup"><span data-stu-id="3460b-431">Related articles</span></span>

- [<span data-ttu-id="3460b-432">고객 키를 사용한 서비스 암호화</span><span class="sxs-lookup"><span data-stu-id="3460b-432">Service encryption with Customer Key</span></span>](customer-key-overview.md)

- [<span data-ttu-id="3460b-433">고객 키 관리</span><span class="sxs-lookup"><span data-stu-id="3460b-433">Manage Customer Key</span></span>](customer-key-manage.md)

- [<span data-ttu-id="3460b-434">고객 키 또는 가용성 키 롤 또는 회전</span><span class="sxs-lookup"><span data-stu-id="3460b-434">Roll or rotate a Customer Key or an availability key</span></span>](customer-key-availability-key-roll.md)

- [<span data-ttu-id="3460b-435">가용성 키에 대해 자세히</span><span class="sxs-lookup"><span data-stu-id="3460b-435">Learn about the availability key</span></span>](customer-key-availability-key-understand.md)

- [<span data-ttu-id="3460b-436">서비스 암호화</span><span class="sxs-lookup"><span data-stu-id="3460b-436">Service Encryption</span></span>](office-365-service-encryption.md)
