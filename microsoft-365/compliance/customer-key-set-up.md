...
Office 365에 대한 고객 키를 사용하려면 먼저 Azure를 설정해야 합니다. 이 문서에서는 필요한 Azure 리소스를 만들고 구성하기 위해 따라야 하는 단계에 대해 설명한 다음 Office 365에서 고객 키를 설정하는 단계를 제공합니다. Azure 설치를 완료한 후 조직의 사서함 및 파일에 할당할 정책 및 키를 결정할 수 있습니다. 정책을 할당하지 않는 사서함 및 파일은 Microsoft에서 제어 및 관리하는 암호화 정책을 사용합니다. 고객 키에 대한 자세한 내용은 [Office 365의 고객](customer-key-overview.md)키를 사용하여 서비스 암호화를 참조하세요.
...
3. 프로세스를 완료하기 위해 Microsoft에 문의합니다. SharePoint 및 비즈니스용 OneDrive 팀에 문의할 수 [spock@microsoft.com.](mailto:spock@microsoft.com) Exchange Online 및 비즈니스용 Skype의 경우 에지 [exock@microsoft.com.](mailto:exock@microsoft.com) 전자 메일에 다음 정보를 포함합니다.
...
4. Microsoft로부터 등록이 완료된다는 알림을 받으면 다음과 같이 Get-AzProviderFeature 등록 상태를 확인하면 됩니다. 확인된 Get-AzProviderFeature 명령은 Registration State 속성에 **대해 Registered** 값을 **반환합니다.** 각 구독에 대해 이 단계를 완료합니다.
