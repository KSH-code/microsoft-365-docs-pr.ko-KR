...
EOP(Exchange Online Protection) 및 Exchange Online은 DKIM(Domain Keys Identified Mail) 메시지의 인바운드 유효성 검사를 지원합니다. DKIM은 메시지가 보낸 도메인에서 보낸 메시지와 다른 사람이 스푸핑되지 않았다는 것을 유효성을 검사하는 방법입니다. 전자 메일 메시지를 전송하는 조직에 링크하며, 보다 큰 전자 메일 암호화 패러다임의 일부입니다. 이 패러다임의 세 부분에 대한 자세한 내용은 다음을 참조하세요.
