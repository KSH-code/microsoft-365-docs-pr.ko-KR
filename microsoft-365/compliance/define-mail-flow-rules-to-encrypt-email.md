...
7. 조건을 더 추가 하려면 **기타 옵션** 을 선택한 다음 **조건 추가** 를 선택 하 고 목록에서를 선택 합니다.
   ...
   예를 들어 받는 사람이 조직 외부에 있는 경우에만이 규칙을 적용 하려면 **조건 추가** 를 선택한 다음 받는 사람이 \> **조직** \> 외부에 있는 **외부/내부에** **있습니다 .를**선택 합니다.
...
6. **다음의 경우이 규칙 적용**에서 암호화를 메시지에서 제거할 조건을 선택 합니다. \> **조직 내부**에 **있는 보낸 사람** 추가 이제 받는 사람이 **조직 외부**에 **있는** \> 것과 같은 특정 받는 사람을 대상으로 하는 조건을 더 추가 합니다.
