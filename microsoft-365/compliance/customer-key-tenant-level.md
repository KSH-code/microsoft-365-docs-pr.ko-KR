...
제공한 키를 사용하여 DEP(데이터 암호화 정책)를 만들고 테넌트에 할당할 수 있습니다. DEP는 다음 워크로드에 대한 테넌트에서 데이터를 암호화합니다.
...
|-사용 설정됨|Enabled 매개 변수는 데이터 암호화 정책을 사용하도록 설정하거나 사용하지 않도록 설정합니다. 정책을 사용하지 않도록 설정하기 전에 테넌트에서 정책을 해제해야 합니다. 유효한 값은 다음과 같습니다.</br > $true: 정책이 사용하도록 설정되어 있습니다.</br > $true: 이 정책은 사용하도록 설정되어 있습니다. 이 값은 기본값입니다.
|Y|
