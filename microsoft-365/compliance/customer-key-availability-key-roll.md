...
Microsoft는 가용성 키에 대 한 직접 제어를 고객에 게 공개 하지 않습니다. 예를 들어 Azure Key Vault에서 소유한 키만 롤 (회전) 할 수 있습니다. Office 365은 내부적으로 정의 된 일정에 따라 가용성 키를 조정 합니다. 이러한 키 롤에는 고객 측 SLA (서비스 수준 계약)가 없습니다. Office 365에서는 수동 지원 이외의 프로세스에서 Office 365 서비스 코드를 사용 하 여 가용성 키를 회전 합니다. Microsoft 관리자가 롤 프로세스를 시작할 수 있습니다. 키가 키 저장소에 직접 액세스 하지 않고 자동화 된 메커니즘을 사용 하 여 롤오버 됩니다. 가용성 키 보안 저장소에 대 한 액세스는 Microsoft 관리자에 게 프로 비전 되지 않습니다. 가용성 키 롤링은 처음에 키를 생성 하는 데 사용 되는 것과 동일한 메커니즘을 활용 합니다. 가용성 키에 대 한 자세한 내용은 [가용성 키 이해](customer-key-availability-key-understand.md)를 참조 하십시오.
