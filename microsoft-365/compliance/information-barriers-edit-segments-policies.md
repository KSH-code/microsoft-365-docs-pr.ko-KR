...
정보 장벽 [정책을](information-barriers-policies.md)정의한 후 문제 해결의 일부로 또는 정기적인 유지 관리로 해당 정책이나 사용자 세그먼트를 변경해야 할 수 있습니다. [](information-barriers-troubleshooting.md) 이 문서를 가이드로 사용하세요.

## <a name="what-do-you-want-to-do"></a>무슨 작업을 하고 싶으십니까?

|**작업**|**설명**|
|:---------|:--------------|
| [사용자 계정 특성 편집](#edit-user-account-attributes) | 세그먼트를 정의하는 데 사용할 수 있는 Azure Active Directory의 특성을 입력합니다.<br/>사용자가 세그먼트에 포함되지 않은 경우, 사용자가 포함되는 세그먼트를 변경하거나, 다른 특성을 사용하여 세그먼트를 정의하는 경우 사용자 계정 특성을 편집합니다. |
| [세그먼트 편집](#edit-a-segment) | 세그먼트 정의 방법을 변경하려는 경우 세그먼트를 편집합니다. <br/>예를 들어 Department를 사용하여 원래 세그먼트를 정의한 경우 *이제* *MemberOf와* 같은 다른 특성을 사용할 수 있습니다. |
| [정책 편집](#edit-a-policy) | 정책의 작동 방법을 변경하려는 경우 정보 장벽 정책을 편집합니다.<br/>예를 들어 두 세그먼트 간의 통신을 차단하는 대신 특정 세그먼트 간에만 통신이 발생하도록 할 수 있습니다. |
...
| [정책 응용 프로그램 중지](#stop-a-policy-application) | 정보 장벽 정책 적용 프로세스를 중지하려는 경우 이 작업을 수행하세요.<br/> 정책 응용 프로그램을 중지하는 것은 인스턴트가 아니며 사용자에게 이미 적용된 정책을 실행 취소하지 않습니다. |
...
1. 특성 값 및 할당된 세그먼트와 같은 특정 사용자 계정에 대한 세부 정보를 확인하려면 Identity 매개 변수와 함께 **Get-InformationBarrierRecipientStatus** cmdlet을 사용합니다.
    ...
    |:---------|:----------|
    | `Get-InformationBarrierRecipientStatus -Identity <value> -Identity2 <value>` <p> 이름, 별칭, 고유 이름, 정식 도메인 이름, 전자 메일 주소 또는 GUID와 같이 각 사용자를 고유하게 식별하는 모든 값을 사용할 수 있습니다. <p> 단일 사용자에 대해 이 cmdlet을 사용할 수도 `Get-InformationBarrierRecipientStatus -Identity <value>` 있습니다. |`Get-InformationBarrierRecipientStatus -Identity meganb -Identity2 alexw` <p> 이 예에서는 Office 365의 두 사용자 계정인 *Meganb과* *Alex의 alexw를* *참조합니다.*  |
...
1. 모든 기존 세그먼트를 보시고 **Get-OrganizationSegment** cmdlet을 사용 합니다.
    ...
    세그먼트 유형, 해당 UserGroupFilter 값, 이를 만들거나 마지막으로 수정한 사람, GUID 등 각각에 대한 세그먼트 및 세부 정보 목록이 표시됩니다.

    > [!TIP]
    > 나중에 참조할 수 있도록 세그먼트 목록을 인쇄하거나 저장합니다. 예를 들어 세그먼트를 편집하려면 해당 이름이나 식별 값을 알아야 합니다(Identity 매개 변수와 함께 사용됩니다).

2. 세그먼트를 편집하려면 Identity 매개 변수 및 관련 세부  정보가 있는 **Set-OrganizationSegment** cmdlet을 사용합니다.
...
1. 현재 정보 장벽 정책 목록을 보시다시피 **Get-InformationBarrierPolicy** cmdlet을 사용하세요.

    구문: `Get-InformationBarrierPolicy`

    결과 목록에서 변경할 정책을 식별합니다. 정책의 GUID 및 이름을 유의합니다.

2. Identity 매개 변수와 **함께 Set-InformationBarrierPolicy** cmdlet을 사용하여 변경할 내용을 지정합니다. 
...
1. 현재 정보 장벽 정책 목록을 보시다시피 **Get-InformationBarrierPolicy** cmdlet을 사용하세요.
    ...
    결과 목록에서 변경하거나 제거할 정책을 식별합니다. 정책의 GUID 및 이름을 유의합니다.
...
1. 현재 정보 장벽 정책 목록을 보시다시피 **Get-InformationBarrierPolicy** cmdlet을 사용하세요.
    ...
    결과 목록에서 제거할 정책을 식별합니다. 정책의 GUID 및 이름을 유의합니다. 정책이 비활성 상태로 설정되어 있는지 확인
...
4. 정책 제거가 완료되면 변경 내용을 적용합니다. 이 작업을 수행하기 위해 **Start-InformationBarrierPoliciesApplication** cmdlet을 사용하세요.
    ...
    변경 내용은 사용자에 따라 조직에 적용됩니다. 조직이 규모가 큰 경우 이 프로세스를 완료하는 데 24시간 이상이 걸릴 수 있습니다.
...
2. Identity 매개 **변수와 함께 Stop-InformationBarrierPoliciesApplication** cmdlet을 사용합니다.
