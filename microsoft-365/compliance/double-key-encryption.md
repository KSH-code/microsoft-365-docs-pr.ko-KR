...
1. [<span data-ttu-id="1a9d5-174">DKE 서비스에 대한 소프트웨어 선행 구성</span><span class="sxs-lookup"><span data-stu-id="1a9d5-174">Install software prerequisites for the DKE service</span></span>](#install-software-prerequisites-for-the-dke-service)
...
- <span data-ttu-id="1a9d5-195">C# 코드 Visual Studio 코드</span><span class="sxs-lookup"><span data-stu-id="1a9d5-195">C# for Visual Studio Code</span></span>
...
<span data-ttu-id="1a9d5-248">**전자 메일 권한 부여를 사용하여 DKE에 대한 키 액세스 설정을 설정하려면**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-248">**To set key access settings for DKE using email authorization**</span></span>
...
4. <span data-ttu-id="1a9d5-357">다음 중 하나를 사용하여 생성된 키를 게시합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-357">Do one of the following to publish your generated keys:</span></span>
...
#### <a name="publish-via-ftp"></a><span data-ttu-id="1a9d5-374">FTP를 통해 게시</span><span class="sxs-lookup"><span data-stu-id="1a9d5-374">Publish via FTP</span></span>
...
### <a name="validate-your-deployment"></a><span data-ttu-id="1a9d5-392">배포 유효성 검사</span><span class="sxs-lookup"><span data-stu-id="1a9d5-392">Validate your deployment</span></span>
...
8. <span data-ttu-id="1a9d5-414">URI **리디렉션에서** 이중 키 암호화 서비스의 URI를 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-414">Under **Redirect URIs**, enter the URI of your double key encryption service.</span></span> <span data-ttu-id="1a9d5-415">호스트 이름과 도메인을 모두 포함하여 앱 서비스 URL을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-415">Enter the App Service URL, including both the hostname and domain.</span></span>
   ...
   - <span data-ttu-id="1a9d5-417">입력하는 URL은 DKE 서비스가 배포된 호스트 이름과 일치해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-417">The URL you enter must match the hostname where your DKE service is deployed.</span></span>
   - <span data-ttu-id="1a9d5-418">로컬로 테스트하는 경우 를 **https://localhost:5001** Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-418">If you're testing locally with Visual Studio, use **https://localhost:5001**.</span></span>
   - <span data-ttu-id="1a9d5-419">모든 경우에 스키마는 **https 되어야 합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-419">In all cases, the scheme must be **https**.</span></span>

   <span data-ttu-id="1a9d5-420">호스트 이름을 앱 서비스 호스트 이름과 정확히 일치하도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-420">Ensure the hostname exactly matches your App Service hostname.</span></span> <span data-ttu-id="1a9d5-421">빌드 문제를 해결하기 위해 `localhost` 변경한 것일 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-421">You may have changed it to `localhost` to troubleshoot the build.</span></span> <span data-ttu-id="1a9d5-422">에서 **appsettings.js** 이 값은 에 대해 설정한 호스트 `JwtAudience` 이름입니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-422">In **appsettings.json**, this value is the hostname you set for `JwtAudience`.</span></span>

9. <span data-ttu-id="1a9d5-423">**암시적 부여에서** **ID 토큰** 확인란을 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-423">Under **Implicit grant**, select the **ID tokens** checkbox.</span></span>

10. <span data-ttu-id="1a9d5-424">**저장** 을 선택하여 변경 내용을 저장합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-424">Select **Save** to save your changes.</span></span>

11. <span data-ttu-id="1a9d5-425">왼쪽 창에서 **API** 노출을 선택한 다음 응용 프로그램 ID URI 옆에 있는 설정 을 **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-425">On the left pane, select **Expose an API**, then next to Application ID URI, select **Set**.</span></span>

12. <span data-ttu-id="1a9d5-426">API 표시 **페이지의** 이 **API에** 정의된 범위에서 범위 **추가를 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-426">Still on the **Expose an API** page, in the **Scopes defined by this API** area, select **Add a scope**.</span></span> <span data-ttu-id="1a9d5-427">새 범위에서 다음을 합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-427">In the new scope:</span></span>

    1. <span data-ttu-id="1a9d5-428">범위 이름을 에 로 **user_impersonation.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-428">Define the scope name as **user_impersonation**.</span></span>

    2. <span data-ttu-id="1a9d5-429">동의할 수 있는 관리자 및 사용자를 선택합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-429">Select the administrators and users who can consent.</span></span>

    3. <span data-ttu-id="1a9d5-430">필요한 나머지 값을 정의합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-430">Define any remaining values required.</span></span>

    4. <span data-ttu-id="1a9d5-431">범위 **추가 를 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-431">Select **Add scope**.</span></span>

    5. <span data-ttu-id="1a9d5-432">맨 **위에** 저장을 선택하여 변경 내용을 저장합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-432">Select **Save** at the top to save your changes.</span></span>

13. <span data-ttu-id="1a9d5-433">API 표시 **페이지의** 권한이 부여된 클라이언트 응용 프로그램 **영역에서** 클라이언트 응용 프로그램 **추가를 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-433">Still on the **Expose an API** page, in the **Authorized client applications** area, select **Add a client application**.</span></span>

    <span data-ttu-id="1a9d5-434">새 클라이언트 응용 프로그램에서 다음을 실행합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-434">In the new client application:</span></span>

    1. <span data-ttu-id="1a9d5-435">클라이언트 ID를 로 `d3590ed6-52b3-4102-aeff-aad2292ab01c` 정의합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-435">Define the Client ID as `d3590ed6-52b3-4102-aeff-aad2292ab01c`.</span></span> <span data-ttu-id="1a9d5-436">이 값은 Microsoft Office 클라이언트 ID로, Office에서 키 저장소에 대한 액세스 토큰을 얻을 수 있도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-436">This value is the Microsoft Office client ID, and enables Office to obtain an access token for your key store.</span></span>

    2. <span data-ttu-id="1a9d5-437">권한이 **부여된 범위에서** user_impersonation **선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-437">Under **Authorized scopes**, select the **user_impersonation** scope.</span></span>

    3. <span data-ttu-id="1a9d5-438">응용 **프로그램 추가를 선택합니다.**</span><span class="sxs-lookup"><span data-stu-id="1a9d5-438">Select **Add application**.</span></span>

    4. <span data-ttu-id="1a9d5-439">맨 **위에** 저장을 선택하여 변경 내용을 저장합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-439">Select **Save** at the top to save your changes.</span></span>

    5. <span data-ttu-id="1a9d5-440">이러한 단계를 반복하지만 이번에는 클라이언트 ID를 로 `c00e9d32-3c8d-4a7d-832b-029040e7db99` 정의합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-440">Repeat these steps, but this time, define the client ID as `c00e9d32-3c8d-4a7d-832b-029040e7db99`.</span></span> <span data-ttu-id="1a9d5-441">이 값은 Azure Information Protection 통합 레이블 지정 클라이언트 ID입니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-441">This value is the Azure Information Protection unified labeling client ID.</span></span> 

<span data-ttu-id="1a9d5-442">이제 DKE 서비스가 등록됩니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-442">Your DKE service is now registered.</span></span> <span data-ttu-id="1a9d5-443">[DKE를 사용하여 레이블을 만들어 계속합니다.](#create-sensitivity-labels-using-dke)</span><span class="sxs-lookup"><span data-stu-id="1a9d5-443">Continue by [creating labels using DKE](#create-sensitivity-labels-using-dke).</span></span>

## <a name="create-sensitivity-labels-using-dke"></a><span data-ttu-id="1a9d5-444">DKE를 사용하여 민감도 레이블 만들기</span><span class="sxs-lookup"><span data-stu-id="1a9d5-444">Create sensitivity labels using DKE</span></span>

<span data-ttu-id="1a9d5-445">Microsoft 365 규정 준수 센터에서 새 민감도 레이블을 만들고 다른 경우처럼 암호화를 적용합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-445">In the Microsoft 365 compliance center, create a new sensitivity label and apply encryption as you would otherwise.</span></span> <span data-ttu-id="1a9d5-446">이중 **키 암호화 사용을 선택하고** 키의 끝점 URL을 입력합니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-446">Select **Use Double Key Encryption** and enter the endpoint URL for your key.</span></span>

<span data-ttu-id="1a9d5-447">예를 들어 다음과 같습니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-447">For example:</span></span>

> [!div class="mx-imgBorder"]
> <span data-ttu-id="1a9d5-448">![Microsoft 365 규정 준수 센터에서 이중 키 암호화 사용을 선택합니다.](../media/dke-use-dke.png)</span><span class="sxs-lookup"><span data-stu-id="1a9d5-448">![Select Use Double Key Encryption in the Microsoft 365 compliance center](../media/dke-use-dke.png)</span></span>

<span data-ttu-id="1a9d5-449">추가한 DKE 레이블은 엔터프라이즈용 Microsoft 365 앱의 최신 버전에서 사용자에게 표시됩니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-449">Any DKE labels you add will start appearing for users in the latest versions of Microsoft 365 Apps for enterprise.</span></span>

> [!NOTE]
> <span data-ttu-id="1a9d5-450">클라이언트가 새 레이블로 새로 고쳐지기까지 최대 24시간이 걸릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-450">It may take up to 24 hours for the clients to refresh with the new labels.</span></span>

### <a name="enable-dke-in-your-client"></a><span data-ttu-id="1a9d5-451">클라이언트에서 DKE 사용</span><span class="sxs-lookup"><span data-stu-id="1a9d5-451">Enable DKE in your client</span></span>

<span data-ttu-id="1a9d5-452">Office Insider인 경우 DKE가 사용하도록 설정됩니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-452">If you're an Office Insider, DKE is enabled for you.</span></span> <span data-ttu-id="1a9d5-453">그렇지 않은 경우 다음 레지스트리 키를 추가하여 클라이언트에 대해 DKE를 사용하도록 설정하십시오.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-453">Otherwise, enable DKE for your client by adding the following registry keys:</span></span>
...
## <a name="migrate-protected-files-from-hyok-labels-to-dke-labels"></a><span data-ttu-id="1a9d5-454">HYOK 레이블에서 DKE 레이블로 보호된 파일 마이그레이션</span><span class="sxs-lookup"><span data-stu-id="1a9d5-454">Migrate protected files from HYOK labels to DKE labels</span></span>

<span data-ttu-id="1a9d5-455">원하는 경우 DKE 설정이 완료되면 HYOK 레이블을 사용하여 보호한 콘텐츠를 DKE 레이블로 마이그레이션할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-455">If you want, once you're finished setting up DKE, you can migrate content that you've protected using HYOK labels to DKE labels.</span></span> <span data-ttu-id="1a9d5-456">마이그레이션하려면 AIP 스캐너를 사용하게 됩니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-456">To migrate, you'll use the AIP scanner.</span></span> <span data-ttu-id="1a9d5-457">스캐너 사용을 시작하기 위해 Azure Information Protection 통합 레이블 지정 [스캐너란?을 참조하세요.](/azure/information-protection/deploy-aip-scanner)</span><span class="sxs-lookup"><span data-stu-id="1a9d5-457">To get started using the scanner, see [What is the Azure Information Protection unified labeling scanner?](/azure/information-protection/deploy-aip-scanner).</span></span>

<span data-ttu-id="1a9d5-458">콘텐츠를 마이그레이션하지 않는 경우 HYOK로 보호된 콘텐츠는 영향을 받지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="1a9d5-458">If you don't migrate content, your HYOK protected content will remain unaffected.</span></span>
