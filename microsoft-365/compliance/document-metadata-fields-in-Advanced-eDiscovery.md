...
|그룹 Id| GroupID|  |그룹화 된 콘텐츠의 그룹 ID입니다.|
...
|최신 첨부 파일| IsModernAttachment|  |이 파일은 최신 첨부 파일이 나 연결 된 파일입니다.|
|문서 버전의 | IsFromDocumentVersion |  |현재 문서가 다른 문서 버전과 다릅니다.|
|전자 메일 첨부 파일 | IsEmailAttachment|  |이 항목은 메시지에 첨부 된 항목으로 표시 되는 전자 메일 첨부 파일에서 가져온 것입니다.|
|인라인 첨부 파일| IsInlineAttachment|  |이 줄은 인라인으로 첨부 되어 메시지 본문에 표시 됩니다.|
