...
다음 표에서는 SharePoint 및 OneDrive에서 전자 메일 주소 및 암호화된 문서에 첨부된 암호화된 파일에 대해 Microsoft 365 eDiscovery 도구에서 수행할 수 있는 지원되는 작업을 설명합니다. 이러한 지원되는 작업은 검색 조건과 일치하는 암호화된 파일에 대해 수행할 수 있습니다. 값은 해당 eDiscovery 도구에서 기능을 사용할 수 없는 `N/A` 경우를 나타냅니다.
...
**참고:** 암호화를 적용한 민감도 레이블이 다음 설정 중 하나를 사용하여 구성된 경우 eDiscovery는 SharePoint 및 OneDrive에서 암호화된 파일을 지원하지 않습니다.
...
이전 설정으로 암호화된 문서는 eDiscovery 검색에서 반환할 수 있습니다. 제목, 만든 이 또는 수정한 날짜와 같은 문서 속성이 검색 조건과 일치할 때 이러한 일이 있을 수 있습니다. 이러한 문서가 검색 결과에 포함될 수 있는 경우 미리 보거나 검토할 수 없습니다. 이러한 문서는 Advanced eDiscovery에서 내보낼 때도 암호화된 상태로 유지됩니다.
