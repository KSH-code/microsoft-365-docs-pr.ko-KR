...
커넥터를 만든 후 데이터 커넥터 페이지로 돌아가 새 커넥터의 가져오기 프로세스 진행률을  볼 수 있습니다(필요한 경우 새로 고침을 선택하여 커넥터 목록을 업데이트합니다).  상태 **열의 값이** 시작 **대기 중입니다.** 초기 가져오기 프로세스를 시작하는 데 최대 24시간이 소요됩니다. 커넥터가 처음으로 실행되고 LinkedIn 항목을 가져오면 커넥터가 24시간마다 한 번 실행되고 지난 24시간 동안 LinkedIn 회사 페이지에 만들어진 새 항목을 모두 가져올 수 있습니다.
