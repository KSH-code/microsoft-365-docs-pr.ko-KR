...
통신 준수 기능을 관리 하기 위한 권한을 구성 하는 데 사용 되는 역할에는 다섯 가지가 있습니다. Microsoft 365 준수 센터에서 **통신 준수** 를 메뉴 옵션으로 사용할 수 있도록 설정 하 고 이러한 구성 단계를 계속 하려면 *통신 준수 관리자* 역할이 할당 되어야 합니다.

통신 정책 및 알림을 관리 하는 방법에 따라 관리자, 검토자 및 investigators에 대 한 새 역할 그룹을 하나 이상 만들어야 합니다. 특정 역할 그룹에 사용자를 할당 하 여 서로 다른 통신 준수 기능 영역을 관리할 수 있는 옵션이 제공 됩니다. 또는 역할 그룹을 하나 만들어서 모든 통신 준수 역할을 해당 그룹에 할당할 수 있습니다. 준수 관리 요구 사항에 가장 적합 한 단일 역할 그룹 또는 여러 역할 그룹을 만듭니다.
...
### <a name="option-1-create-a-new-role-group-with-all-communication-compliance-roles"></a>옵션 1: 모든 통신 준수 역할을 사용 하 여 새 역할 그룹 만들기
...
5. **역할 선택을** 선택 하 고 **추가**를 선택 합니다. 다음 역할에 대 한 확인란을 선택 합니다.
...
6. **추가** 및 **완료**를 선택 하 고 **다음** 을 선택 하 여 계속 합니다.

7. **구성원 선택을** 선택 하 고 **추가**를 선택 합니다. 정책을 만들 모든 사용자 및 그룹에 대 한 확인란을 선택 하 고 정책 일치가 포함 된 메시지를 관리 한 다음 **추가** 및 **완료**를 선택 합니다. **다음**을 선택합니다.

8. 마칠 **역할 그룹 만들기** 를 선택 합니다.

### <a name="option-2-create-new-role-groups-with-different-communication-compliance-roles"></a>옵션 2: 서로 다른 통신 준수 역할을 사용 하 여 새 역할 그룹 만들기

여러 역할 그룹을 만들어 조직의 여러 사용자 간에 통신 준수 액세스 및 책임을 분할 합니다. 각각의 새 역할 그룹에 대해 서로 다른 통신 준수 역할을 할당 합니다.
...
5. **역할 선택을** 선택 하 고 **추가**를 선택 합니다. 이 그룹에 할당 하려는 통신 준수 역할의 확인란을 선택 합니다. 예를 들어 조직의 준수 분석가가이 역할 그룹을 사용할 경우에는 *통신 준수 분석* 및 *통신 준수 사례 관리* 역할을 선택 합니다. 이 역할 그룹이 준수 investigators 경우에는 *통신 준수 조사* 및 *통신 준수 사례 관리* 역할을 선택 합니다.
