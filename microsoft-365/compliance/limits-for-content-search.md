...
ms.locfileid: "49371528"
---
# <a name="limits-for-content-search-in-the-security--compliance-center"></a>보안 & 준수 센터의 콘텐츠 검색에 대 한 제한

> [!NOTE]
> 이 항목의 제한은 Exchange Online의 In-Place eDiscovery와 SharePoint Online의 eDiscovery 센터에 대 한 현재 제한과는 다릅니다.
...
|조직에서 동시에 실행할 수 있는 최대 콘텐츠 검색 수입니다.  <br/> |kb  <br/> |
...
|콘텐츠 검색에서 항목을 내보낼 수 있는 최대 사서함 수입니다. 내보내려는 콘텐츠 검색에이 제한 보다 많은 원본 사서함이 있는 경우 내보내기가 실패 합니다. 자세한 내용은 [Export Content Search results](export-search-results.md)를 참조 하세요.  <br/> |100,000  <br/> |

> [!NOTE]
> <sup>1</sup> 단일 검색에서 사서함을 무제한으로 검색할 수 있지만 Office 365 보안 & 준수 센터 또는 Microsoft 365 준수 센터의 EDiscovery 내보내기 도구를 사용 하 여 최대 10만 사서함에서 내보낸 검색 결과만 다운로드할 수 있습니다. 10만 개 이상의 사서함에서 검색 결과를 다운로드 하려면 보안 & 준수 센터 PowerShell을 사용 해야 합니다. 자세한 내용 및 샘플 스크립트에 대 한 자세한 내용은 [10만 개 이상의 사서함에서 결과 내보내기를](export-search-results.md#exporting-results-from-more-than-100000-mailboxes)참조 하십시오. <br/><br/> <sup>2</sup> SharePoint 및 비즈니스용 OneDrive 위치를 검색 하는 경우 검색 되는 사이트의 url에 포함 된 문자가이 한도를 초과 하 여 계산 됩니다. <br/><br/> <sup>3</sup> 비 구 쿼리 (큰따옴표를 사용 하지 않는 키워드 값)의 경우에는 특수 접두사 인덱스를 사용 합니다. 이렇게 하면 문서에서 단어가 나타나지만 문서에서 나타나는 위치를 알 수 없습니다. 구문 쿼리 (큰따옴표를 사용한 키워드 값)를 수행 하려면 문서 내에서 구의 단어를 비교 해야 합니다. 즉, 구 쿼리에 접두사 인덱스를 사용할 수 없습니다. 이 경우에는 접두사를 확장 하는 모든 가능한 단어를 내부적으로 확장 합니다. 예를 들면  `"time*"` 로 확장 될 수 있습니다  `"time OR timer OR times OR timex OR timeboxed OR …"` . 1만은 쿼리와 일치 하는 문서 수가 아닌 단어를 확장할 수 있는 최대 variant 개수입니다. 구문이 아닌 용어의 상한 값은 없습니다. 
...
| 인덱싱 제한 | 최대값 | 설명 |
|:-----|:-----|:-----|
|최대 첨부 파일 크기|150MB  <br/> |인덱싱을 위해 구문 분석할 전자 메일 첨부 파일의 최대 크기입니다. 이 제한 보다 큰 모든 첨부 파일은 인덱싱할 수 있도록 구문 분석 되지 않으며 첨부 파일이 있는 메시지는 부분 인덱싱되는 것으로 표시 됩니다.  <br/> <br/>**참고:** 구문 분석은 인덱싱 서비스가 첨부 파일에서 텍스트를 추출 하 고, 문장 부호와 공백 같은 불필요 한 문자를 제거한 다음, 텍스트를 토큰화 라는 프로세스의 단어로 나누어 인덱스에 저장 하는 프로세스입니다.           |
