...
12. **정책 시뮬레이션을 지금 또는 나중에 실행할지 결정** 페이지에서 지금 자동 레이블 지정 정책을 실행할 준비가 되었으면, 시뮬레이션 모드에서 **시뮬레이션 모드에서 정책 실행**을 선택합니다. 그렇지 않으면 **정책을 해제로 두기**를 선택합니다. **다음**을 선택합니다. 
...
- **시뮬레이션** 구역의 정책의 경우, 페이지 맨 위의 다음의 탭에서 **편집** 옵션을 선택합니다.
