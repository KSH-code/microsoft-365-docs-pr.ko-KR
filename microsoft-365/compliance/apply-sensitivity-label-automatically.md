...
**학습 가능한 분류자** 옵션을 선택하는 경우 미리 학습된 분류자 또는 사용자 지정 학습 가능한 분류자 하나 이상을 선택합니다.
...
시뮬레이션 모드는 최대 1,000,000개의 일치 파일을 지원합니다. 자동 레이블 지정 정책에서 이 수보다 많은 파일이 일치하는 경우 레이블을 적용하는 정책을 켤 수 없습니다. 이 경우 일치하는 파일이 줄어들도록 자동 레이블 지정 정책을 다시 구성하고 시뮬레이션을 다시 실행해야 합니다. 이 최대 1,000,000개의 일치 파일은 시뮬레이션 모드에만 적용되며 민감도 레이블을 적용하기 위해 이미 설정된 자동 레이블 지정 정책에는 적용되지 않습니다.
...
3. 결과를 검토하고 필요한 경우 정책을 구체화합니다. 예를 들어 가양성을 줄이기 위해 정책 규칙을 편집하거나 일치하는 파일 수가 1,000,000개를 초과하지 않도록 일부 사이트를 제거해야 할 수 있습니다. 응답 모드를 다시 실행하고 다시 완료될 때까지 기다립니다.
...
- 시뮬레이션 모드에서 자동 레이블 지정 정책당 최대 1,000,000개의 일치 파일입니다. 시뮬레이션 중에 일치하는 파일 수가 이보다 많으면 정책을 켤 수 없습니다. 더 적은 수의 파일과 일치하도록 자동 레이블 지정 정책을 재구성하고 시뮬레이션을 다시 실행합니다.
