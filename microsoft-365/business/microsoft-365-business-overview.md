...
- 전자 메일 및 문서의 링크를 실시간으로 검색 하 여 안전 하지 않은 웹 사이트 (안전한 링크)를 차단 합니다.

- 샌드박스 환경에서 전자 메일 첨부 파일에 대 한 고급 분석을 수행 하 여 새로 개발한 맬웨어 (안전한 첨부 파일)를 검색 합니다. 

- 기계 학습 모델 및 가장 검색을 사용 하 여 고급 공격 (Defender for Office 365 intelligence의 피싱 방지)을 제공 하는 피싱 방지 정책을 사용 하도록 설정 합니다. 
