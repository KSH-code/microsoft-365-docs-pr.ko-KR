...
1. 먼저 [다음 지침](connect-to-microsoft-365-powershell.md)을 사용 하 여 AAD에 연결 합니다.
...
3. 이 목록에 내부 및 외부 MAPI/HTTP, EWS, ActiveSync, OAB 및 자동 검색 레코드가 표시 되지 않으면 아래 명령을 사용 하 여 추가 해야 합니다 (예: Url은 ' `mail.corp.contoso.com` ' 및 ' ' 임 `owa.contoso.com` ). **replace the example URLs with your own**
...
TCP 443에서 Exchange server를 사용 하는 온-프레미스 고객의 경우 다음 IP 범위를 허용 목록.
