...
사용자는 암호 쓰기 저장을 사용하여 Azure AD(Azure Active Directory)를 통해 암호를 업데이트한 다음 로컬 AD DS(Active Directory 도메인 서비스)로 복제됩니다. 암호 쓰기 저장을 사용하는 경우 사용자는 원래 사용자 계정이 저장된 모든 프레미스 AD DS를 통해 암호를 업데이트할 수 없습니다. 이렇게 하면 원격 액세스 연결이 없는 로밍 또는 원격 사용자가 해당 On-프레미스 네트워크에 연결될 수 있습니다.

이 문서에서는 암호 쓰기 저장을 위해 Microsoft 365 테스트 환경을 구성하는 방법을 설명합니다.

암호 쓰기 저장을 위한 테스트 환경 구성에는 다음 두 단계가 있습니다.
...
먼저 암호 해시 [동기화의 지침을 따릅니다.](password-hash-sync-m365-ent-test-environment.md) 결과 구성은 다음과 같습니다.
...
4. **user1** 창에서 역할 **옆에** 있는 **편집을 선택합니다.**
...
2. APP1의 바탕 화면에서 **시작을** 선택하고 활성을 **입력한** 다음 Active Directory 사용자 및 **컴퓨터를 선택합니다.**

3. 메뉴 표시줄에서 보기를 **선택합니다.** 고급 **기능을** 사용하도록 설정하지 않은 경우 해당 기능을 선택하여 사용하도록 설정합니다.

4. 트리 창에서 도메인을 선택하고 보류(또는 마우스 오른쪽 단추 클릭)한 다음 **속성을** 선택한 다음 보안 **탭을** 선택합니다.
...
4. 추가 **작업 페이지에서** 동기화 **옵션** 사용자 지정을 선택하고 다음을 **선택합니다.**

5. Azure **AD에 연결 페이지에서** 전역 관리자 계정 자격 증명을 입력하고 다음을 **선택합니다.**

6. Connect **directories** and **Domain/OU filtering** pages, select **Next**.

7. 선택적 **기능 페이지에서** 암호 쓰기 저장을 선택하고 다음을 **선택합니다.** 

8. 구성 **준비 완료 페이지에서**  구성을 선택하고 프로세스가 완료될 때까지 기다릴 수 있습니다.

9. 구성이 완료되면 종료를 **선택합니다.**

이제 시뮬레이트된 인트라넷의 가상 네트워크에 연결되지 않은 컴퓨터의 사용자에 대한 암호 쓰기 저장을 테스트할 준비가 완료되었습니다.
