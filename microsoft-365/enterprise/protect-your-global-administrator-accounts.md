...
정보 수집 및 피싱 공격을 비롯한 Microsoft 365 구독의 보안 위반은 일반적으로 Microsoft 365 전역 관리자 계정의 자격 증명을 침해하여 수행됩니다. 클라우드의 보안은 사용자와 Microsoft 간의 파트너 관계입니다.
  
- Microsoft 클라우드 서비스는 신뢰할 수 있는 보안 기반을 토대로 구축됩니다. Microsoft는 데이터 및 응용 프로그램을 보호하는 데 도움이 되는 보안 제어 및 기능을 제공합니다.
    
- 데이터 및 ID와 보호에 대한 책임, 프레미스 리소스의 보안 및 제어하는 클라우드 구성 요소의 보안을 소유합니다.
...
> [!Note]
> 이 문서에서는 전역 관리자 계정에 중점을 두지만 구독의 데이터에 액세스할 수 있는 광범위한 권한이 있는 추가 계정(예: eDiscovery 관리자 또는 보안 또는 규정 준수 관리자 계정)을 동일한 방식으로 보호해야 하는지 여부를 고려해야 합니다. <br > 전역 관리자 계정은 라이선스를 추가하지 않고 만들 수 있습니다.
...
7. 1단계에서 전역 관리자 역할이 할당된 각 기존 사용자 계정에 대해:
...
- 구독에서 전역 관리자 역할이 할당된 사용자 계정은 새로운 전용 전역 관리자 계정 집합뿐입니다. 다음 PowerShell 명령을 사용하여 이를 검증합니다.
...
자세한 내용은 [Microsoft 365의 MFA를 참조하세요.](https://docs.microsoft.com/microsoft-365/admin/security-and-compliance/multi-factor-authentication-microsoft-365)
...
자세한 내용은 [Azure AD Privileged Identity Management를 참조하세요.](https://docs.microsoft.com/azure/active-directory/active-directory-privileged-identity-management-configure)
...
- [하이브리드 ID를 사용하는](prepare-for-directory-synchronization.md) 경우 디렉터리 동기화 준비
