...
Azure AD(Azure Active Directory) Connect(이전의 디렉터리 동기화 도구, 디렉터리 동기화 도구 또는 DirSync.exe 도구)는 도메인 가입 서버에 설치하여 Ad DS(Active Directory 도메인 서비스) 사용자를 Microsoft 365 구독의 Azure AD 테넌트와 동기화하는 응용 프로그램입니다. Microsoft 365는 디렉터리 서비스에 Azure AD를 사용 합니다. Microsoft 365 구독에는 Azure AD 테넌트가 포함됩니다. 이 테넌트는 Azure의 다른 SaaS 응용 프로그램 및 앱을 포함하여 다른 클라우드 워크로드에서 조직의 ID를 관리하기 위해 사용할 수도 있습니다.
...
> [!NOTE]
> 이 문서에서는 단일 포리스트에서 단일 도메인의 동기화에 대해 설명합니다. Azure AD Connect는 Active Directory 포리스트의 모든 AD DS 도메인을 Microsoft 365와 동기화합니다. Microsoft 365와 동기화할 Active Directory 포리스트가 여러 개 있는 경우 [Multi-forest Directory Sync with Single Sign-On 시나리오를 참조합니다.](/azure/active-directory/hybrid/whatis-hybrid-identity) 
  
## <a name="overview-of-deploying-microsoft-365-directory-synchronization-in-azure"></a>Azure에서 Microsoft 365 디렉터리 동기화 배포 개요

다음 다이어그램은 Azure의 가상 머신(디렉터리 동기화 서버)에서 실행되는 Azure AD Connect를 보여 주며, 이 포리스트를 Microsoft 365 구독과 동기화합니다.
...
-  Azure AD Connect는 계정 및 암호에 대한 변경 내용을 Microsoft 365 구독의 Azure AD 인스턴스로 전송합니다. 디렉터리 동기화 서버가 사내 네트워크의 확장된 부분에 있기 때문에 이러한 변경 내용은 사내 네트워크의 프록시 서버를 통해 전송됩니다.
    
> [!NOTE]
> 이 솔루션은 단일 Active Directory 포리스트에서 단일 Active Directory 도메인의 동기화에 대해 설명합니다. Azure AD Connect는 Active Directory 포리스트의 모든 Active Directory 도메인을 Microsoft 365와 동기화합니다. Microsoft 365와 동기화할 Active Directory 포리스트가 여러 개 있는 경우 [Multi-forest Directory Sync with Single Sign-On 시나리오를 참조합니다.](/azure/active-directory/hybrid/whatis-hybrid-identity) 
...
2. Azure의 도메인에 가입된 가상 머신에 [Azure AD Connect를](https://www.microsoft.com/download/details.aspx?id=47594) 설치한 다음, Microsoft 365에 대한 Azure AD Connect를 동기화합니다. 여기에는 이러한 과정이 수반됩니다.
    ...
    Azure AD Connect를 구성하려면 Azure AD 관리자 계정 및 AD DS 엔터프라이즈 관리자 계정의 자격 증명(사용자 이름 및 암호)이 필요합니다. Azure AD Connect는 즉시 실행되어 계속 실행되어, Microsoft 365에 대한 사내 AD DS 포리스트를 동기화합니다.
...
> [!NOTE]
> 이 솔루션에서는 단일 AD DS 포리스트를 Microsoft 365와 동기화하는 데 대해 설명하고 있습니다. 이 문서에서 설명하는 토폴로지는 이 솔루션을 구현하는 한 가지 방법만 제공합니다. 조직의 토폴로지가 고유한 네트워크 요구 사항 및 보안 고려 사항에 따라 다를 수 있습니다. 
  
## <a name="plan-for-hosting-a-directory-sync-server-for-microsoft-365-in-azure"></a>Azure에서 Microsoft 365용 디렉터리 동기화 서버 호스팅 계획
...
- Active Directory 통합 기능이 포함된 Microsoft 365 구독이 있습니다. Microsoft 365 구독에 대한 자세한 내용은 [Microsoft 365 구독 페이지로 이동하세요.](https://products.office.com/compare-all-microsoft-office-products?tab=2)
    
- Azure AD Connect를 실행하여 Microsoft 365와 프레미스 AD DS 포리스트를 동기화하는 Azure Virtual Machine 하나를 프로비전합니다.
...
배포 후 Microsoft 365에서 새 사용자 계정에 대한 위치 및 라이선스도 할당해야 합니다.
...
2. 디렉터리 동기화 서버에서 Microsoft [365에](set-up-directory-synchronization.md) 대한 디렉터리 동기화 설정 문서를 열고 암호 해시 동기화를 사용하여 디렉터리 동기화에 대한 지침을 따르십시오.
...
### <a name="assign-locations-and-licenses-to-users-in-microsoft-365"></a>Microsoft 365에서 사용자에게 위치 및 라이선스 할당

Azure AD Connect는 Microsoft 365 구독에 계정을 추가합니다. 그러나 사용자가 Microsoft 365에 로그인하고 해당 서비스를 사용하려면 위치 및 라이선스를 사용하여 계정을 구성해야 합니다. 다음 단계를 사용하여 위치를 추가하고 해당 사용자 계정에 대한 라이선스를 활성화합니다.
  
1. [Microsoft 365 관리 센터에 로그인하고](https://admin.microsoft.com)관리자 를 **클릭합니다.**
...
[Microsoft 365에 대한 디렉터리 동기화 설정](set-up-directory-synchronization.md)
