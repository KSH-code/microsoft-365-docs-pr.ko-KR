...
---
# <a name="step-2-deploy-windows-10-enterprise-for-existing-devices-as-an-in-place-upgrade"></a><span data-ttu-id="6e8ac-104">2 단계: 전체 업그레이드로 기존 장치에 대 한 Windows 10 엔터프라이즈 배포</span><span class="sxs-lookup"><span data-stu-id="6e8ac-104">Step 2: Deploy Windows 10 Enterprise for existing devices as an in-place upgrade</span></span>

<span data-ttu-id="6e8ac-105">*Microsoft 365 Enterprise의 E3와 e 5 버전에 적용 하는이 문서*</span><span class="sxs-lookup"><span data-stu-id="6e8ac-105">*This article applies to both the E3 and E5 versions of Microsoft 365 Enterprise*</span></span>

![](./media/deploy-foundation-infrastructure/win10enterprise_icon-small.png)

<span data-ttu-id="6e8ac-p101">현재 Windows 10 Windows 7 또는 Windows 8.1 실행 하는 Pc를 업그레이드 하는 전체 업그레이드를 통해의 가장 간단한 경로가 합니다. 완전히 프로세스를 자동화 하는 System Center Configuration Manager (Configuration Manager) 작업 시퀀스를 사용할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p101">The simplest path to upgrade PCs currently running Windows 7 or Windows 8.1 to Windows 10 is through an in-place upgrade. You can use a System Center Configuration Manager (Configuration Manager) task sequence to completely automate the process.</span></span> 

<span data-ttu-id="6e8ac-p102">Windows 7 또는 Windows 8.1 실행 하는 기존 컴퓨터에 있는 조직 Windows 10을 배포 하는 경우이 경로 권장 합니다. Windows 설치 프로그램 (Setup.exe) 모든 데이터, 설정, 응용 프로그램을 자동으로 보존, 전체 업그레이드를 수행 하 고 기존 운영 체제 버전에서 만든 드라이버를 활용 합니다. 이 경우 모든 복잡 한 배포 인프라가 필요 하지 않으므로 최소한의 IT 작업을 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p102">If you have existing computers running Windows 7 or Windows 8.1, we recommend this path if your organization is deploying Windows 10. This leverages the Windows installation program (Setup.exe) to perform an in-place upgrade, which automatically preserves all data, settings, applications, and drivers from the existing operating system version. This requires the least IT effort, because there is no need for any complex deployment infrastructure.</span></span>

<span data-ttu-id="6e8ac-111">구성 및 Configuration Manager를 사용 하 여 전체 업그레이드로 Windows 10 Enterprise 이미지를 배포 하려면 다음이 단계를 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-111">Follow these steps to configure and deploy a Windows 10 Enterprise image using Configuration Manager as an in-place upgrade.</span></span>

## <a name="part-1-verify-readiness-to-upgrade-windows"></a><span data-ttu-id="6e8ac-112">1 부: Windows를 업그레이드할 준비 상태 확인</span><span class="sxs-lookup"><span data-stu-id="6e8ac-112">Part 1: Verify readiness to upgrade Windows</span></span>

<span data-ttu-id="6e8ac-p103">Windows 분석의 업그레이드 준비 상태 기능을 사용 하면 먼저 강력한 인 사이트와 컴퓨터, 응용 프로그램 및 조직 전체에서 아니요에 드라이버에 대 한 권장 사항을 제공 추가 비용 및 추가 인프라 요구 사항입니다. 이 새로운 서비스는 Microsoft 권장 되는 정보를 기반으로 워크플로 사용 하 여 업데이트 프로젝트를 업그레이드 하 고 기능을 안내 합니다. 최신 인벤토리 데이터를 분산 비용 및 업그레이드 프로젝트의 위험에 있습니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p103">First, use the Upgrade Readiness capability of Windows Analytics to provide powerful insights and recommendations about the computers, applications, and drivers in your organization, at no extra cost and without additional infrastructure requirements. This new service guides you through upgrade and feature update projects using a workflow based on Microsoft recommended practices. Up-to-date inventory data allows you to balance cost and risk in your upgrade projects.</span></span>

<span data-ttu-id="6e8ac-116">자세한 내용은, 시작, 사용 및 업그레이드 준비 문제를 해결을 [업그레이드 준비 상태와 관리 Windows 업그레이드](https://docs.microsoft.com/windows/deployment/upgrade/manage-windows-upgrades-with-upgrade-readiness) 를 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-116">See [Manage Windows upgrades with Upgrade Readiness](https://docs.microsoft.com/windows/deployment/upgrade/manage-windows-upgrades-with-upgrade-readiness) to learn more, get started, use, and troubleshoot Upgrade Readiness.</span></span>

<span data-ttu-id="6e8ac-p104">다음 Windows 10 Windows 7 또는 이상의 운영 체제를 업그레이드 하려면 System Center Configuration Manager (현재 분기)를 사용 하 여 가이드에 따라 합니다. 모든 위험성이 높은 배포와 마찬가지로 계속 하기 전에 사용자 데이터를 백업 하는 것이 좋습니다. OneDrive 클라우드 저장소 사용이 허가 된 Microsoft 365 사용자에 대해 사용할 준비가 되었는지 하 고 안전 하 게 그 파일을 저장 하는데 사용할 수 있습니다. 자세한 정보를 [OneDrive 빠른 시작 가이드를](https://aka.ms/ODfBquickstartguide)참조 하십시오. 이 페이지에 액세스 하려면 테 넌 트 관리 또는 전역 관리자는 Office 365 또는 Microsoft 365 테 넌 트에 로그인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p104">Next, follow the guide to use System Center Configuration Manager (Current Branch) to upgrade Windows 7 or later operating system to Windows 10. As with any high-risk deployment, we recommend backing up user data before proceeding. OneDrive cloud storage is ready to use for licensed Microsoft 365 users and can be used to securely store their files. For more info, see [OneDrive quick start guide](https://aka.ms/ODfBquickstartguide). To access this page, you must sign in as a tenant admin or global admin in an Office 365 or Microsoft 365 tenant.</span></span>

<span data-ttu-id="6e8ac-122">Configuration Manager 버전 및 지원 되는 해당 Windows 10 클라이언트 버전 목록에 대 한 [지원에 대 한 Windows 10 System Center Configuration Manager에 대 한](https://aka.ms/supportforwin10sccm)참조입니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-122">For a list of Configuration Manager versions and the corresponding Windows 10 client versions that are supported, see [Support for Windows 10 for System Center Configuration Manager](https://aka.ms/supportforwin10sccm).</span></span>

<span data-ttu-id="6e8ac-123">**Windows를 업그레이드할 준비 상태를 확인 하려면**</span><span class="sxs-lookup"><span data-stu-id="6e8ac-123">**To verify readiness to upgrade Windows**</span></span>

<span data-ttu-id="6e8ac-124">Windows 10 배포를 시작 하기 전에 이러한 요구 사항을 검토 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-124">Review these requirements before starting your Windows 10 deployment:</span></span>

- <span data-ttu-id="6e8ac-p105">**Windows 버전에 대 한 가능한 업그레이드** -Windows 10 Enterprise로의 업그레이드에 사용할 수 있는 버전의 Windows 7 또는 Windows 8.1 장치로 실행 해야 합니다. 지원 되는 버전의 목록에 대 한 [Windows 10 업그레이드 경로](https://aka.ms/win10upgradepaths)참조 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p105">**Windows editions eligible for upgrade** - Your devices must be running editions of Windows 7 or Windows 8.1 that are eligible for upgrade to Windows 10 Enterprise. For a list of supported editions, see [Windows 10 upgrade paths](https://aka.ms/win10upgradepaths).</span></span> 
- <span data-ttu-id="6e8ac-p106">**지원 되는 장치** -Windows 8.1 호환 되는 대부분의 컴퓨터에 Windows 10과 호환 됩니다. 제대로 작동 하려면 장치에 대 한 Windows 10에서 업데이트 된 드라이버를 설치 해야 합니다. 추가 정보에 대 한 [Windows 10 사양](https://aka.ms/windows10specifications) 를 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p106">**Supported devices** - Most computers that are compatible with Windows 8.1 will be compatible with Windows 10. You may need to install updated drivers in Windows 10 for your devices to properly function. See [Windows 10 specifications](https://aka.ms/windows10specifications) for more info.</span></span>
- <span data-ttu-id="6e8ac-130">**배포 준비** -구성 배포를 시작 하기 전에 다음 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-130">**Deployment preparation** - Make sure you have the following before you start configuring the deployment:</span></span>
    - <span data-ttu-id="6e8ac-p107">Windows 10 설치 미디어-설치 미디어 이미 탑재 ISO와는 별도 드라이브에 배치 되어야 합니다. [MSDN 구독자 다운로드](https://aka.ms/msdn-subscriber-downloads) 또는 [볼륨 라이선스 서비스 센터](https://aka.ms/mvlsc)에서 ISO를 얻을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p107">Windows 10 installation media - The installation media must be located on a separate drive, with the ISO already mounted. You can obtain the ISO from [MSDN Subscriber Downloads](https://aka.ms/msdn-subscriber-downloads) or from the [Volume Licensing Service Center](https://aka.ms/mvlsc).</span></span>
    - <span data-ttu-id="6e8ac-p108">사용자 데이터-의 백업을 업그레이드에서 사용자 데이터를 마이그레이션할 수는 있지만 것이 가장 좋습니다 백업 시나리오를 구성 하 합니다. 예, OneDrive 계정, 이동 BitLocker 암호화 USB 플래시 드라이브 또는 네트워크 파일 서버에 모든 사용자 데이터를 내보냅니다. 자세한 내용은 [Windows에서 데이터를 전송 하거나 백업](https://aka.ms/backuptransferdatawindows)을 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p108">Backups of user data - Although user data will be migrated in the upgrade, best practice is to configure a backup scenario. For example, export all user data to a OneDrive account, BitLocker To Go-encrypted USB flash drive, or network file server. For more information, see [Back up or transfer data in Windows](https://aka.ms/backuptransferdatawindows).</span></span>
- <span data-ttu-id="6e8ac-p109">**환경 준비** -운영 체제 배포를 위한 준비 하는 기존 Configuration Manager 서버 구조를 사용 합니다. 기본 설정 외에도 다음과 같은 구성 구성 관리자 환경에서 만들어야 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p109">**Environment preparation** - You will use an existing Configuration Manager server structure to prepare for operating system deployment. In addition to the base setup, the following configurations should be made in the Configuration Manager environment:</span></span>
    1. <span data-ttu-id="6e8ac-138">[Active Directory 스키마 확장](https://aka.ms/extendadschema) 을 선택 하 고 [시스템 관리 컨테이너를 만듭니다](https://aka.ms/createsysmancontainer).</span><span class="sxs-lookup"><span data-stu-id="6e8ac-138">[Extend the Active Directory Schema](https://aka.ms/extendadschema) and [create a System Management container](https://aka.ms/createsysmancontainer).</span></span>
    2. <span data-ttu-id="6e8ac-p110">Enable Active Directory 포리스트 검색 및 Active Directory 시스템 검색 합니다. 자세한 정보에 대 한 [구성을 검색 방법에 대 한 System Center Configuration Manager를](https://aka.ms/configurediscoverymethods)참조 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p110">Enable Active Directory Forest Discovery and Active Directory System Discovery. For more info, see [Configure discovery methods for System Center Configuration Manager](https://aka.ms/configurediscoverymethods).</span></span>
    3. <span data-ttu-id="6e8ac-p111">IP 범위 경계 및 콘텐츠 및 사이트 할당에 대 한 경계 그룹을 만듭니다. 자세한 정보를 [정의 사이트 경계 및 System Center Configuration Manager에 대 한 경계 그룹을](https://aka.ms/definesiteboundaries)참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p111">Create IP range boundaries and boundary group for content and site assignment. For more info, see [Define site boundaries and boundary groups for System Center Configuration Manager](https://aka.ms/definesiteboundaries).</span></span>
    4. <span data-ttu-id="6e8ac-p112">추가 하 고 reporting services 지점 역할 구성 관리자를 구성 합니다. 자세한 정보, [구성 관리자에서 보고 구성](https://aka.ms/configurereporting)을 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p112">Add and configure the Configuration Manager reporting services point role. For more info, see [Configuring Reporting in Configuration Manager](https://aka.ms/configurereporting).</span></span>
    5. <span data-ttu-id="6e8ac-145">패키지에 대 한 파일 시스템 폴더 구조를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-145">Create a file system folder structure for packages.</span></span>
    6. <span data-ttu-id="6e8ac-146">패키지에 대 한 Configuration Manager 콘솔 폴더 구조를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-146">Create a Configuration Manager console folder structure for packages.</span></span>
    7. <span data-ttu-id="6e8ac-147">System Center Configuration Manager (현재 분기) 업데이트 하 고 모든 추가 Windows 10 필수 구성 요소를 설치 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-147">Install System Center Configuration Manager (Current Branch) updates and any additional Windows 10 prerequisites.</span></span>

## <a name="part-2-add-a-windows-10-os-image-using-configuration-manager"></a><span data-ttu-id="6e8ac-148">2 부: 구성 관리자를 사용 하 여 Windows 10 운영 체제 이미지를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-148">Part 2: Add a Windows 10 OS image using Configuration Manager</span></span>
<span data-ttu-id="6e8ac-p113">이제 전체 Windows 10 설치 미디어를 포함 하는 운영 체제 업그레이드 패키지를 만드는 필요 합니다. 다음 단계에서 구성 관리자를 사용 하 여 Windows 10 Enterprise x64에 대 한 업그레이드 패키지를 만들려면 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p113">Now you'll need to create an operating system upgrade package that contains the full Windows 10 installation media. In the following steps, you’ll use Configuration Manager to create an upgrade package for Windows 10 Enterprise x64.</span></span>

<span data-ttu-id="6e8ac-151">**Configuration Manager를 사용 하 여 Windows 10 운영 체제 이미지를 추가 하려면**</span><span class="sxs-lookup"><span data-stu-id="6e8ac-151">**To add a Windows 10 OS image using Configuration Manager**</span></span>

1. <span data-ttu-id="6e8ac-152">Configuration Manager 콘솔을 사용 하 여 **소프트웨어 라이브러리** 작업 영역에, **운영 체제 업그레이드 패키지** 노드를 마우스 오른쪽 단추로 클릭 하 고 **운영 체제 업그레이드 패키지 추가**선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-152">Using the Configuration Manager console, in the **Software Library** workspace, right-click the **Operating System Upgrade Packages** node, and then select **Add Operating System Upgrade Package**.</span></span>
2. <span data-ttu-id="6e8ac-153">**데이터 원본** 페이지에서 Windows 10 Enterprise x64 미디어의 UNC 경로 지정 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-153">On the **Data Source** page, specify the UNC path to the Windows 10 Enterprise x64 media, and then select **Next**.</span></span>
3. <span data-ttu-id="6e8ac-154">**일반** 페이지에서 **Windows 10 Enterprise x64 업그레이드**를 지정 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-154">On the **General** page, specify **Windows 10 Enterprise x64 Upgrade**, and then select **Next**.</span></span> 
4. <span data-ttu-id="6e8ac-155">**요약** 페이지에서 **다음**을 선택 하 고 **닫기**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-155">On the **Summary** page, select **Next**, and then select **Close**.</span></span> 
5. <span data-ttu-id="6e8ac-156">생성된 된 **Windows 10 Enterprise x64 업데이트** 패키지를 마우스 오른쪽 단추로 클릭 하 고 **콘텐츠 배포**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-156">Right-click the created **Windows 10 Enterprise x64 Update** package, and then select **Distribute Content**.</span></span> 
6. <span data-ttu-id="6e8ac-157">배포 지점을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-157">Choose your distribution point.</span></span>

## <a name="part-3-configure-deployment-settings"></a><span data-ttu-id="6e8ac-158">3 부: 배포 설정 구성</span><span class="sxs-lookup"><span data-stu-id="6e8ac-158">Part 3: Configure deployment settings</span></span>
<span data-ttu-id="6e8ac-p114">이 단계는 Windows 10 업그레이드에 대 한 설정을 포함 하는 업그레이드 작업 시퀀스를 구성 합니다. 그런 다음를 업그레이드 하기 위해 장치를 식별 하 고 해당 장치에 작업 시퀀스를 배포 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p114">In this step, you'll configure an upgrade task sequence that contains the settings for the Windows 10 upgrade. You'll then identify the devices to upgrade, and then deploy the task sequence to those devices.</span></span>

### <a name="create-a-task-sequence"></a><span data-ttu-id="6e8ac-161">작업 시퀀스 만들기</span><span class="sxs-lookup"><span data-stu-id="6e8ac-161">Create a task sequence</span></span>
<span data-ttu-id="6e8ac-162">업그레이드 작업 시퀀스를 만들려면 다음 단계를 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-162">To create an upgrade task sequence, perform the following steps:</span></span>
  
1. <span data-ttu-id="6e8ac-163">Configuration Manager 콘솔 **소프트웨어 라이브러리** 작업 영역에서 **운영 체제**를 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-163">In the Configuration Manager console, in the **Software Library** workspace, expand **Operating Systems**.</span></span> 
2. <span data-ttu-id="6e8ac-164">**작업 시퀀스 (영문)** 노드를 마우스 오른쪽 단추로 클릭 하 고 **작업 시퀀스 만들기**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-164">Right-click the **Task Sequences** node, and then select **Create Task Sequence**.</span></span>
3. <span data-ttu-id="6e8ac-165">**새 작업 시퀀스 만들기** 페이지에서 **업그레이드 패키지에서 운영 체제 업그레이드**를 선택 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-165">On the **Create a new task sequence** page, select **Upgrade an operating system from upgrade package**, and then select **Next**.</span></span>
4. <span data-ttu-id="6e8ac-166">**작업 시퀀스 정보** 페이지에서 **Windows 10 Enterprise x64 업그레이드**를 지정 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-166">On the **Task Sequence Information** page, specify **Windows 10 Enterprise x64 Upgrade**, and then select **Next**.</span></span>
5. <span data-ttu-id="6e8ac-167">**Windows 운영 체제를 업그레이드** 페이지에서 **찾아보기** 를 선택 하 고 **운영 체제를 업그레이드 하는 Windows 10 Enterprise x64 업그레이드 패키지**를 선택, **확인**선택 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-167">On the **Upgrade the Windows operating system** page, select **Browse** and choose the **Windows 10 Enterprise x64 Upgrade operating system upgrade package**, select **OK**, and then select **Next**.</span></span>
6. <span data-ttu-id="6e8ac-168">마법사의 나머지 페이지를 통해 계속 하 고 **닫기**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-168">Continue through the remaining wizard pages, and then select **Close**.</span></span>

### <a name="create-a-device-collection"></a><span data-ttu-id="6e8ac-169">장치 컬렉션 만들기</span><span class="sxs-lookup"><span data-stu-id="6e8ac-169">Create a device collection</span></span>
<span data-ttu-id="6e8ac-170">업그레이드 작업 시퀀스를 만든 후에 업그레이드 하는 장치를 포함 하는 컬렉션을 만들 필요 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-170">After you create the upgrade task sequence, you'll need to create a collection that contains the devices you will upgrade.</span></span>

> [!NOTE]
> <span data-ttu-id="6e8ac-p115">다음 설정을 사용 하 여 단일 장치에 배포를 테스트 하. 준비가 되 면 장치 그룹을 포함 하도록 다른 구성원 규칙을 사용할 수 있습니다. 자세한 정보, [System Center Configuration Manager의 컬렉션을 만드는 방법](https://aka.ms/sccm-create-collections)을 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p115">Use the following settings to test the deployment on a single device. You can use different membership rules to include groups of devices when you are ready. For more info, see [How to create collections in System Center Configuration Manager](https://aka.ms/sccm-create-collections).</span></span>

1. <span data-ttu-id="6e8ac-174">Configuration Manager 콘솔 **자산 및 규정 준수** 작업 영역에서 **장치 컬렉션**마우스 오른쪽 단추로 클릭 하 고 **장치 모음 만들기**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-174">In the Configuration Manager console, in the **Assets and Compliance** workspace, right-click **Device Collections**, and then select **Create Device Collection**.</span></span> 
2. <span data-ttu-id="6e8ac-175">장치 컬렉션 만들기 마법사의 **일반** 페이지에서 다음 설정을 입력 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-175">In the Create Device Collection wizard, on the **General** page, enter the following settings and then select **Next**:</span></span>
    - <span data-ttu-id="6e8ac-176">이름: Windows 10 Enterprise x64 업그레이드</span><span class="sxs-lookup"><span data-stu-id="6e8ac-176">Name: Windows 10 Enterprise x64 Upgrade</span></span>
    - <span data-ttu-id="6e8ac-177">제한 컬렉션: 모든 시스템</span><span class="sxs-lookup"><span data-stu-id="6e8ac-177">Limiting Collection: All Systems</span></span>
3. <span data-ttu-id="6e8ac-178">**구성원 규칙** 페이지에서 **규칙 추가**선택 > **직접 규칙** 직접 구성원 규칙 만들기 마법사를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-178">On the **Membership Rules** page, select **Add Rule** > **Direct rule** to launch the Create Direct Membership Rule Wizard.</span></span>
4. <span data-ttu-id="6e8ac-179">직접 구성원 규칙 만들기 마법사의 **시작** 페이지에서 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-179">On the **Welcome** page of the Create Direct Membership Rule Wizard, select **Next**.</span></span>
5. <span data-ttu-id="6e8ac-180">**자원에 대 한 검색** 페이지에서 개체 틀 텍스트 **값** 을 업그레이드 하는 장치의 이름을 바꿉니다, 다음 설정을 입력 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-180">On the **Search for Resources** page, enter the following settings, replacing the placeholder **Value** text with the name of the device you are upgrading:</span></span> 
    - <span data-ttu-id="6e8ac-181">리소스 클래스: 시스템 리소스</span><span class="sxs-lookup"><span data-stu-id="6e8ac-181">Resource Class: System Resource</span></span>
    - <span data-ttu-id="6e8ac-182">특성 이름: 이름</span><span class="sxs-lookup"><span data-stu-id="6e8ac-182">Attribute Name: Name</span></span>
    - <span data-ttu-id="6e8ac-183">값: *PC0003*</span><span class="sxs-lookup"><span data-stu-id="6e8ac-183">Value: *PC0003*</span></span>
6. <span data-ttu-id="6e8ac-184">**자원을 선택** 페이지에서 장치를 선택 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-184">On the **Select Resources** page, select your device, and select **Next**.</span></span>
7. <span data-ttu-id="6e8ac-185">직접 구성원 규칙 만들기 마법사와 장치 컬렉션 만들기 마법사를 완료 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-185">Complete the Create Direct Membership Rule wizard and the Create Device Collection Wizard.</span></span>  
8. <span data-ttu-id="6e8ac-p116">Windows 10 Enterprise x64 업그레이드 컬렉션을 검토 합니다. 컴퓨터를 나타날 때까지 계속 하지 마세요 컬렉션에 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p116">Review the Windows 10 Enterprise x64 Upgrade collection. Do not continue until you see the machines you added in the collection.</span></span>

### <a name="create-an-operating-system-deployment"></a><span data-ttu-id="6e8ac-188">운영 체제 배포 만들기</span><span class="sxs-lookup"><span data-stu-id="6e8ac-188">Create an operating system deployment</span></span>
<span data-ttu-id="6e8ac-189">작업 시퀀스에 대 한 배포를 만들려면 다음이 단계를 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-189">Follow these steps to create a deployment for the task sequence.</span></span>

1. <span data-ttu-id="6e8ac-190">Configuration Manager 콘솔 **소프트웨어 라이브러리** 작업 영역에서 이전 단계에서 만든 작업 시퀀스를 마우스 오른쪽 단추로 클릭 하 고 **배포**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-190">In the Configuration Manager console, in the **Software Library** workspace, right-click the task sequence you created in a previous step, and then select **Deploy**.</span></span>
2. <span data-ttu-id="6e8ac-191">**일반** 페이지에서 **Windows 10 Enterprise x64 업그레이드** 컬렉션을 선택 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-191">On the **General** page, select the **Windows 10 Enterprise x64 Upgrade** collection, and then select **Next**.</span></span>
3. <span data-ttu-id="6e8ac-192">**콘텐츠** 페이지에서 **다음**을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-192">On the **Content** page, select **Next**.</span></span>
4. <span data-ttu-id="6e8ac-193">**배포 설정** 페이지에서 다음 설정을 선택 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-193">On the **Deployment Settings** page, select the following settings, and then select **Next**:</span></span>

    > [!NOTE]
    > <span data-ttu-id="6e8ac-p117">이 테스트 배포용 **대화 가능**, 배포를 시작 하려면 사용자 작업이 필요 하는 용도 설정 합니다. 프로덕션 환경에서 배포를 실행 하는 경우 일정 등의 추가 옵션을 구성 하는 단계는 필요한 용도 사용 하 여 배포를 자동화 하는 것이 좋습니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p117">For this test deployment, you'll set the purpose to **Available**, which requires user intervention to start the deployment. In a production environment, you may wish to automate the deployment using the Required purpose, which involves configuring additional options such as scheduling when the deployment is run.</span></span> 

    - <span data-ttu-id="6e8ac-196">작업: 설치</span><span class="sxs-lookup"><span data-stu-id="6e8ac-196">Action: Install</span></span>
    - <span data-ttu-id="6e8ac-197">용도: 사용 가능한</span><span class="sxs-lookup"><span data-stu-id="6e8ac-197">Purpose: Available</span></span>

5. <span data-ttu-id="6e8ac-198">**일정** 페이지에서 기본 설정을 그대로 적용 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-198">On the **Scheduling** page, accept the default settings, and then select **Next**.</span></span>
6. <span data-ttu-id="6e8ac-199">**사용자 환경** 페이지에서 기본 설정을 그대로 적용 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-199">On the **User Experience** page, accept the default settings, and then select **Next**.</span></span>
7. <span data-ttu-id="6e8ac-200">**경고** 페이지에서 기본 설정을 그대로 적용 하 고 \*\*\*\* 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-200">On the **Alerts** page, accept the default settings, and then select **Next**.</span></span>
8. <span data-ttu-id="6e8ac-201">**요약** 페이지에서 **다음**을 선택 하 고 **닫기**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-201">On the **Summary** page, select **Next**, and then select **Close**.</span></span>

## <a name="part-5-start-the-windows-10-upgrade-task-sequence"></a><span data-ttu-id="6e8ac-202">5 단계: 시작 Windows 10 업그레이드 작업 시퀀스</span><span class="sxs-lookup"><span data-stu-id="6e8ac-202">Part 5: Start the Windows 10 upgrade task sequence</span></span>
<span data-ttu-id="6e8ac-203">업그레이드 하는 장치에 Windows 10 업그레이드 작업 시퀀스를 시작 하려면 다음이 단계를 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-203">Follow these steps to start the Windows 10 Upgrade task sequence on the device that you are upgrading.</span></span>
 
1. <span data-ttu-id="6e8ac-204">Windows 컴퓨터에 로그온 하 고 **소프트웨어 센터**를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-204">Log on to the Windows computer and start **Software Center**.</span></span>
2. <span data-ttu-id="6e8ac-205">이전 단계에서 만든 작업 시퀀스를 선택 하 고 **설치**를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-205">Select the task sequence that you created in a previous step, and then select **Install**.</span></span>
3. <span data-ttu-id="6e8ac-206">Windows 설치 프로그램 (Setup.exe)를 유지 하는 모든 데이터, 설정, 앱, 자동화 된 업그레이드를 수행 하는데 필요한 명령줄 매개 변수를 호출 하 여 전체 업그레이드 프로세스 작업 시퀀스 시작 되 면 자동으로 시작 하 고 드라이버 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-206">When the task sequence begins, it automatically initiates the in-place upgrade process by invoking the Windows setup program (Setup.exe) with the necessary command-line parameters to perform an automated upgrade, which preserves all data, settings, apps, and drivers.</span></span>
4. <span data-ttu-id="6e8ac-207">작업 시퀀스 성공적으로 완료 되 면 Windows 10 컴퓨터를 완벽 하 게 업그레이드 됩니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-207">After the task sequence completes successfully, the computer will be fully upgraded to Windows 10.</span></span>

<span data-ttu-id="6e8ac-p118">엔터프라이즈 환경에서 Windows 10을 사용 하는 경우 문제를 발생 하면 [가장 일반적인 문제에 대 한 주요 Microsoft 기술 지원 서비스 솔루션](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions)을 참조할 수 있습니다. 이러한 리소스 KB 문서, 업데이트 및 라이브러리 문서에 포함 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p118">If you experience issues when using Windows 10 in an enterprise environment, you can consult [top Microsoft Support solutions for the most common issues](https://docs.microsoft.com/windows/client-management/windows-10-support-solutions). These resources include KB articles, updates, and library articles.</span></span>

<span data-ttu-id="6e8ac-p119">조직 전체에서 업데이트를 배포 하는 동안 운영 체제 업데이트 준수, 배포 진행 상황 업데이트 및 Windows 10 장치에 대 한 문제를 해결 하는 오류에 대 한 통찰력을 제공 하려면 Windows 분석의 업데이트 규정 준수 기능을 사용 합니다. 이 새로운 서비스 설치 진행률, Windows Update 구성 및 기타 정보를 포함 하 여 진단 데이터를 사용 하 여 아니요에 이러한 정보를 제공 하려면 추가 비용 및 추가 인프라 요구 사항입니다. 비즈니스 또는 기타 관리 도구에 대 한 Windows Update를 통해 사용, 여부 보장할 수 있습니다 장치로 적절 하 게 업데이트 하는 합니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-p119">During the rollout of updates across your organization, use the Update Compliance capability of Windows Analytics to provide a holistic view of OS update compliance, update deployment progress, and failure troubleshooting for Windows 10 devices. This new service uses diagnostic data including installation progress, Windows Update configuration and other information to provide such insights, at no extra cost and without additional infrastructure requirements. Whether it's used with Windows Update for Business or other management tools, you can be assured that your devices are properly updated.</span></span>

<span data-ttu-id="6e8ac-213">[모니터 Windows 업데이트 및 준수가 업데이트 된 Windows Defender 바이러스 백신 소프트웨어를](https://docs.microsoft.com/windows/deployment/update/update-compliance-monitor) 자세한 정보, 시작 및 업데이트 준수를 사용 하 여를 참조 하십시오.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-213">See [Monitor Windows Updates and Windows Defender Antivirus with Update Compliance](https://docs.microsoft.com/windows/deployment/update/update-compliance-monitor) to learn more, get started, and use Update Compliance.</span></span>

<span data-ttu-id="6e8ac-214">중간 검사점으로 이 단계에 해당하는 [종료 조건](windows10-exit-criteria.md#crit-windows10-step2)을 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="6e8ac-214">As an interim checkpoint, you can see the [exit criteria](windows10-exit-criteria.md#crit-windows10-step2) corresponding to this step.</span></span>

## <a name="next-step"></a><span data-ttu-id="6e8ac-215">다음 단계</span><span class="sxs-lookup"><span data-stu-id="6e8ac-215">Next step</span></span>

|||
|:-------|:-----|
|![](./media/stepnumbers/Step3.png)| [<span data-ttu-id="6e8ac-216">Windows 작업을 자동화할을 사용 하 여 새 장치에 대 한 Windows 10 엔터프라이즈 배포</span><span class="sxs-lookup"><span data-stu-id="6e8ac-216">Deploy Windows 10 Enterprise for new devices with Windows Autopilot</span></span>](windows10-deploy-autopilot.md) |
