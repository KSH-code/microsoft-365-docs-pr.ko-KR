...
이 문서에서는 개인 팀에 대해 Teams 기능(예: 채팅, 모임, 파일)에 대한 액세스를 Microsoft 365 그룹의 구성원과 소유자로 차단하는 개인 팀을 Microsoft Teams에서 구성하기 위한 권장 사항과 단계를 제공합니다. 

Microsoft 365 그룹을 기반으로 하는 개인 액세스를 넘어서, 이 문서에서는 기본 개인 SharePoint 팀 사이트를 구성하는 방법을 설명합니다. SharePoint 팀 사이트는 높은 규제 대상 데이터를 저장하는 데 필요한 추가 보안을 위해 팀 채널의 **파일** 섹션에서 액세스할 수 있습니다. 이 SharePoint 팀 사이트에서 파일, 페이지, 공유 일정, 작업, 전자 필기장 및 목록에 저장하고 공동 작업할 수 있습니다.
...
- 소유자와 구성원 사용자 계정을 포함하는 해당 Microsoft 365 그룹을 보유하고 있는 개인 팀입니다.
- 팀을 위한 기본 SharePoint 사이트에 대 한 추가 보안:
  - 사이트의 멤버가 다른 사람에게 액세스 권한을 부여하지 못하도록 합니다.
  - 사이트의 멤버 이외의 사용자가 다른 사람에게 액세스 권한을 요청하지 못하도록 합니다.
- 보존 정책을 정의하는 기본 방법으로 사이트에 있는 새 파일에 자동으로 적용되는 기본 SharePoint 사이트의 보존 레이블입니다.
- 보존 레이블을 사용하고 사용자가 조직 외부에서 파일을 공유하거나 보내지 못하도록 차단하는 DLP(데이터 손실 방지) 정책입니다.
- 민감도 레이블 또는 팀의 Microsoft 365 그룹에 대해 암호화가 활성화되어 있고 공동 작성자 권한이 있는 높은 규제 대상 레이블의 하위 레이블입니다. Word, Excel 및 PowerPoint의 **민감도** 메뉴 모음 옵션에서 팀의 **파일** 섹션에 저장된 파일에 레이블이나 하위 레이블을 적용할 수 있습니다.
...
- 팀의 Microsoft 365 그룹(팀 그룹)에는 그룹 소유자와 그룹 구성원이 있습니다.
