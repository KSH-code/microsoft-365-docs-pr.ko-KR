...
단일 마이그레이션을 사용하여 원본 전자 메일 시스템에서 Microsoft 365로 사용자 사서함의 콘텐츠를 한 동시에 마이그레이션할 수 있습니다. 이 문서에서는 Exchange Online PowerShell을 사용하는 전자 메일 단독형 마이그레이션 작업을 살펴봅니다.

[Microsoft 365로의](https://go.microsoft.com/fwlink/p/?LinkId=536688)간략한 전자 메일 마이그레이션에 대해 알아야 할 사항 항목인 항목을 검토하면 마이그레이션 프로세스에 대한 개요를 확인할 수 있습니다. 이 문서 내용을 충분히 이해할 수 있으면 다음을 사용하여 다른 전자 메일 시스템으로 사서함을 마이그레이션해 보세요.

> [!NOTE]
> Exchange 관리 센터를 사용하여 단독형 마이그레이션을 수행할 수도 있습니다. Microsoft 365로의 전자 메일에 대한 [컷오버 마이그레이션 수행을 참조하세요.](https://go.microsoft.com/fwlink/p/?LinkId=536689)
...
- **Microsoft 365 조직의 허용 도메인으로 On-premises Exchange 조직을 추가합니다.** 마이그레이션 서비스는 새 Microsoft 365 사서함에 대한 Microsoft Online Services 사용자 ID 및 전자 메일 주소를 만드는 데 사용할 수 있는 SMTP 주소를 제공합니다. Exchange 도메인이 Microsoft 365 조직의 허용 도메인 또는 기본 도메인이 아닌 경우 마이그레이션이 실패합니다. 자세한 내용은 [도메인 확인을 참조하세요.](https://go.microsoft.com/fwlink/p/?LinkId=534110)
...
- **온-프레미스 사용자 계정에 Exchange 조직의 사서함에 액세스하는 데 필요한 사용 권한 할당** Microsoft 365로 마이그레이션할온-프레미스 사서함에 액세스하는 데 사용하는온-프레미스 사용자 계정(마이그레이션 관리자라고도함)에 필요한 권한이 있어야 합니다. 이 사용자 계정은 온-프레미스 조직에 대한 마이그레이션 끝점을 만드는 데 사용됩니다.
...
- **보안 그룹 및 대리인** 전자 메일 마이그레이션 서비스는 Microsoft 365에서 마이그레이션된 그룹을 보안 그룹으로 프로비전할 수 없습니다. Microsoft 365 테넌트에 보안 그룹을 사용하려면 먼저 Microsoft 365 테넌트에서 빈 메일 사용이 가능한 보안 그룹을 프로비전해야만 마이그레이션이 가능합니다. 또한 이 마이그레이션 방법을 사용하는 경우에는 사서함, 메일 사용자, 메일 연락처 및 메일 사용 가능 그룹만 이동됩니다. Microsoft 365로 마이그레이션되지 않은 사용자와 같은 다른 Active Directory 개체가 마이그레이션되는 개체에 대한 관리자 또는 대리인으로 할당된 경우 마이그레이션하기 전에 개체에서 제거해야 합니다.

### <a name="step-2-create-a-migration-endpoint"></a>2단계: 마이그레이션 끝점 만들기
<a name="BK_Step2"> </a>

전자 메일을 마이그레이션하려면 Microsoft 365가 원본 전자 메일 시스템에 연결하고 통신해야 합니다. 이를 위해 Microsoft 365는 마이그레이션 끝점을 사용 합니다. 단독형 마이그레이션을 위한 "외부에서 Outlook 사용" 마이그레이션 끝점을 만들려면 먼저 [Exchange Online에 연결](https://go.microsoft.com/fwlink/p/?LinkId=534121)합니다.
...
### <a name="step-5-route-your-email-to-microsoft-365"></a>5단계: Microsoft 365로 전자 메일 라우팅
<a name="BK_Step5"> </a>

전자 메일 시스템은 MX 레코드라는 DNS 레코드를 사용하여 전자 메일을 배달할 위치를 확인합니다. 전자 메일 마이그레이션 프로세스 중에는 MX 레코드가 원본 전자 메일 시스템을 가리켰습니다. 이제 Microsoft 365로의 전자 메일 마이그레이션이 완료되었습니다. 이제 Microsoft 365에서 MX 레코드를 설정해야 합니다. 이렇게 하면 전자 메일이 Microsoft 365 사서함으로 배달됩니다. MX 레코드를 이동하여 준비가 되었을 때 이전 전자 메일 시스템을 해제할 수도 있습니다.
...
MX 레코드를 변경하고 모든 전자 메일이 Microsoft 365 사서함으로 라우팅되고 있는지 확인한 후 해당 메일이 Microsoft 365로 이동될 것임 사용자에게 알릴 수 있습니다. 그런 후에는 단독형 마이그레이션 일괄 처리를 삭제해도 됩니다. 마이그레이션 일괄 처리를 삭제하기 전에 다음을 확인합니다.

- 모든 사용자가 Microsoft 365 사서함을 사용하고 있습니다. 일괄 처리가 삭제되고 나면 해당 Microsoft 365 사서함에 Exchange Server 사서함으로 전송되는 메일이 복사되지 않습니다.

- 메일을 직접 보내기 시작한 후 Microsoft 365 사서함이 한 번 이상 동기화됩니다. 이 작업을 수행하기 위해 마이그레이션 일괄 처리에 대한 마지막 동기화 시간 상자의 값이 메일이 Microsoft 365 사서함으로 직접 라우팅하기 시작한 시간보다 최신 상태인지 확인해야 합니다.
 ...
 **라이선스를 할당하여 마이그레이션된 계정에 대해 Microsoft 365 사용자 계정을 활성화합니다.** 라이선스를 할당하지 않은 경우 30일 유예 기간이 끝나면 사서함을 사용할 수 없습니다. Microsoft 365 관리 센터에서 라이선스를 할당하려면 라이선스 할당 또는 [할당을 참조하세요.](https://docs.microsoft.com/microsoft-365/admin/manage/assign-licenses-to-users)
...
- **사용자가 자신의 사서함으로 쉽게 이동할 수 있도록 자동 검색 DNS 레코드를 만듭니다.** 모든온-프레미스 사서함을 Microsoft 365로 마이그레이션한 후 사용자가 Outlook 및 모바일 클라이언트를 사용하여 새 Microsoft 365 사서함에 쉽게 연결할 수 있도록 Microsoft 365 조직의 자동 검색 DNS 레코드를 구성할 수 있습니다. 이 새 자동iscover DNS 레코드는 Microsoft 365 조직에 사용 중이던 동일한 네임스페이스를 사용해야 합니다. 예를 들어 클라우드 기반 네임스페이스가 cloud.contoso.com인 경우 만들어야 할 자동 검색 DNS 레코드는 autodiscover.cloud.contoso.com입니다.
    ...
    클라이언트를 Exchange Server 경우 Outlook 클라이언트가 올바른 사서함에 연결할 수 있도록 자동 검색 DNS CNAME 레코드가 마이그레이션 후 내부 및 외부 DNS 둘 다에서 Microsoft 365를 설정해야 합니다.
...
- **온-프레미스 Exchange 서버를 해제합니다.** 모든 전자 메일이 Microsoft 365 사서함으로 직접 라우팅되고 있는 것을 확인한 후 더 이상 전자 메일 조직을 유지 관리할 필요가 없는 경우 또는 SSO(Single Sign-On) 솔루션 구현을 계획하지 않은 경우 서버에서 Exchange를 제거하고, Exchange 조직을 제거할 수 있습니다.
