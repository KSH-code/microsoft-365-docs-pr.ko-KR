...
- 클라우드 사용에 사용되는 모든 페이지/기능/사용자 지정은 다른 영역 또는 지역의 사용자가 보다 일관된 환경을 제공하도록 사용자의 분산된 위치뿐만 아니라 더 높은 대기 시간으로 최적화해야 합니다. 클라우드는 분산된 사용자 기반 및 최신 SharePoint 최적화를 위해 콘텐츠 배달 네트워크(CDN)와 같은 최적화를 제공합니다SharePoint 마지막으로 알려진 LKG(Good)는 OOTB(기본 제공) 웹 파트에서 활용됩니다.

### <a name="what-to-do"></a>할 일:
 - SharePoint Online의 모든 사이트 페이지에는 [](./page-diagnostics-for-spo.md)분석 및 지침을 제공하는 데 도움이 될 Chromium 확장인 페이지 진단 도구를 사용합니다. 사이트 소유자, 편집자, 관리자 및 개발자가 분석 및 최적화의 시작점으로 사용할 수 있습니다.
...
사이트 실행을 위한 기존의 Big bang 접근 방식에서는 사용자 지정, 외부 원본, 서비스 또는 프로세스가 올바른 규모로 테스트된 것이 확인되지 않습니다. 이는 시작되는 데 몇 개월이 걸릴 수 있는 것은 아니며 조직 규모에 따라 적어도 며칠 동안 권장됩니다. 따라서 웨이브 롤아웃 계획에 따라 다음 단계를 진행하기 전에 문제를 일시 중지하고 해결할 수 있는 옵션을 제공하므로 문제가 발생할 수 있는 사용자 수가 줄어든다. SharePoint 서비스로 사용하면 사용량 및 예측된 사용량에 따라 용량을 확장할 수 있으며, 시작을 알려드릴 필요가 없는 경우에는 지침에 따라 성공을 보장해야 합니다.
  
다음 이미지와 같이 초대된 사용자 수가 실제로 사이트를 사용하는 사용자보다 훨씬 많은 경우가 종종 있습니다. 이 이미지는 릴리스를 롤아웃하는 방법에 대한 전략을 보여줍니다. 이 방법은 대부분의 사용자에게 표시되기 SharePoint 사이트를 개선하는 방법을 식별하는 데 도움이 됩니다.
