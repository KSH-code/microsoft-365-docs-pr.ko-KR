...
Windows 위치 서비스는 컴퓨터에서 consented 해야 합니다. **지도** 앱을 실행 하 고 자신을 찾아 테스트할 수 있습니다. 설정이 포함 된 단일 컴퓨터에서 사용 하도록 설정할 수 있습니다. **| 개인 정보 |** _앱에서 사용자의 위치에 액세스 하도록 허용_ 설정이 사용 하도록 설정 되어 있어야 합니다. Windows 위치 서비스 동의 _를 설정 하는 경우에_ 는 MDM 또는 그룹 정책을 사용 하 여 pc에 배포할 수 있습니다.
...
클라이언트 컴퓨터의 모든 테스트 단위에는 입력 한 사무실 위치 정보와 상호 관련 된 LAN 서브넷 정보가 포함 됩니다. 측정 및 사무실 위치는 이러한 필수 구성 요소가 충족 된 후 24 시간 전에 표시 되어야 합니다.
