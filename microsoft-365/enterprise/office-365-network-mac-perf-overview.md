...
ms.locfileid: "46794313"
---
# <a name="network-connectivity-in-the-microsoft-365-admin-center-preview"></a>Microsoft 365 관리 센터의 네트워크 연결 (미리 보기)

Microsoft 365 관리 센터는 이제 Microsoft 365 테 넌 트에서 수집한 네트워크 연결 메트릭을 includesaggregated 테 넌 트의 관리 사용자만 볼 수 있도록 합니다. **네트워크 평가** 및 **네트워크 통찰력** 은 Microsoft 365 관리 센터의 상태 |에 표시 됩니다. ** 연결**합니다.

![네트워크 성능 페이지](../media/m365-mac-perf/m365-mac-perf-page-nav.png)

먼저 네트워크 연결 페이지로 이동 하면 전역 네트워크 성능 맵과 전체 테 넌 트로 범위가 지정 된 네트워크 평가가 포함 된 개요 창 및 현재 문제 목록이 표시 됩니다. 이 개요에서는 드릴 다운을 통해 위치로 특정 네트워크 성능 메트릭 및 문제를 볼 수 있습니다. 자세한 내용은 [Microsoft 365 관리 센터의 네트워크 성능 개요](#network-performance-overview-in-the-microsoft-365-admin-center)를 참조 하세요.

## <a name="pre-requisites-for-network-connectivity-assessments-to-appear"></a>네트워크 연결 평가를 표시 하기 위한 필수 구성 요소

Office 위치에서 네트워크 평가를 받기 위한 옵션에는 다음 세 가지가 있습니다.
...
Windows 위치 서비스는 컴퓨터에서 consented 해야 합니다. **지도** 앱을 실행 하 고 자신을 찾아 테스트할 수 있습니다. **Settings**  ->  "앱에서 위치 액세스 허용" 설정을 사용 하도록 설정 하 여**개인 정보 보호**  ->  **위치가** 있는 단일 컴퓨터에서이 기능을 사용 하도록 설정할 수 있습니다. Windows 위치 서비스 동의 _를 설정 하는 경우에_는 MDM 또는 그룹 정책을 사용 하 여 pc에 배포할 수 있습니다.

이 방법을 사용 하는 경우에는 도시 해상도에서 자동으로 식별 되므로이 방법으로는 관리 센터에서 위치를 추가할 필요가 없습니다. Windows Location Services를 사용 하 여 한 도시에 여러 office 위치를 표시할 수는 없습니다.
...
이 옵션의 경우 Windows 위치 서비스와 Wi-fi가 모두 필요 하지 않습니다. 해당 위치에서 puter에 Windows 버전 20.161.0811.0001 이상이 설치 되어 있어야 합니다.
...
이 옵션의 경우 각 위치에서 사람을 식별 해야 합니다. 사용자에 게 관리 권한이 있는 Windows 컴퓨터에서 [Microsoft 365 네트워크 연결 테스트](https://connectivity.office.com) 에 대 한 브라우저를 요청 합니다. 웹 사이트에서 결과를 보려는 동일한 테 넌 트에 Office 365 계정에 로그인 해야 합니다. 그런 다음 테스트 실행을 클릭 합니다. 테스트 중에는 다운로드 된 연결 테스트 EXE가 있습니다. 또한이를 열고 실행 해야 합니다. 테스트가 완료 되 면 테스트 결과가 Microsoft로 업로드 됩니다.

LAN 서브넷 정보를 사용 하 여 추가 된 경우 테스트 보고서가 위치에 연결 되 고 그렇지 않으면 도시 위치에만 표시 됩니다.

테스트 결과가 완료 되 면 2-3 분 후에 측정 샘플과 office 위치를 표시 하기 시작 합니다.
...
LAN 서브넷 office id의 경우 각 locaiton를 미리 추가 해야 합니다. **위치** 탭에서 개별 사무실 위치를 추가 하는 대신 CSV 파일에서 가져올 수 있습니다. 통화 품질 대시보드 또는 Active Directory 사이트 및 서비스와 같이 저장 한 다른 위치에서이 데이터를 가져올 수 있습니다.

CSV 파일에서 검색 된 도시 위치는 **구/군/시**로 레이블이 지정 되 고 수동으로 추가 된 사무실 위치에는 **location**이라는 레이블이 지정 됩니다.
