...
이 문서에서는 Microsoft 365 네트워킹의 목표와 Microsoft 365 네트워킹에 일반 인터넷 트래픽과 다른 최적화 방식이 필요한 이유에 대해 학습합니다.
...
- Microsoft 365 트래픽이 Proxies 및 패킷 검사 장치를 무시하도록 허용
...
기존 네트워크 아키텍처에서 일반 인터넷 트래픽의 대기 시간은 네트워크 경계 보안을 유지 관리하기 위해 필요한 장단점이고, 인터넷 트래픽에 대한 성능 최적화에는 일반적으로 네트워크 전송 지점에서 장비를 업그레이드 또는 수리하는 과정이 있습니다. 그러나 이 방법은 Microsoft 365와 같은 SaaS 서비스의 최적의 네트워크 성능을 위한 요구 사항을 해결하지는 않습니다.

## <a name="identifying-microsoft-365-network-traffic"></a>Microsoft 365 네트워크 트래픽 식별

Microsoft는 Microsoft 365 네트워크 트래픽을 보다 쉽게 식별하고 네트워크 ID를 보다 간단하게 관리할 수 있도록 합니다.
...
- [Microsoft 365](./microsoft-365-networking-partner-program.md) 네트워크 연결 원칙을 따르고 간단한 구성을 제공하는 장치 또는 서비스를 제공하는 Microsoft 파트너와의 Office 365 네트워크 파트너 프로그램입니다.
...
Microsoft 365는 Microsoft 365 기능 및 워크로드를 위해 특별히 설계된 기본 제공 보안 및 거버넌스 기능을 사용하여 콘텐츠 보안 및 데이터 사용 규정 준수에 대한 조직의 요구를 충족하는 데 도움을 줍니다. Microsoft 365 보안 및 규정 준수에 대한 자세한 내용은 [Office 365 보안 로드맵을 참조하세요.](/office365/securitycompliance/security-roadmap) Microsoft 365 트래픽에 대해 고급 수준의 처리를 수행하는 고급 네트워크 솔루션에 대한 Microsoft의 권장 사항 및 지원 위치에 대한 자세한 내용은 [Office 365](https://support.microsoft.com/help/2690045)트래픽에 타사 네트워크 장치 또는 솔루션 사용을 참조하세요.

## <a name="why-is-microsoft-365-networking-different"></a>Microsoft 365 네트워킹이 다른 이유는 무엇입니까?

Microsoft 365는 끝점 보안 및 암호화된 네트워크 연결을 사용하여 최적의 성능을 제공하도록 설계되어 경계 보안 적용의 필요성을 줄입니다. Microsoft 365 데이터 센터는 전 세계에 있으며, 이 서비스는 클라이언트를 사용 가능한 최상의 서비스 끝점에 연결하는 다양한 방법을 사용할 수 있도록 디자인되었습니다. 사용자 데이터 및 처리는 여러 Microsoft 데이터 센터 간에 분산되어 있으며 클라이언트 컴퓨터를 연결할 수 있는 단일 네트워크 끝점이 없습니다. 실제로 Microsoft 365 테넌트의 데이터 및 서비스는 Microsoft 전역 네트워크에 의해 동적으로 최적화되어 최종 사용자가 액세스하는 지리적 위치에 맞게 조정됩니다.

Microsoft 365 트래픽에 패킷 검사 및 중앙 집중식 전송이 적용될 때 일반적인 성능 문제가 생성됩니다.

- 대기 시간이 길면 비디오 및 오디오 스트림의 성능이 매우 저하되고 데이터 검색, 검색, 실시간 공동 작업, 약속이 있는 일정 정보, 제품 내 콘텐츠 및 기타 서비스에 대한 응답 속도가 느려질 수 있습니다.
- 중앙 위치에서 연결을 유출하면 Microsoft 365 전역 네트워크의 동적 라우팅 기능이 끊어지며 대기 시간 및 왕복 시간이 추가됩니다.
- SSL 보안 Microsoft 365 네트워크 트래픽의 암호를 해독하고 다시 암호화하면 프로토콜 오류가 발생할 수 있으며 보안 위험이 있습니다.

클라이언트 트래픽이 지리적 위치에 최대한 가깝게 유입될 수 있도록 하여 Microsoft 365 진입점의 네트워크 경로를 단축하면 Microsoft 365의 연결 성능과 최종 사용자 환경을 개선할 수 있습니다. 또한 향후 네트워크 아키텍처가 Microsoft 365 성능 및 안정성에 미치는 영향을 줄일 수 있습니다. 최적의 연결 모델은 회사 네트워크에 있는지, 집, 커피숍, 공항과 같은 원격 위치에 있는지에 관계없이 항상 사용자 위치에 네트워크 발신을 제공하는 것입니다. 일반 인터넷 트래픽 및 WAN 기반 회사 네트워크 트래픽은 별도로 라우팅될 것이고 로컬 직접 발신 모델을 사용하지 않습니다. 이 로컬 직접 송신 모델은 아래 다이어그램에 표시되어 있습니다.
...
- 경로 길이를 최적화하여 최적의 Microsoft 365 성능을 제공합니다. 최종 사용자 연결은 Microsoft 전역 네트워크의 분산 서비스 _프런트_ 도어 인프라를 통해 가장 가까운 Microsoft 365 진입점으로 동적으로 라우팅되고 트래픽은 Microsoft의 대기 시간이 매우 짧은 고가용성 파이버를 통해 내부적으로 데이터 및 서비스 끝점으로 라우팅됩니다.
- Microsoft 365 트래픽에 대한 로컬 발신을 허용하고, Proxies 및 트래픽 검사 장치를 우회하여 회사 네트워크 인프라의 부하를 줄입니다.
- 클라이언트 끝점 보안 및 클라우드 보안 기능을 활용하고 중복 네트워크 보안 기술을 적용하지 않도록 하여 양 끝에서 연결을 보호합니다.
...
Microsoft 365 네트워크 연결 원칙을 이해하고 적용하는 데 대한 자세한 내용은 [Microsoft 365 네트워크 연결 원칙을 참조하세요.](microsoft-365-network-connectivity-principles.md)

## <a name="conclusion"></a>결론

Microsoft 365 네트워크 성능을 최적화하는 것이 불필요한 단점을 제거하는 데 있습니다. Microsoft 365 연결을 신뢰할 수 있는 트래픽으로 처리하면 패킷 검사 및 프록시 대역폭 경쟁으로 인해 대기 시간이 도입되지 않도록 할 수 있습니다. 클라이언트 컴퓨터와 Office 365 끝점 간의 로컬 연결을 허용하면 트래픽을 Microsoft 전역 네트워크를 통해 동적으로 라우팅할 수 있습니다.
