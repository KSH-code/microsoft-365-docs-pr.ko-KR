...
| PDF, JPG, PNG 형식 교육 총 50MB/500 pp | 부정적인 예제를 포함하여 5-10 PDF, Office 또는 전자 메일 파일을 교육합니다. |

다음 표에서는 SharePoint Syntex의 가용성 및 라이선싱에 대해 설명합니다.

| 양식 처리 | 문서 이해 |
|:-------|:-------|
| 양식 처리는 Power Platform에 의존합니다. <br>Power Platform 및 AI Builder의 전역 가용성에 대한 자세한 내용은 [Power Platform 가용성을 참조하세요.](https://dynamics.microsoft.com/geographic-availability/) | 모든 지역에서 사용할 수 있습니다. |
| AI 작성기 크레딧을 사용 합니다.<br>크레딧은 1M의 일괄로 구매할 수 있습니다.<br>300개 이상의 SharePoint Syntex 라이선스를 구입하면 1M 크레딧이 포함됩니다.<br>1M 크레딧은 2,000개 파일 페이지를 처리하도록 허용합니다. | 모델은 모든 라틴어 알파벳 언어로 작동됩니다. 영어 외에 독일어, 스웨덴어, 프랑스어, 스페인어, 이탈리아어 및 포르투갈어 |
...
또 다른 예로, 회사가 다른 회사 또는 개인과 계약한 계약을 식별하는 프로세스를 설정할 수 있습니다. 모델을 설정하여 클라이언트 이름, 수수료, 날짜 또는 기타 중요한 정보와 같은 주요 정보를 해당 계약에서 추출하고 라이브러리에 해당 정보를 필드로 빠르게 볼 수 있습니다. 또한 문서 라이브러리에 보존 레이블을 적용하여 비즈니스 규정을 적절하게 준수하기 위해 특정 기간 전에 계약을 삭제할 수 없습니다.
...
관련자와 협의하여 SharePoint Syntex 사용 전략에 맞춰야 합니다. 이 위치에 도움이 되는 다음 리소스를 조사하고 제공합니다.
...
1. 종료 상태 계획
    - 문서 이해 모델은 끝이 아닌 수단입니다.
    - 다음을 사용하여 추출된 메타데이터의 값을 사용할 계획입니다.
      - 검색
      - 필터링 및 보기 서식
      - 규정 준수
      - 자동화
...
1. 단계로 롤아웃합니다.  
1. 피드백을 수집하고 이르게 합니다.
1. 사용량이 증가함에 따라 필요한 경우 모든 AI 작성기 크레딧에 대한 계획이 확장됩니다.
