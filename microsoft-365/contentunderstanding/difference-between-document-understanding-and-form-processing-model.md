...
| 기능 | 양식 처리 | 문서 이해 |
| ------- | ------- | ------- |
| 모델 유형 - 각 모델을 사용해야 하는 경우 | 반구조화된 파일 형식에 사용(예: 레이아웃에는 차이가 있지만 비슷한 정보가 추출되는 Office 문서). | 구조화되지 않은 파일 형식에 사용(예: 레이아웃 및 서식이 비슷한 송장이나 구매 주문과 같은 양식 콘텐츠의 PDF). |
| 모델 만들기 | SharePoint 문서 라이브러리에서 원활하게 액세스할 수 있도록 AI 작성기로 만든 모델.| SharePoint 콘텐츠 센터에 기본 제공되는 기본 인터페이스로 만든 모델.|
| 분류 유형| 컴퓨터 티칭을 사용하여 추출할 데이터에 대한 단서를 시스템에 제공하는 설정 가능한 분류자입니다.| 옵션인 추출기에서 컴퓨터 티칭을 사용하여 추출할 데이터에 대한 문서 위치를 할당하는 학습 가능한 분류자입니다.|
| 위치 | Power Platform을 사용하여 CDS에서 검색하지 않는 한 단일 문서 라이브러리로 제한됩니다.| 여러 라이브러리에 적용할 수 있습니다.|
...
| 관리되는 메타데이터와 통합 | 아니요 | 예, 교육 모델 이전의 문서 라이브러리 열에서 설정합니다.|
| Microsoft Information Protection을 사용하도록 설정했을 때 규정 준수 기능 통합 | 보존 레이블 설정<br>민감도 레이블 설정 기능이 곧 제공됩니다. | 보존 레이블 설정<br>민감도 레이블 설정 기능이 곧 제공됩니다. |
...
| 트랜잭션 비용 | AI 작성기 크레딧을 사용합니다.<br>크레딧은 백만 크레딧 단위로 구입할 수 있습니다.<br>300개 이상의 SharePoint Syntex 라이선스를 구입하면 1백만 크레딧이 포함됩니다.<br>1백만 크레딧으로는 2,000개 파일 페이지를 처리할 수 있습니다.| 해당 없음 |
| 용량 | 기본 일반 데이터 서비스 환경에 대해 프로비전됩니다.| 용량 제한이 없습니다.|
| 지원되는 언어| 영어 <br>2021년 하반기 출시 예정: 스페인어, 독일어, 프랑스어, 이탈리아어| 모델은 모든 라틴어 알파벳 언어를 기반으로 작동합니다. 영어 외 언어: 독일어, 스웨덴어, 프랑스어, 스페인어, 이탈리아어 및 포르투갈어.|
