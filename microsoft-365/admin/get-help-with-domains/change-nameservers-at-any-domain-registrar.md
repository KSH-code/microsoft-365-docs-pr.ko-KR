...
3.  사용자 **지정 DNS 레코드** 추가 창의 **유형** 드롭다운 목록에서 **CNAME(별칭)을 선택합니다.**
4.  호스트 **이름 또는 별칭 상자에** www 를 **입력합니다.**
5.  주소 **지정 상자에** 웹 사이트의 FQDN(FQDN)을 입력합니다. 예를 들어 **contoso.com.**
6.  레코드의 TTL 설정을 변경하려면 **TTL** 드롭다운 목록에서 새 기간을 선택합니다. 그렇지 않은 경우 6단계를 계속 진행합니다.
7.  **저장** 을 선택합니다.
