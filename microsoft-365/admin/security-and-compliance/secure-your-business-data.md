...
이 지침 외에도 Microsoft는 사용자가 이 문서에 설명된 작업을 수행하여 해커와 맬웨어로부터 계정 및 장치를 보호하는 [것이 좋습니다.](https://support.microsoft.com/office/066d6216-a56b-4f90-9af3-b3a1e9a327d6) 이러한 작업은 다음과 같습니다.
...
Microsoft 365 환경에는 맬웨어로부터 보호가 포함되어 있지만 맬웨어에 일반적으로 사용되는 파일 형식의 첨부 파일을 차단하여 이 보호를 강화할 수 있습니다. 전자 메일에서 맬웨어 보호를 범프하거나 짧은 교육 비디오를 [보거나](https://support.microsoft.com/office/02b5783a-eea0-42e8-8856-62440718c3f0)다음 단계를 완료합니다.
...
5. 일반적인 **첨부 파일 형식 필터에서** 을 **선택합니다.** 차단된 파일 형식은 이 컨트롤 바로 아래에 있는 창에 나열됩니다. 필요한 경우 나중에 파일 형식을 추가하거나 삭제할 수 있습니다.
...
- 랜섬웨어 또는 기타 악성 코드를 포함할 수 있는 파일 형식을 차단합니다. 먼저 일반적인 실행 가능 목록(아래 표에 나열)을 시작할 것입니다. 조직에서 이러한 실행 형식을 사용하며 이러한 형식이 전자 메일로 전송될 것으로 예상되는 경우 이전 규칙에 추가합니다(사용자에게 경고).
...
5. 각 규칙에 대해 다음 표의 설정을 적용합니다. 이러한 설정을 변경하지 않는 한 나머지 설정은 기본값으로 두면 됩니다.
...
In Outlook.com, select **Protect** in the email. 기본 보호는 **전달 금지입니다.** 암호화를 위해 이 옵션을 변경하려면 사용 권한 암호화 **변경을** \> **선택합니다.**
...
Office 365용 Defender에서 피싱 방지 정책을 만들거나 [](https://support.microsoft.com/office/86c425e1-1686-430a-9151-f7176cce4f2c)짧은 교육 비디오를 보거나 다음 단계를 완료합니다.
...
3. 전자 **메일을** 제외한 콘텐츠에 적용되는 설정에서 **Office 365** 응용 프로그램을 **선택하고,** 사용자가 안전한 링크를 클릭하는 경우 추적하지 말고, 사용자가 원래 URL에 대한 안전한 링크를 클릭하지 못하게 **합니다.**
...
|메시지의 알 수 없는 악의적인 URL에 대한 작업 선택|Select On - 사용자가 링크를 클릭할 때 알려진 악성 링크 목록에 대해 URL을 다시 덮고 **검사합니다.**|
