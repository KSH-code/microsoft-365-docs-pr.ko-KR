---
title: OAuth Microsoft 365 통합을 AAD 구성
f1.keywords:
- NOCSH
ms.author: pebaum
author: pebaum
...
ms.locfileid: "60661976"
---
# <a name="configure-microsoft-365-support-integration-with-aad-oauth-token"></a>OAuth Microsoft 365 통합을 AAD 구성

## <a name="prerequisites-aad-oauth-token"></a>OAuth 토큰의 AAD 선행 준비

이러한 필수 구성은 지원 통합을 설정하는 Microsoft 365 필요합니다.
...
1. \[AAD 관리자 테넌트에서 AAD REST API에 대한 Microsoft 365 \] 응용 프로그램을 생성합니다.

    1. 테넌트 자격 증명으로 [azure Portal에](https://portal.azure.com/) Microsoft 365 로그인하고 앱 등록 페이지로 이동하여 새 응용 프로그램을 만드십시오.

    1. 이 조직 디렉터리에서 계정만 **{(Microsoft-365-tenant-name} only – Single tenant)를 선택합니다.**

        :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image22.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image22.png" alt-text="그래픽 사용자 인터페이스, 텍스트, 응용 프로그램, 전자 메일 설명이 자동으로 생성됩니다.":::

1. 응용 프로그램 클라이언트 ID를 만들고 클라이언트 비밀을 만들고 해당 값을 얻습니다.

1. \[AAD 관리자 \] 테넌트에서 AAD 사용자에 대한 Microsoft 365 응용 프로그램을 생성합니다.

    1. 테넌트 자격 증명으로 [azure Portal에](https://portal.azure.com/) Microsoft 365 로그인하고 앱 등록 페이지로 이동하여 새 응용 프로그램을 만드십시오.

    1. 이 조직 디렉터리에서 계정만 **{(Microsoft-365-tenant-name} only – Single tenant)를 선택합니다.**

        :::image type="content" source="../../media/ServiceNow-guide/ServiceNow-guide-image23.png" lightbox="../../media/ServiceNow-guide/ServiceNow-guide-image23.png" alt-text="그래픽 사용자 인터페이스, 텍스트, 응용 프로그램, 전자 메일 설명이 자동으로 생성됩니다.":::
...
1. 타사 **OAuth** 공급자 커넥트 옵션을 사용하여 새 응용 프로그램을 만들고 다음 값을 입력합니다.

    - 클라이언트 ID: OAuth 토큰 1단계에서 만든 응용 프로그램의 클라이언트 ID입니다(AAD 토큰). \#

    - 클라이언트 비밀: OAuth 토큰 1단계에서 만든 응용 프로그램의 클라이언트 AAD \# 값입니다.
...
1. \[ServiceNow 관리자 ServiceNow에서 OIDC 공급자를 구성하려면 \] 온라인 [설명서를 참조하세요.](https://docs.servicenow.com/bundle/quebec-platform-administration/page/administer/security/task/add-OIDC-entity.html)
...
1. 새로 **만들기 를** 선택한 다음 새 열기 ID 커넥트 **선택합니다.**

1. **OAuth OIDC 공급자** 구성에서 검색을 선택하고 다음 값을 사용하여 **oidc \_ 공급자 \_ 구성.목록에서** 새 OIDC 공급자 구성을 생성합니다. 

    - OIDC 공급자: **{테넌트 \_ 이름} Azure(예:** Contoso Azure)
    ...
    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image24.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image24.png" alt-text="그래픽 사용자 인터페이스, 텍스트, 응용 프로그램 설명이 자동으로 생성됩니다.":::

1. OIDC 공급자 구성을 선택하여 다음 값을 사용하여 **ID 토큰을** 확인하여 새 응용 프로그램을 만들 수 있습니다.

    - 이름: **{Tenant \_ Name} \_ 응용 프로그램 \_ 인바운드 \_ api(예:** contoso 응용 프로그램 \_ \_ 인바운드 \_ api)

    - 클라이언트 ID: OAuth 토큰 2단계에서 만든 응용 프로그램의 클라이언트 ID입니다(AAD \# 토큰).

    - 클라이언트 비밀: OAuth 토큰 2단계에서 사전 요구 사항(AAD 응용 프로그램의 앱 비밀) \#

    - OAuth OIDC 공급자 구성: 이전 단계에서 만든 OIDC 공급자

    - 리디렉션 URL: `https://{service-now-instance-name}.service-now.com/oauth\_redirect.do`

    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image25.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image25.png" alt-text="그래픽 사용자 인터페이스, 응용 프로그램 설명이 자동으로 생성됩니다.":::

1. \[ServiceNow 관리자 \] 통합 사용자 만들기

    통합 사용자를 지정해야 합니다. 기존 통합 사용자가 없는 경우 또는 이 통합에 대해 특별히 만들어야 하는 **&gt;** 경우 조직 사용자로 이동하여 새 사용자를 만들 수 있습니다. 사용자 **ID의 값은** OAuth 토큰 3단계의 사전 요구 AAD 만든 응용 [프로그램 클라이언트 \# ID입니다.](#prerequisites-aad-oauth-token)

    새 통합 사용자를 만드는 경우 웹 서비스 **액세스만 옵션을** 선택합니다. 또한 이 사용자에게 인시던트 관리자 역할을 **\_ 부여해야** 합니다.

    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image26.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image26.png" alt-text="그래픽 사용자 인터페이스, 응용 프로그램 설명이 자동으로 생성됩니다.":::
...
1. \[ServiceNow 관리자 \] 동의 프롬프트에 동의를 선택하여  계속합니다.
...
1. \[ServiceNow 관리자가 \] 아웃바운드 OAuth 공급자를 설정합니다.
    ...
    [Prerequisites (AAD OAuth Token) \# 4단계에서](#prerequisites-aad-oauth-token)만든 아웃바운드 OAuth 공급자에 대한 OAuth 프로필을 선택하고 다음 을 **선택합니다.**
...
1. \[ServiceNow 관리자가 \] 인바운드 OAuth 공급자를 설정합니다.
    ...
    1. [Prerequisites (AAD OAuth Token) \# 5단계에서 만든 OAuth 클라이언트를](#prerequisites-aad-oauth-token)선택하고 다음 을 **선택합니다.**

    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image28.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image28.png" alt-text="그래픽 사용자 인터페이스, 텍스트, 응용 프로그램 설명이 자동으로 생성됩니다.":::

1. \[ServiceNow 관리자가 \] 인바운드 통화 통합 사용자를 설정합니다.

    1. 현재 단계 **건너뛰기(Skip current step)를 Uncheck(현재 단계 건너뛰기)**

    1. [Prerequisites (AAD OAuth Token) \# 3단계에서](#prerequisites-aad-oauth-token)만든 응용 프로그램의 클라이언트 ID를 입력하고 다음 을 **선택합니다.** 

    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image39.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image39.png" alt-text="그래픽 사용자 인터페이스, 텍스트, 응용 프로그램, 전자 메일 설명이 자동으로 생성됩니다.":::
...
1. \[Microsoft 365 테넌트 관리자 \] 통합을 완료합니다.
    ...
    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image40.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image40.png" alt-text="그래픽 사용자 인터페이스, 텍스트, 응용 프로그램, 전자 메일 설명이 자동으로 생성됩니다.":::

1. 조직 **Microsoft 365 관리 조직 &gt; 프로필 설정 &gt; 포털로 &gt; 이동합니다.**

1. 지원 통합 설정을 구성합니다.

    기본 정보 **탭>** **내부** 지원 도구 ServiceNow 를 선택하고 응용 프로그램 ID에 아웃바운드 앱 ID 값을 입력하여  >   **OAuth 토큰 필드를 발급합니다.**  이 아웃바운드 앱 ID는 [Prerequisite (AAD OAuth Token) \# 1단계에서](#prerequisites-aad-oauth-token)만든 6단계 - 통합 완료에 있습니다.
...
1. **리포지토리 탭에서** **리포지토리** 추가를 선택하여 다음 설정을 사용하여 새 리포지토리를 만들 수 있습니다.
    ...
    - 리포지토리: "6단계 - 통합 완료"의 리포지토리 **ID** 값입니다.

    - 끝점:  "6단계 - 통합 완료"의 끝점 값입니다.

    - 인증 유형: 인증 **AAD 선택합니다.**

    - 클라이언트 ID: **6단계** - 통합 완료의 클라이언트 ID 값입니다.

    - 클라이언트 비밀: OAuth 토큰 2단계에서 사전 요구 사항(AAD OAuth 토큰)에서 만든 인바운드 OAuth 공급자의 \# 비밀입니다.
    ...
    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image31.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image31.png" alt-text="그래픽 사용자 인터페이스, 응용 프로그램 설명이 자동으로 생성됩니다.":::

1. ServiceNow로 돌아갈 수 있습니다.

1. **다음을** 선택하여 통합을 완료합니다.

    :::image type="content" source="../../media/ServiceNow-guide/servicenow-guide-image32.png" lightbox="../../media/ServiceNow-guide/servicenow-guide-image32.png" alt-text="그래픽 사용자 인터페이스, 응용 프로그램 설명이 자동으로 생성됩니다.":::
...
1. \[OPTIONAL \] \[ 역할 x \_ mioms \_ m365 \_ assis.administrator 링크 링크가 Microsoft 365 \] 계정입니다.
