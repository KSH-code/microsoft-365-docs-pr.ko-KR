...
## <a name="dismiss-and-restore"></a>해제 및 복원

해당 메시지가 표시 되지 않거나 이미 처리 된 경우 메시지를 해제 하 여 메시지 센터 보기에서 해당 메시지를 제거할 수 있습니다. 메시지 센터에 표시 되는 보기는 사용자 계정에 따라 달라 지 며 보기에서 파일을 해제 하면 다른 관리자에 게 영향을 주지 않습니다.

::: moniker range="o365-worldwide"

- 메시지 센터의 기본 페이지에서 메시지를 선택 하 고 메시지 목록 위에 있는 **해제** 를 선택 합니다.

- 메시지를 열고 메시지 아래쪽에서 **해제** 를 선택 합니다.

해제 된 메시지를 다시 받아야 하나요? 문제 없어요.
  
1. 메시지 센터 위쪽에서 해제 된 **메시지** 탭을 선택 합니다. 해제 된 메시지 목록이 표시 됩니다. 
...
메시지를 해제 하는 방법에는 두 가지가 있습니다.
  
- 메시지 ID 오른쪽에 표시 되는 X를 선택 합니다. 메시지가 목록에서 제거 됩니다. Or,

- 메시지를 열고 메시지 맨 위에 있는 **해제** 를 선택 합니다.

해제 된 메시지를 다시 받아야 하나요? 문제 없어요.
  
1. **보기** 드롭다운 메뉴에서 해제 된 **메시지** 필터를 선택 합니다. 해제 된 메시지 목록이 표시 됩니다.
