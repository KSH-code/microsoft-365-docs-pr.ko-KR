...
한 시간 이내에 또는 현재 Microsoft 365 페이지에서 나간 후에 다시 로그인 하 라는 메시지가 표시 됩니다. 액세스 토큰은 한 시간 동안 유효 하므로 타임 라인은 해당 토큰에 남아 있는 시간 및 현재 웹 페이지에서 외부로 이동 하는지 여부에 따라 달라 집니다.
  
> [!IMPORTANT]
> 사용자가 웹용 Outlook에 있는 경우 사서함을 클릭 하는 것 만으로는 즉시 추방 되지 않을 수 있습니다. OneDrive와 같은 다른 타일을 선택 하거나 브라우저를 새로 고치면 로그 아웃이 시작 됩니다.
...
|:-----|:-----|
|**단계** <br/> |**이렇게 하는 이유** <br/> |
|1. [이전 직원 사서함의 콘텐츠 저장](#save-the-contents-of-a-former-employees-mailbox) <br/> |이는 직원의 작업을 수행 하려는 사람이 나 소송 인 경우에 유용 합니다.  <br/> |
...
|4. [Microsoft 365 데이터에 대 한 이전 직원의 액세스 차단](#block-a-former-employees-access-to-microsoft-365-data)<br/> |이를 통해 사용자가 이전 Microsoft 365 사서함 및 데이터에 액세스 하지 못합니다.  <br/><br/> **팁**: 사용자의 액세스를 차단 해도 해당 라이선스에 대 한 비용을 지불 하 고 있습니다. 이에 대 한 비용 지불을 중지 하려면 구독에서 라이선스를 삭제 합니다 (5 단계).  |
...
|5a. 사용자가 개인용 컴퓨터를 사용하여 OneDrive 및 SharePoint에 액세스하는 경우 어떻게 해야 하나요?  <br/> |회사에서 발급한 컴퓨터 대신 개인용 컴퓨터를 사용하여 OneDrive 및 SharePoint에서 파일을 다운로드하는 경우 저장한 파일을 지울 방법이 없습니다.  <br/><br/> 컴퓨터와 동기화 된 모든 파일에 계속 액세스할 수 있습니다.  <br/> |
...
이전 직원에 게 조직 전화가 있는 경우 Exchange 관리 센터를 사용 하 여 모든 조직 데이터가 장치에서 제거 되 고 더 이상 Office 365에 연결 되지 않도록 해당 장치를 초기화 하 고 차단할 수 있습니다.
...
3. 사용자를 선택 하 고 **모바일 장치**에서 **세부 정보 보기**를 선택 합니다.
4. 모바일 장치 **세부 정보** 페이지의 **모바일**장치에서 모바일 장치를 선택 하 고 **데이터** ![ 지우기 장치 지우기를 선택한 ](../../media/1c113a36-53cb-4974-884f-3ecd9535506e.png) 다음 **차단을**선택 합니다.
5. **저장**을 선택합니다.
   > [!TIP]
...
라이선스를 제거하면 해당 사용자의 모든 데이터가 30일간 보존됩니다. 데이터에 [액세스](get-access-to-and-back-up-a-former-user-s-data.md)하거나 해당 사용자가 복귀하면 계정을 [복원](restore-user.md)할 수 있습니다. 30 일 후에는 SharePoint Online에 저장 된 문서를 제외 하 고 사용자의 모든 데이터가 Microsoft 365에서 영구적으로 삭제 되며 복구할 수 없습니다.

::: moniker range="o365-worldwide"

1. 관리 센터에서 **사용자** \> <a href="https://go.microsoft.com/fwlink/p/?linkid=834822" target="_blank">활성 사용자</a> 페이지로 이동합니다.
2. 차단할 직원의 이름을 선택 하 고 **라이선스 및 앱** 탭을 선택 합니다.
3. 제거할 라이선스에 대 한 확인란의 선택을 취소 한 다음 **변경 내용 저장**을 선택 합니다.
...
다른 사람을 고용할 때까지 비용을 **지불 하는 라이선스 수를 줄이려면** 다음 단계를 수행 합니다.

::: moniker range="o365-worldwide"
1. 관리 센터에서 제품 **대금 청구** 페이지로 이동 하 여 \> <a href="https://go.microsoft.com/fwlink/p/?linkid=842054" target="_blank">Your products</a> **제품** 탭을 선택 합니다.
2. 라이선스를 제거할 구독을 선택 합니다.
3. 세부 정보 페이지에서 **라이선스 제거**를 선택 합니다.
4. **라이선스 제거** 창의 새 수량에 있는 **총 라이선스** 수 상자에이 구독에 대해 원하는 총 라이선스 개수를 입력 합니다. 예를 들어 25 개의 라이선스가 있고 이러한 라이선스 중 하나를 제거 하려면 24를 입력 합니다.
5. **저장**을 선택합니다.
::: moniker-end
...
Microsoft 365 for business에 대 한 사용자 라이선스를 관리 하는 방법에 대 한 자세한 내용은 microsoft 365의 사용자 [에 게 라이선스 할당](../manage/assign-licenses-to-users.md)및 [비즈니스를 위한 microsoft 365 사용자 로부터 라이선스](../manage/remove-licenses-from-users.md)할당 해제를 참조 하세요.
...
Active Directory에서 사용자 계정을 삭제 하 고 복원 하는 방법에 대 한 자세한 내용은 [사용자 계정 삭제](https://go.microsoft.com/fwlink/?linkid=841808)를 참조 하십시오.
  
Azure Active Directory를 사용 하는 경우 [Get-msoluser](https://go.microsoft.com/fwlink/?linkid=842230) PowerShell cmdlet을 참조 하세요.
...
|세션 종료 및 향후 세션에 대한 액세스 차단(모든 프로토콜에 대해)  <br/> |계정을 사용하지 않도록 설정합니다. 예를 들어 Exchange 관리 센터에서 또는 PowerShell을 사용 하 여 다음을 수행 합니다.  <br/>  `Set-Mailbox user@contoso.com -AccountDisabled:$true` <br/> |
|특정 프로토콜(예: ActiveSync)에 대한 세션 종료  <br/> |프로토콜을 사용하지 않도록 설정합니다. 예를 들어 Exchange 관리 센터에서 또는 PowerShell을 사용 하 여 다음을 수행 합니다.  <br/>  `Set-CASMailbox user@contoso.com -ActiveSyncEnabled:$false` <br/> |
