...
- 사용자의 OneDrive 데이터를 유지하려는 경우 다른 위치로 이동합니다. 계정을 삭제 한 후 최대 30 일까지 데이터를 이동할 수도 있습니다. [이전 사용자의 데이터 액세스 및 백업](get-access-to-and-back-up-a-former-user-s-data.md)을 참조하세요. 해당 SharePoint 파일을 이동할 필요는 없으며 계속 액세스할 수 있습니다.
...
|항목 | 설명 |
|:-----|:-----|
|제품 라이선스  <br/> |사용자 로부터 라이선스를 제거 하 고 해당 라이선스에 대 한 비용 지불을 중지할 수 있습니다. 이 옵션을 선택 하면 구독이 구독에서 자동으로 제거 됩니다.  <br/><br/> 파트너 또는 볼륨 라이선스를 통해 **라이선스를 구입한 경우에는 제거할 수 없습니다** . 연간 요금제를 지불 하 고 있거나 대금 청구 주기의 중간에 있는 경우에는 약정을 완료할 때까지 구독에서 라이선스를 제거할 수 없습니다.  <br/> |
...
2. 삭제할 사용자를 선택한 다음 **사용자 삭제** 를 선택 합니다.
...
2. **제품** 탭에서 라이선스를 제거할 구독을 선택 합니다.

3. 구독 정보 페이지에서 **라이선스 제거** 를 선택 합니다.

4. **라이선스 제거** 창의 **새 수량** 에 있는 **총 라이선스** 수 상자에이 구독에 대해 원하는 총 라이선스 개수를 입력 합니다. 예를 들어 100 라이선스가 있고 5 개를 제거 하려면 95를 입력 합니다.
...
- **"사용자를 삭제할 수 없습니다. 라는 줄에 오류 메시지가 표시 됩니다. 나중에 다시 시도 하세요. "** 계정에 전자 메일 전달이 설정 되어 있는지 또는 공유 사서함으로 변환 되었는지 다시 확인 합니다. 두 경우 모두 해당 오류가 발생 합니다. 전자 메일 전달이 있거나 공유 사서함으로 변환 된 경우에는 계정을 삭제 하지 마세요.
...
- **사용자를 삭제 했지만이 이름이 계속 전체 주소록에 나타납니다**. 이 작업은 회사에서 Active Directory를 사용 하는 경우에 발생 합니다. Active Directory에서 사용자 계정을 삭제 해야 합니다. 자세한 내용은 [사용자 계정 삭제](https://go.microsoft.com/fwlink/p/?linkid=841808) 를 참조 하십시오.
