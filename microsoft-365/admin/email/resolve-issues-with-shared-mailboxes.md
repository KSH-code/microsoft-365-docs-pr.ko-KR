...
공유 사서함을 만든 다음 해당 사서함에서 메시지를 보내려고 시도하면 다음 메시지가 표시될 수 있습니다.

**이 메시지를 보낼 수 없습니다. 지정된 사용자를 대신하여 메시지를 보낼 수 있는 권한이 없습니다.**

이 메시지는 Microsoft 365에서 복제 대기 시간 문제가 발생하는 경우 표시됩니다. 새 공유 사서함(또는 추가된 사용자)에 대한 정보가 모든 데이터 센터에 복제될 때 1시간 정도 지나지 않습니다. 1시간을 기다렸다가 메시지를 다시 보내려고 시도합니다.
