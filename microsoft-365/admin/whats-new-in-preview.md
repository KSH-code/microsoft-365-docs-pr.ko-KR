...
## <a name="january-2020---happy-new-year"></a>1 월 2020 일 새 해 행복!

> [!NOTE]
> YouTube의 [Microsoft 365 비디오 시리즈에 새로](https://go.microsoft.com/fwlink/p/?linkid=2118096) 제공 되는 기능을 알고 계십니까? 사용자에 게 롤업되는 최신 기능을 강조 표시 합니다. 매달 [비디오, 교육 및 docs](#videos-training-and-docs) 섹션에서 최신 에피소드 연결을 시작 합니다. <br> <br> January's 에피소드: [1 월 2020-Microsoft 365의 새로운 기능](https://go.microsoft.com/fwlink/p/?linkid=2118096)
...
> [!IMPORTANT]
> 이제 일반 URL을 사용 하 여 SharePoint 관리 센터에 액세스할 수 있습니다.https://admin.microsoft.com/SharePoint
...
### <a name="featured-feedback-fix--the-option-to-convert-the-deleted-users-mailbox-to-a-shared-mailbox-is-back"></a>중요 한 피드백 수정 – 삭제 된 사용자 사서함을 공유 사서함으로 변환 하는 옵션은 뒤로 이동 합니다.
...
## <a name="march-2019---weve-officially-released-the-admin-center"></a>3 월 2019-관리 센터를 공식적으로 출시 했습니다.
...
- 또한 몇 가지 더 작은 향상 된 기능과 버그 수정도 수행 했습니다.
  ...
  - 또한 인벤터리 세부 정보 페이지에 몇 가지 알림 기능이 추가 되었으며, 작업을 수행할 수 있는 실제 페이지로 연결 되 고 재고 정보 카드에 더 많은 작업이 포함 됩니다. **청구** > **** 금액 > 으로 이동 하 여 모든 송장에 대 한**세부 정보 보기**

## <a name="january-2019---happy-new-year"></a>1 월 2019 일 새 해 행복!
...
체크 아웃: 사용자 **> 활성 사용자 >** **사용자 > 라이선스 및 앱**을 편집 하거나 추가할 수 있습니다.

## <a name="december-2018"></a>2018년 12월

- **서비스 & 추가 기능** -10 월에 새 디자인 **** \> 스타일을 사용 하 여 **추가 기능 & 설정 서비스** 를 업데이트 했지만 모든 설정 페이지가 여전히 클래식 스타일 인 경우 이번 달에는 새 디자인에 대 한 실제 설정을 업데이트 하기 시작 했습니다. **예약** 을 체크 아웃 하 여 새로운 모양을 확인 합니다.
- **사용자 관리** -사용자 이름, 별칭, 기본 전자 메일 주소를 관리 합니다.
- 두 개의 홈 카드를 라이브러리에 추가 했습니다. 라이브러리에서 전체 설명을 읽을 수 있습니다.
  - **Intune 장치 등록**
  - **도메인**
- **권장 사항** -관리 센터에 새로운 추천 팝업이 표시 될 수 있습니다. 물론 조직에 이익이 있다고 생각 되는 경우에만 추천을 볼 수 있습니다. 하지만 권장 사항을 확인할 때까지 기다리지 않으면 카드 라이브러리에서이를 추가할 수 있습니다.
  - **중요 한 정보 보호** -중요 한 정보가 포함 된 항목이 조직 외부의 사용자와 공유 되는 경우를 검색 하는 DLP (데이터 손실 방지) 정책을 만드는 것이 좋습니다.

## <a name="november-2018"></a>2018년 11월

- 검색 상자가 뒤로 이동 합니다. 이를 약간 이동 했습니다. 홈 페이지의 맨 위에 있지만 이제는 페이지 오른쪽의 **명함 추가** 단추 근처에 있습니다. <br> ![검색 상자가 홈 페이지의 오른쪽 위에 있음을 설명 하는 화면 캡처](media/M365PreviewSearchBox.png)

- 대시보드에 대 한 카드를 추가 했습니다. 관리 센터로 이동한 후 **명함 추가** 를 선택 하 여 새 카드를 확인 하기만 하면 됩니다.
  - **Office 365 활성 사용자 보고서** -구매한 Office 365 서비스의 사용을 모니터링 합니다.
  - **Office 365 소프트웨어** -Office 365 ProPlus 소프트웨어를 설치 또는 배포 하 고 정품 인증을 받은 사용자 수를 확인 합니다.
  - **Azure Active Directory** -셀프 서비스 암호 재설정, 로그인 사용자 지정 및 앱 추가와 같은 일반적인 Azure AD 작업에 빠르게 액세스할 수 있도록 합니다.
- **사용자 내보내기** 빠른 작업은 활성 사용자 페이지의 명령 모음에 추가 됩니다. ![새로 고침 옆에 있는 명령 모음의 사용자 내보내기를 보여 주는 화면 캡처](media/M365PreviewExportUsersQT.png)
- 구매 서비스에는 11 월에 대 한 추가 향상 기능이 있습니다. 그리고 모든 것이 좋습니다! 받으려면 결제 > 구입 서비스를 방문 하세요. <br>![향상 된 3 가지 새로운 기능을 포함 하는 purchase services 페이지를 보여 주는 화면 캡처](media/M365PreviewBillingPurchasServices.png)
  > 1.  페이지 맨 위에는 비즈니스 계획과 학력 계획 중에서 피벗을 할 수 있습니다. 이를 통해 원하는 정보를 쉽게 찾을 수 있습니다.
  > 2.  원하는 정보를 찾기 위해 계속 해 서 스크롤하지 않아도 되는 검색 상자가 추가 되었습니다.
  > 3.  마지막으로 확장 및 축소할 수 있는 섹션으로 구매할 수 있는 요금제를 논리적으로 정렬 했습니다. **Office 앱 및 서비스**를 구매할 필요가 없습니까? 이 섹션을 축소 하 고 Microsoft 365 계획에 집중 합니다.

## <a name="october-2018"></a>2018년 10월

- Microsoft 365 관리 센터에서 구독을 구입할 수 있는 새로운 방법이 있습니다. **서비스 구매** 로 이동 하 여 구독을 보다 쉽게 구입 하는 데 도움이 되는 다양 한 기능의 검색, 계획 및 기타 고급 기능을 확인할 수 있습니다. <br/>![Microsoft 365 관리 센터 미리 보기에서 서비스 계획 비교 구매](media/0ad5acf1-8ffc-44bf-aac2-c21269910e65.png)

- **서비스 및 추가 기능** 페이지가 새로운 모양 및 느낌으로 업데이트 되었지만 다른 모든 작업은 동일 하 게 작동 합니다. 물론 목록에서 간단히 보기로 변경 하는 등의 작업을 수행 하려면 명령 모음을 체크 아웃 해야 합니다. 탐색 창에서 **설정** \> **서비스 &amp; 추가 기능** 으로 이동 하 여 체크 아웃 합니다. <br/>![Microsoft 365 관리 센터 미리 보기의 설정 및 추가 기능 페이지](media/af9a3ed3-bfe6-438b-b4b5-2ca4164f8e3f.png)

## <a name="september-2018"></a>2018년 9월

- **향상 된 홈 기능** 홈 페이지가 완전히 다시 완료 되었으므로 사용자가 작업을 시도 하 여 생각 되는 내용을 확인할 수 없습니다. 사용자의 개인 홈 페이지 이며 조직의 다른 관리자 홈 페이지에는 영향을 주지 않습니다. <br/>![화면 캡처: Microsoft 365 관리 센터 미리 보기 홈 페이지](media/d1a497f1-30e6-4c1a-9bb2-d4d7895906da.png)

  - 홈 페이지에서 홈페이지 카드를 추가, 제거 및 이동할 수 있습니다. 오른쪽 위 모서리에 있는 **+ 추가** 를 선택 하 여 카드 라이브러리에서 사용할 수 있는 카드를 확인 한 다음 홈 페이지에서 임의의 위치로 끌어 놓습니다. 마법사로 돌아갑니다 하는 위치를 마음에 들지 않습니까? 원하는 위치로 끕니다. 더 이상 저장 하지 않아도 됩니다.

  - 카드를 섹션으로 그룹화 하 여 조직을 다음 수준으로 이동 합니다. 기본 및 **장치 및 업데이트** 의 두 가지 기본적인 섹션을 **만들었습니다.** 이제 섹션을 추가, 제거 및 이름을 변경 하 여 자신에 게 적합 한 홈페이지를 디자인할 수 있게 됩니다.

  - 대부분의 카드는 정보 및 작업을 수행할 수 있으며,이는 일상적인 작업을 보다 빠르고 효율적으로 수행 하는 데 도움이 되도록 just-in-time 정보를 제공 하는 것입니다.

  - 탐색 창은 다시 구성 되었으며 빠른 작업을 위해 사용자 지정할 수 있습니다. 탐색 창의 아래쪽에서 **편집** 을 선택 하 여 사용자 지정을 시작 하면 됩니다. <br/>![Microsoft 365 관리 센터 미리 보기 탐색 창](media/7d5568cc-9ce8-40b2-aa87-e777fcd455e3.png)

- **대금 청구** 청구 섹션에서 Microsoft 365 관리 센터에 대 한 몇 가지 변경 사항을 볼 수 있을 것입니다. Microsoft 또는 다른 공급 업체 로부터 구매한 구독 및 앱을 관리 하 고 재고 관리, 주문 내역, 지불 방법 및 송장 발부에 대 한 새로운 경험을 포함할 수 있습니다.

  - 구독 관리는 앱, 소프트웨어, 추가 기능 등의 추가 제품 유형에 대 한 새로운 재고 관리 환경을 제공 하기 위해 새롭게 설계 되었습니다. 새로 추가 되는 제품 유형에 대 한 지원을 보다 잘 반영 하기 위해 이제 **제품 &amp; 서비스가** 제공 됩니다. 이제 제품 및 서비스에 대 한 카드나 표 보기를 선택 하 고 검색 및 필터링 하 여 원하는 정보를 빠르게 찾을 수 있습니다. <br/>![Microsoft 365의 제품 및 서비스 관리 센터 미리 보기](media/f1bba7dc-2034-4a53-b4ad-b7fbef3fb442.png)

  - 이제 사용자에 게 제품 라이선스를 할당 하는 것이 전보다 더 쉽습니다. 구매한 제품을 선택 하 고 제품을 할당할 수 있습니다. 특정 제품이 이미 할당 된 사용자를 확인할 수도 있습니다. <br/>![Microsoft 365 관리 센터 미리 보기의 대금 청구에서 제품 세부 정보](media/82422685-e1b7-4f3f-aea8-1819df9d9311.png)

  - **스위치 요금제** 는 이제 * * upgrade * *로 불리 며, 마지막으로 구독을 업그레이드 하는 경우에는 업그레이드 완료를 감시할 필요가 없습니다. 마지막으로 업그레이드에 대 한 라이선스 제한은 더 이상 없습니다.

- **사용자 관리** Microsoft는 작업을 보다 신속 하 게 수행할 수 있도록 사용자 관리를 많이 변경 했습니다. * * 활성 사용자 * * 페이지와 사용자의 세부 정보 창이 업데이트 되었습니다. 다음은 보다 놀라운 변경 사항입니다.

  - 새 디자인 및 **활성 사용자** 페이지에 새로운 레이아웃이 제공 됩니다. 목록이 표시 되는 방식이 마음에 들지 않습니까? **필터**옆에 있는 **변경 보기** 를 선택 하 고 **기본 목록** 에서 **간단히**목록으로 목록을 변경할 수 있습니다. 압축 목록은 특히 조직에 사용자가 많은 경우에 유용 합니다. <br/>![미리 보기의 활성 사용자 페이지](media/4dd9f1c9-ef23-4194-9ff8-5206ba2930f8.png)

  - **검색** 내용이 동적인 명령 모음 (위치)은 선택한 사용자 수, 사용자 한 명 또는 여러 명에 따라 변경 됩니다. 또한 **검색**옆에 선택한 사용자 수를 볼 수 있습니다. <br/>![Microsoft 365 preview 활성 사용자 명령 모음입니다.](media/9576bd19-31a5-4e59-a8dd-0663a833def0.png)

  - 목록 보기에서 직접 한 명 또는 여러 명의 사용자에 대 한 암호를 다시 설정 하는 등의 빠른 작업을 수행할 수 있습니다.

  - 사용자의 세부 정보 및 설정은 이제 **계정**, **장치**, **라이선스**, **앱**, **메일**및 **OneDrive**탭으로 구성 됩니다. 이전에는 이러한 설정이 여러 화면에 걸쳐 있습니다. <br/>![사용자 세부 정보 창 (관리 센터 미리 보기)](media/60c6b9d3-d62f-4b8d-97db-41ff247b1bf0.png)<br/>
