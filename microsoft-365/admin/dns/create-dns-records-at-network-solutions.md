---
title: 커넥트 솔루션에서 DNS 레코드를 확인하여 Microsoft 365
f1.keywords:
- CSH
ms.author: pebaum
author: pebaum
...
ms.locfileid: "60557031"
---
# <a name="connect-your-dns-records-at-network-solutions-to-microsoft-365"></a>커넥트 솔루션에서 DNS 레코드를 확인하여 Microsoft 365
...
네트워크 솔루션에서 이러한 레코드를 추가하고 나면 도메인이 네트워크 솔루션과 함께 Microsoft 서비스.
...
1. 수정할 도메인 옆의 확인란을 선택합니다.
  
1. 작업 **아래에서** 세 개의 점을 선택한  다음 드롭다운 목록에서 관리를 선택합니다.

    :::image type="content" source="../../media/dns-networksolutions/networksolutions-domains-1.png" alt-text="드롭다운 목록에서 관리를 선택합니다.":::
  
1. 아래로 스크롤하여 **고급** 도구 를 선택하고 **고급 DNS 레코드** 옆에 있는 관리 를 **선택합니다.**

    :::image type="content" source="../../media/dns-networksolutions/networksolutions-domains-2.png" alt-text="고급 DNS 레코드 옆에 있는 관리를 선택합니다.":::

    고급 DNS 레코드  관리 페이지로 이동하려면 계속을 선택해야 할 수 있습니다.
  
1. 계속을 **선택하고** 고급 DNS 레코드 관리 페이지에서 **+레코드 추가를 선택합니다.**
...
1. In the boxes for the new record, type or copy and paste the values in the following table.
    ...
    |**참조 대상**|**TXT 값**|**TTL**|
    |:-----|:-----|:-----|
    |@  <br/> (The system will change this value to **@ (None)** when you save the record.)  <br/> |MS=ms *XXXXXXXX*  <br/> **참고:** 이 값은 예시입니다. 여기에는 표에 있는 특정 **대상 또는 주소 가리키기** 값을 사용합니다.  [이 값을 찾는 방법](../get-help-with-domains/information-for-dns-records.md) |3600  <br/>    |
  
1. 추가 **를 선택합니다.**
  
    > [!NOTE]
    > 만든  TXT 레코드를 보기 위해 오른쪽 위에 있는 클래식 보기를 선택합니다.   
...
1. 새 레코드의 상자에서 다음 표의 값을 입력하거나 복사하여 붙여넣습니다.
    ...
    **참조 대상** |**메일 서버**|**Priority(우선 순위)**|**TTL**|
    |:-----|:-----|:-----|:-----|
    | @ | *\<domain-key\>*  .mail.protection.outlook.com  <br/> **이 값은 기간(.)으로 끝날 수 없습니다.** <br/> **참고:** Microsoft  *\<domain-key\>*  계정에서 다운로드하세요. [이 값을 찾는 방법](../get-help-with-domains/information-for-dns-records.md) | 0  <br/> 우선 순위에 대한 자세한 내용은 [MX 우선 순위란?](../setup/domains-faq.yml)을 참조하세요. <br/> | 1시간  <br/> |  
...
1. 다른 MX 레코드가 있는 경우 편집 도구를 선택한 다음 각 레코드에 대해 **삭제를** 선택하여 모든 레코드를 삭제합니다. 
...
1. 고급 **도구 를 선택하고** 고급 DNS **레코드** 옆에 있는 관리 **를 선택합니다.**
...
1. CNAME 레코드의 상자에서 다음 표의 값을 입력하거나 복사하여 붙여넣습니다.
    
    |**참조 | 호스트 이름**|**별칭 대상**| **TTL** |
    |:-----|:-----|:-----|:-----|
    |기타 호스트| autodiscover  <br/> | autodiscover.outlook.com 이 **값은 마침침(.)으로 끝날 수 없습니다.** <br/> 1 Hour  <br/> |
  
1. 추가 **를 선택합니다.**

    > [!NOTE]
    > 만든 **레코드를** 보기 위해 오른쪽 위에 있는 클래식 보기를 선택합니다.  
...
1. In the boxes for the new record, type or copy and paste the following values.
    ...
    |**참조 대상**|**TXT 값**| **TTL**
    |:-----|:-----|:-----|
    |@  <br/> (The system will change this value to **@ (None)** when you save the record.)  <br/> |v=spf1 include:spf.protection.outlook.com -all  <br/> **참고:** 모든 공백이 올바르게 유지되도록 이 항목을 복사하여 붙여 넣는 것이 좋습니다. | 1 Hour  <br/> |
...
1. 두 개의 새 레코드 상자에서 다음 표의 값을 입력하거나 복사하여 붙여넣습니다.
    
    (드롭다운 목록에서 **서비스** 및 **프로토콜** 값을 선택합니다.) 
    
    | **유형** |**서비스**|**프로토콜**|**Weight(가중치)**|**Port(포트)**|**대상**|**Priority(우선 순위)**|**TTL**|
    |:-----|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
    | SRV |_sip  <br/> |_tls  <br/> |100  <br/> |443  <br/> |sipdir.online.lync.com  <br/> **이 값은 기간(.)으로 끝날 수 없습니다.** <br/> | 1  <br/> | 1시간  <br/> |
    | SRV |_sipfederationtls  <br/> |_tcp  <br/> |100  <br/> |5061  <br/> |sipfed.online.lync.com  <br/> **이 값은 기간(.)으로 끝날 수 없습니다.** <br/> |1  <br/> | 1 Hour  <br/> |
