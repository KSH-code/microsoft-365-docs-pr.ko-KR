...
Azure에 등록할 때 DNS 영역 내에 리소스 그룹을 만든 다음 해당 리소스 그룹에 도메인 이름을 할당합니다. 해당 도메인 이름은 외부 도메인 등록 기관에 등록됩니다. Azure는 도메인 등록 서비스를 제공하지 않습니다.
...
2. 다음 표의 값을 사용하여 이름 서비스 레코드를 두 개 만들거나 이러한 값과 일치하게 기존 이름 서비스 레코드를 편집합니다. Azure에 할당된 이름 서서의 예는 다음과 같습니다.
...
2. 대시보드 **페이지의 검색** **표시줄을** 사용하여 DNS 영역 **을 입력합니다.** 결과 표시에서 서비스 부분 아래에 **있는 DNS** **영역 을** 선택합니다. 리디렉션된 후 업데이트할 도메인을 선택합니다.
    
    ![Azure-BP-Configure-1-2](../../media/eb4baad2-92d7-49c9-95e5-1dd8510d5ec9.png)
  
3. 도메인의 **설정** 페이지의 DNS 영역 **영역에서** **+ 레코드 집합 을 선택합니다.**
    
    ![Azure-BP-Configure-1-3](../../media/b04db89a-3dbc-4cd2-aaca-af17fda53a60.png)
  
4. 레코드 **집합 추가 영역의** 새 레코드 집합용 상자에서 다음 표의 값을 선택합니다. 
    
    **(드롭다운 목록에서 유형** 및 **TTL** 단위 값을 선택합니다.) 
...
4. 레코드 **집합 추가 영역의** 새 레코드 집합용 상자에서 다음 표의 값을 선택합니다. 
    ...
    **(드롭다운 목록에서 유형** 및 **TTL** 단위 값을 선택합니다.) 
    ...
    |:-----|:-----|:-----|:-----|:-----|:-----|
    |@  <br/> |MX  <br/> |1  <br/> |시간  <br/> |10    <br/> 우선 순위에 대한 자세한 내용은 [MX 우선 순위란?](../setup/domains-faq.yml)을 참조하세요. <br/> | *\<domain-key\>*  .mail.protection.outlook.com  <br/> **참고:** Microsoft  *\<domain-key\>*  계정에서 다운로드하세요.   [이 값을 찾는 방법](../get-help-with-domains/information-for-dns-records.md)  
...
6. MX 레코드 구역에 나열된 다른 **MX** 레코드가 있는 경우 해당 레코드를 삭제해야 합니다. 
    
    먼저 DNS 영역 **영역에서** **MX 레코드 집합 을 선택합니다.**
...
7. 상황에 맞는 **메뉴(...)를** 선택한 다음 제거 를 **선택합니다.**
...
4. 4개의 CNAME 레코드 중 첫 번째 레코드를 추가합니다.
    
    레코드 **집합 추가 영역의** 새 레코드 집합용 상자에 다음 표에 있는 첫 번째 행의 값을 입력하거나 복사하여 붙여넣습니다. 
...
6. 다른 세 개의 CNAME 레코드를 각각 추가합니다.
    ...
    DNS 영역 **영역에서** **+ 레코드 집합 을 선택합니다.** 그런 다음 빈 레코드 집합에서 테이블의 다음 행 값을 사용하여 레코드를 만든  다음 확인을 다시 선택하여 해당 레코드를 완료합니다. 
...
3. DNS **영역 영역에서** **TXT 레코드 집합 을 선택합니다.**
