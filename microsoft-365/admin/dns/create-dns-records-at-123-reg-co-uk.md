---
title: 커넥트 DNS 레코드를 123-reg.co.uk Microsoft 365
f1.keywords:
- CSH
ms.author: pebaum
author: pebaum
...
다음에 이러한 레코드를 123-reg.co.uk 도메인이 해당 레코드와 함께 작동하게 Microsoft 서비스. 
...
2. 도메인을 **선택하고** 도메인 이름 개요 페이지에서 확인할 도메인의 이름을 선택하거나 제어판으로 이동합니다.
...
5. 새 레코드의 **종류** 상자에 있는 드롭다운 목록에서 **TXT/SPF를** 선택한 다음 다음 표의 다른 값을 입력하거나 복사하여 붙여넣습니다. 
...
5. 새 **레코드의** 종류 상자에 있는 드롭다운 목록에서 **MX를** 선택한 다음 다음 표의 다른 값을 입력하거나 복사하여 붙여넣습니다.
...
7. 다른 MX 레코드가 있는 경우 해당 레코드의  삭제(휴지통) 아이콘을 선택하여 각 레코드를 제거합니다.
...
5. 처음 2개의 SRV 레코드를 추가합니다.
   ...
   새 **레코드의 유형** 상자에 있는 드롭다운 목록에서 **SRV를** 선택한 다음 다음 표의 다른 값을 입력하거나 복사하여 붙여넣습니다.

    ||||||
    |:-----|:-----|:-----|:-----|:-----|
    |**호스트 이름**|**종류**|**Priority(우선 순위)**|**TTL**|**대상 SRV**|
