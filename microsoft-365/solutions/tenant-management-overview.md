...
Microsoft 365 테넌트의 올바른 구성은 이러한 기반을 제공하며, 직원들은 업무를 완료하는 데 집중하고 IT 부서는 추가 비즈니스 가치를 제공하는 종단-종단식 솔루션에 집중할 수 있도록 합니다. 
...
4. Windows 장치, Office 클라이언트 및 사내 Office 서버 및 데이터 마이그레이션
5. 장치 및 앱 관리 배포
...
Microsoft 365 테넌트는 Microsoft 365 및 유럽 또는 북미와 같은 특정 기본 위치에 저장된 조직 데이터의 전용 인스턴스입니다. 이 위치는 조직에 대한 테넌트를 만들 때 지정됩니다. 각 Microsoft 365 테넌트는 다른 모든 Microsoft 365 테넌트와는 고유하며 고유합니다. Microsoft 365 E3 또는 E5와 같은 Microsoft에서 하나 이상의 제품을 구매할 때 Microsoft 365 테넌트와 각 제품에 대한 라이선스 집합을 만들 수 있습니다.

Microsoft 365 테넌트에는 사용자 계정, 그룹 및 기타 개체에 대한 Azure AD 전용 인스턴스인 Azure AD(Azure Active Directory) 테넌트도 포함됩니다. 각 Azure AD 테넌트는 다른 모든 Azure AD 테넌트와는 고유하며 고유합니다. 조직에 Azure 구독으로 설정할 수 있는 여러 Azure AD 테넌트가 있을 수 있는 반면, Microsoft 365 테넌트는 테넌트 생성 시 만들어진 Azure AD 테넌트 하나만 사용할 수 있습니다. 
...
테넌트의 올바른 이름과 위치 이외에 Microsoft Teams 및 Exchange Online과 같은 클라우드 생산성 앱에 대한 사용자 환경이 효과적이고 안전하고, 효율적인지 보장하기 위해 계획, 배포 및 관리할 추가 요소가 &mdash; &mdash; 있습니다.
...
- 사내 Office 서버 및 해당 데이터가 클라우드 앱으로 마이그레이션되거나 하이브리드 구성에서 사용 중입니다.
- Microsoft 365에서 기본 제공되는 Intune 또는 Basic Mobility and Security를 사용하여 장치 관리를 수행하고 있습니다.
...
| 기능 또는 특징 | 설명 | 라이선싱 |
|:-------|:-----|:-------|
| 네트워크 인사이트 | Microsoft 365 테넌트에서 수집한 네트워크 성능 메트릭을 통해 사무실 위치의 네트워크 경계를 디자인할 수 있습니다. | Microsoft 365 E3 혹은 E5 | 
...
| 기능 또는 특징 | 설명 | 라이선싱 |
|:-------|:-----|:-------|
| Windows 10으로 마이그레이션 | Windows 7 또는 Windows 8.1을 실행한 장치를 Windows 10 Enterprise로 마이그레이션합니다. | Microsoft 365 E3 또는 E5에 포함된 Windows 10 Enterprise 라이선스 | 
| 엔터프라이즈용 Microsoft 365 앱으로 마이그레이션 | Word 및 PowerPoint와 같은 Office 클라이언트 앱을 새 기능으로 업데이트된 클라우드에서 설치한 버전으로 마이그레이션합니다. | Microsoft 365 E3 혹은 E5 | 
| Microsoft 365로의 사내 서버 및 데이터 마이그레이션 | Exchange 사서함, SharePoint 사이트 및 비즈니스용 Skype Online을 Microsoft 365 클라우드 서비스로 마이그레이션합니다. | Microsoft 365 E3 혹은 E5 | 
||||
...
| 기본 모바일 및 보안 | 이 기본 제공 서비스를 사용하여 사용자의 모바일 장치(iPhone, iPad, Androids 및 Windows 휴대폰)를 보호하고 관리합니다.  | Microsoft 365 E3 혹은 E5 | 
||||
...
4. [프레미스 Office 서버 및 데이터 마이그레이션](tenant-management-migration.md)
...
대표적이지만 대표적인 다국적 조직이 Microsoft 365 테넌트의 요소를 배포한 방법을 이해하기 위해 [Contoso 사례](../enterprise/contoso-case-study.md)연구를 참조합니다.
