...
테넌트 간에 Exchange Online 사서함을 이동하기 전에(미리 보기) 테넌트 간에 Exchange Online 사서함을 이동하기 전에 사용자 사서함을 현재 테넌트(원본 테넌트)에서 온-프레미스로 완전히 오프보드한 다음 새 테넌트(대상 테넌트)에 온보드해야 합니다. 새로운 테넌트 간 사서함 마이그레이션 기능을 사용하면 원본 테넌트와 대상 테넌트의 테넌트 관리자가 모두 해당 테넌트 간에 사서함을 이동할 수 있으며, 해당 테넌트 간의 인프라 종속성은 최소 수준으로 유지될 수 있습니다. 이렇게 하여 사서함을 오프보드하고 온보드할 필요가 없습니다.
...
- 조직의 직원 관리 변경에 따라 라이선스가 다수 또는 너무 많은 미지정 라이선스가 없는 경우를 방지할 수 있습니다.
