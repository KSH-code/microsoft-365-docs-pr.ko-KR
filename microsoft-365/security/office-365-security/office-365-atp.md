...
- **[안전한 첨부파일](atp-safe-attachments.md)**: 전자 메일의 첨부 파일에서 악성 콘텐츠가 있는지 확인하여 메시지 시스템을 보호하는 제로-데이 보호 기능을 제공합니다. 바이러스/맬웨어 서명이 없는 모든 메시지 및 첨부 파일을 특수 환경으로 경로 지정한 다음 기계 학습 및 분석 기법을 사용하여 악의적인 의도를 탐지합니다. 의심스러운 활동이 없는 경우 메시지가 사서함으로 전달됩니다. 자세한 내용은 [안전한 첨부 파일 정책 설정](set-up-atp-safe-attachments-policies.md)을 참조하세요.
