...
|1xx, 2xx, 3xx, 4xx, 5xx|메시지가 암시적 인증을 통과한 이유나 인증이 없지만 조치가 적용되지 않은 이유에 대한 다양한 내부 코드에 해당합니다.|
|6xx|메시지가 암시적 전자 메일 인증에 실패했음을 의미하며 전송 도메인은 조직에서 허용하는 도메인 중 하나입니다(이는 자체 대 자체 또는 내부 조직, 스푸핑의 일부).|
...
|:-----|:-----|:-----|:-----|
|DMARC 실패(격리 또는 거부)|HSPM(기본)도 SPM 또는 PHSH일 수 있습니다.|아니오(아직 아님)|모든 Office 365 고객, Outlook.com|
|자체|SPM|예|모든 Office 365 조직, Outlook.com|
|도메인 간|스푸핑|예|Office 365 고급 위협 보호 기능 및 E5 고객|
...
그 다음 [설정-AntiphishPolicy](https://docs.microsoft.com/powershell/module/exchange/advanced-threat-protection/Set-AntiPhishPolicy)의 설명서에 따라 PowerShell을 사용하여 피싱 방지 정책 매개 변수를 수정할 수 있습니다. $name을 매개 변수로 지정할 수 있습니다.
...
#### <a name="method-1---if-your-organization-owns-the-domain-set-up-email-authentication"></a>방법 1 - 조직에서 도메인을 소유한 경우 전자 메일 인증을 설정합니다.
...
#### <a name="method-2---use-spoof-intelligence-to-configure-permitted-senders-of-unauthenticated-email"></a>방법 2 - 스푸핑 인텔리전스를 사용하여 인증되지 않은 이메일의 허용된 보낸 사람 구성
...
#### <a name="method-3---create-an-allow-entry-for-the-senderrecipient-pair"></a>방법 3 - 보낸 사람/받는 사람 쌍에 대한 허용 항목 만들기
...
#### <a name="method-4---contact-the-sender-and-ask-them-to-set-up-email-authentication"></a>방법 4 - 보낸 사람에게 연락하여 전자 메일 인증을 설정하도록 요청
