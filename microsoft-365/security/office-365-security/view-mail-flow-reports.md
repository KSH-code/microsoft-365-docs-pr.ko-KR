...
[필요한 권한이](#what-permissions-are-needed-to-view-these-reports)있는 경우 **보고서** 대시보드로 이동 하 여 [보안 & 준수 센터](https://office.protection.com) 에서 이러한 보고서를 볼 수 있습니다 \> **Dashboard**. 보고서 대시보드로 직접 이동 하려면를 엽니다 <https://protection.office.com/insightdashboard> .
...
### <a name="funnel-view-for-the-mailflow-status-report"></a>메일 흐름 상태 보고서에 대 한 깔때기 보기

**깔때기형** 보기는 Microsoft의 전자 메일 위협 보호 기능이 조직에서 들어오고 나가는 전자 메일을 필터링 하는 방법을 보여 줍니다. 또한 전체 전자 메일 수에 대 한 세부 정보를 제공 하 고 edge 보호, 맬웨어 방지, 피싱 방지, 스팸 방지 및 스푸핑 방지를 포함 하 여이 수에 영향을 주는 구성 된 위협 방지 기능에 대해 설명 합니다.

기본적으로 **깔때기** 탭을 클릭 하면 다음 필터를 사용 하 여 구성 된 차트 및 데이터 테이블이 보기에 포함 됩니다.
...
- **방향** :
  ...
  - **인바운드**
  - **아웃 바운드**
  - **조직 내** :이 수는 테 넌 트 내에서 전송 되는 메시지에 대 한 것입니다. 즉, 보낸 사람 abc@domain.com는 받는 사람 xyz@domain.com에 게 전송 합니다 (인바운드 및 아웃 바운드와 별개로 계산 됨).

집계 보기 및 데이터 테이블 보기는 90 일의 필터링을 허용 합니다.

**필터** 를 클릭 하면 차트와 데이터 테이블을 모두 필터링 할 수 있습니다.

이 차트에는 다음으로 구성 된 전자 메일 수가 표시 됩니다.

- **총 전자 메일**
- **Edge 보호 후의 전자 메일**
- **맬웨어 방지, 파일 신뢰도, 파일 형식 블록 후의 전자 메일**
- **피싱, URL 신뢰도, 브랜드 가장을 사용한 후의 전자 메일-스푸핑 방지**
- **스팸 방지, 대량 메일 필터링 후의 전자 메일**
- **사용자 및 도메인 가장 1 다음의 전자 메일**<sup>1</sup>
- **파일 및 URL 샌드 박싱 1 다음에 전자 메일 보내기**<sup>1</sup>
- **배달 후 보호를 수행한 후 전자 메일이 심각 하지 않음으로 감지 됨 (URL 클릭 시간 보호)**

<sup>1</sup> Defender for Office 365 전용

EOP 또는 Office 365 용 Defender에서 필터링 한 전자 메일을 별도로 보려면 차트 범례의 값을 클릭 합니다.

데이터 테이블에는 다음과 같은 정보가 내림차순으로 표시 됩니다.

- **날짜**
- **총 전자 메일**
- **에 지 보호** 
- **맬웨어 방지, 파일 신뢰도, 파일 형식 블록** :
  - **파일 신뢰도** : 다른 Microsoft 고객이 첨부 한 파일의 식별으로 인해 필터링 된 메시지입니다.
  - **파일 형식 블록** : 메시지에서 식별 된 악의적인 파일의 유형으로 인해 필터가 적용 되었습니다.      
- **앤티 피싱, URL 신뢰도, 브랜드 가장, 스푸핑 방지** :
  - **Url 신뢰도** : 다른 Microsoft 고객의 url 식별으로 인해 필터링 된 메시지입니다.
  - **브랜드 가장** : 보낸 사람이 가장 잘 알려진 브랜드에서 오는 메시지로 인해 필터링 된 메시지입니다.
  - **스푸핑 방지** : 받는 사람이 속한 도메인을 스푸핑 하는 메시지 또는 메시지 보낸 사람이 소유 하지 않은 도메인을 스푸핑할 때 메시지가 필터링 되었습니다.  
- **스팸 방지, 대량 메일 필터링** :
  - **대량 메일 필터링** : 받는 사람에 게 대량 메일을 배달 하는 시도로 인해 필터링 된 메시지입니다. 
- **사용자 및 도메인 가장 (Defender For Office 365)** :
  - **사용자 가장** : 피싱 방지 정책의 가장 보호 설정에 정의 되어 있는 사용자 (메시지 보낸 사람)를 가장 하는 시도로 인해 필터링 된 메시지입니다.
  - **도메인 가장** : 피싱 방지 정책의 가장 보호 설정에서 정의한 도메인을 가장 하는 시도로 인해 필터링 된 메시지입니다. 
- **파일 및 URL 샌드 박싱 (Defender For Office 365)** :
  - **파일 샌드 박싱** : 안전한 첨부 파일 정책에 따라 필터링 된 메시지입니다.
  - **URL 샌드 박싱** : 안전한 링크 정책에 따라 필터링 된 메시지입니다.  
- **전송 이후 ATP (보호 및 zap) 또는 zap (EOP)** : zap은 0 시간 자동 제거를 나타냅니다.

데이터 테이블에서 행을 선택 하면 전자 메일 개수에 대 한 자세한 분석 내용이 플라이 아웃에 표시 됩니다.

**내보내기** :

**옵션** 에서 **내보내기를** 클릭 한 후에는 다음 값 중 하나를 선택할 수 있습니다.

- **요약 (가장 최근 90 일 동안의 데이터 포함)**
- **세부 정보 (최대 30 일 동안의 데이터 포함)**

**날짜** 에서 범위를 선택 하 고 **적용** 을 클릭 합니다. 현재 필터의 데이터를 .csv 파일로 내보냅니다.

내보낸 각 .csv 파일은 15만 행으로 제한 됩니다. 데이터에 포함 된 행이 15만 개 보다 많은 경우 여러 .csv 파일이 만들어집니다.

 ![메일 흐름 상황 보고서의 깔때기 보기 ](../../media/mail-flow-status-report-funnel-view.png)

### <a name="tech-view-for-the-mailflow-status-report"></a>메일 흐름 상태 보고서에 대 한 기술 보기

**기술 보기** 는 **깔때기형** 보기와 비슷하며 구성 된 위협 보호 기능에 대 한 세부 정보를 제공 합니다. 차트에서 메시지가 위협 방지의 각 단계에서 분류 되는 방식을 확인할 수 있습니다.

**기술 보기** 탭을 클릭 하면 기본적으로이 보기에 다음 필터를 사용 하 여 구성 된 차트 및 데이터 테이블이 포함 됩니다.
...
- **방향** :
  ...
  - **인바운드**
  - **아웃 바운드**
  - **조직 내** :이 수는 테 넌 트 내의 메시지에 대 한 것입니다. 보낸 사람 abc@domain.com이 받는 사람 xyz@domain.com에 게 전송 합니다 (인바운드 및 아웃 바운드와 별도로 계산 됨).
...
- **총 전자 메일**
- **에 지 허용,에 지 필터링 됨**
- **맬웨어 아님, 안전한 첨부 파일 감지 (Defender for Office 365), 맬웨어 방지 엔진 검색, 규칙 블록**
- **피싱, DMARC 오류, 가장 감지, 스푸핑 감지, 피싱 검색**
- **URL 샌드 박싱, URL 샌드 박싱 검색 (Office 365 용 Defender)을 사용 하 여 검색 하지 않음**
- **스팸 아님, 스팸**
- **악의적이 아닌 전자 메일, 안전한 링크 검색 (Office 365 용 Defender), ZAP**

차트의 범주 위에 마우스를 올리면 해당 범주의 메시지 수를 볼 수 있습니다.
...
- **총 전자 메일**
- **에 지 필터링 됨**
- **맬웨어 방지 엔진, 안전한 첨부 파일, 규칙 필터링** 됨:
  - **규칙 필터링** 됨: 메일 흐름 규칙 (전송 규칙이 라고도 함)으로 인해 필터링 된 메시지입니다.
- **DMARC, 가장, 스푸핑, 피싱 필터링** 됨:
  - **DMARC** : 해당 DMARC 인증 확인을 실패 한 메시지로 인해 필터링 된 메시지입니다. 
- **URL 샌드 박싱 검색**
- **스팸 방지 필터링**
- **제거 된 ZAP**
- **안전한 링크를 통한 검색**
...
**내보내기** 를 클릭 하면 **옵션** 에서 다음 값 중 하나를 선택할 수 있습니다.
...
내보낸 각 .csv 파일은 15만 행으로 제한 됩니다. 데이터에 포함 된 행이 15만 개 보다 많은 경우 여러 .csv 파일이 만들어집니다.
 ...
 ![메일 흐름 상황 보고서의 기술 보기 ](../../media/mail-flow-status-report-Tech-view.png)
...
- **아래로 나누기: Type** : 다음은 사용 가능한 모든 종류를 표시 하는 차트입니다.
  ...
  - **스팸 감지**
  - **규칙 메시지**
  - **고급 맬웨어** (Office 365 용 Microsoft Defender)
...
- **상위 맬웨어 받는 사람에 대 한 데이터 표시 \> (Office 365 용 Defender)** 
...
- **상위 맬웨어 받는 사람에 대 한 데이터 표시 \> (Office 365 용 Defender)** 
  ...
  - **상위 맬웨어 받는 사람 (Office 365 용 Defender)**
  - **개수**
