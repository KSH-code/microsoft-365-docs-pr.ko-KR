...
1. **개체 유형** 섹션에서 **전자 메일** 을 선택 합니다. **전송 형식** 섹션에서 다음 옵션 중 하나를 사용 합니다.
   ...
   > [!NOTE]
   > Defender for Office 365 계획 1 또는 요금제 2를 포함 하는 관리자는 30 일 동안 오래 된 메시지를 제출할 수 있습니다. 다른 관리자는 7 일 전으로 진행할 수 있습니다.
...
#### <a name="admin-submission-rescan-details"></a>관리자 전송 세부 정보 다시 검사

관리자 전송에 제출 된 메시지는 다시 검색 되며 세부 정보 플라이 아웃에 표시 되는 결과입니다.

- 배달 시 보낸 사람의 전자 메일 인증에 오류가 발생 한 경우
- 메시지의 결과에 영향을 있거나 재정의 했을 수 있는 모든 정책 방문에 대 한 정보입니다.
- Current 샌드 박싱-메시지에 포함 된 Url 이나 파일이 악의적 인지 여부를 확인 합니다.
- Graders의 피드백

재정의가 발견 되 면 몇 분 후에 다시 검사를 완료 해야 합니다. 전자 메일 인증에 문제가 없거나 재정의로 인해 배달 되지 않은 경우 graders의 피드백은 하루까지 소요 될 수 있습니다.
