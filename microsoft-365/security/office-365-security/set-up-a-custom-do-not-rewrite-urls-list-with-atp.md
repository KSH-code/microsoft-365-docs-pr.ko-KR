...
- ATP 안전한 링크 정책에 대해 "다시 쓰지 않음" 목록을 지정 하는 경우 최대 3 개의 와일드 카드 별표 (\*)를 포함할 수 있습니다. 와일드 카드\*()는 or `https://` `https://`와 같은 접두사 또는 하위 도메인을 명시적으로 포함 하는 데 사용 됩니다. 같은 항목 (예: `contoso.com` "다시 쓰지 않음" `*.contoso.com/*` 목록)과는 다릅니다. 사용자가 도메인과 `*.contoso.com/*` 해당 하위 도메인 및 경로를 방문 하도록 허용 하려면 다음을 수행 해야 합니다.
...
|`*.contoso.com/*`  <br/> |받는 사람이,, 또는와 `https://www.contoso.com` `https://www.contoso.com` `https://maps.contoso.com`같은 도메인, 하위 도메인과 경로를 방문할 수 있습니다.`https://www.contoso.com/a`  <br/> |
|`https://contoso.com/a`  <br/> |특정 받는 사람이 같은 사이트를 방문 하 `https://contoso.com/a`되 하위 경로는 볼 수 없습니다.`https://contoso.com/a/b`  <br/> |
|`https://contoso.com/a/*`  <br/> |특정 받는 사람이 같은 사이트 `https://contoso.com/a` 와 같은 하위 경로를 방문할 수 있도록 허용`https://contoso.com/a/b`  <br/> |
