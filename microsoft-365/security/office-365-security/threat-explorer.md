...
> [!TIP]
> Network Message ID는 네트워크 메시지 ID를 통해 탐색기나 관련 타사 도구를 검색할 때 클릭을 특정 메일에 다시 매핑합니다. 네트워크 메시지 ID를 통해 검색 하면 관리자가 클릭 결과와 연결 된 특정 전자 메일을 제공 합니다. 내보낼 때 네트워크 메시지 ID와의 관계를 일치 시키는 것을 통해 더 빠르고 강력한 분석이 가능 합니다.
