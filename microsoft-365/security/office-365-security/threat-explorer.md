...
- URL 필터(URL 및 *URL*  도메인과 URL 도메인 및 *경로)에* 대한 수정 : 업데이트는 URL/클릭 결과를 포함하는 메시지 검색에 영향을 미치게 됩니다. 프로토콜에 무관한 검색에 대한 지원을 사용하도록 설정했습니다. 따라서 를 사용하지 않고도 URL을 검색할 수 `http` 있습니다. 다른 값이 명시적으로 지정되지 않은 경우 기본적으로 URL 검색은 http에 매핑됩니다. 예시:
...
**배달 위치는** 배달 후를 실행한 정책 및 검색의 결과를 보여줍니다. 배달 동작에 **_연결됩니다._** 다음은 가능한 값입니다.
