...
대부분의 보안 조치를 우회 하므로 허용 목록은 항상 제한 해야 합니다. 무시할 수 있는 사용자를 확인 하려면 표준 유지 관리의 일부로 모든 허용 목록을 다시 검사 해야 합니다. 가능한 경우 제한적인 메일 흐름 규칙을 사용 하는 것이 좋습니다.
...
## <a name="using-exchange-mail-flow-rules-to-allow-specific-senders-recommended"></a>Exchange 메일 흐름 규칙을 사용 하 여 특정 보낸 사람 허용 (권장)
...
2. 규칙이 수행 하는 작업을 표시 하는 X-헤더를 추가 합니다. 아래 예제에서는 간단한 머리글 `X-ETR: Bypass spam filtering for authenticated sender 'contoso.com'`을 추가할 수 있습니다. 이 규칙에 도메인이 두 개 이상 있는 경우 머리글 텍스트를 적절 하 게 변경할 수 있습니다. **메일 흐름 규칙으로 인해 메시지의 필터링이 생략 되는 경우, 스팸 방지-Report header** (**IP 허용 목록에 있는 경우**)는이를 스탬프 처리 하 고 IPV: CAL에도 스탬프 처리 합니다. 이는 문제 해결을 지원 합니다.
...
자세한 내용은 [메일 흐름 규칙을 사용 하 여 메시지의 SCL 설정을](use-mail-flow-rules-to-set-the-spam-confidence-level-scl-in-messages.md)참조 하십시오.
...
메일 흐름 규칙을 사용 하 여 보낸 사람 인증의 유효성을 검사 하는 동안 특정 보낸 사람에 게 전역적으로 허용 하거나 도메인과 IP를 함께 연결할 수 없는 경우에는 *스팸 방지 정책 IP 허용 목록*에 보낸 사람을 추가 하는 것이 가장 좋습니다. [연결 필터 정책 구성](configure-the-connection-filter-policy.md)에서 자세한 단계를 확인할 수 있습니다. 허용 되는 IP 주소 수를 최소로 유지 하는 것이 중요 하므로 전체 IP 주소 범위를 사용 하지 않는 것이 좋습니다. 또한 소비자 서비스 또는 공유 인프라에 속하는 IP 주소 범위를 추가 해서는 안 되며, 허용 되는 IP 주소 목록을 정기적으로 검토 하 고 더 이상 필요 하지 *않은 항목은* 제거 해야 합니다.
...
> [!IMPORTANT]
> • *보낸 사람/허용 도메인을 허용* 하도록 스팸 방지 정책을 구성 하면 허용 목록의 보낸 사람 또는 b)가 허용 되는 도메인의 보낸 사람 으로부터 메시지를 건너뛰는 메시지가 반환 됩니다. 이 방법을 통해 스팸 발송자가 보내는 도메인을 스푸핑할 수 있는 위험을 크게 증가 시키며, 모든 스팸 필터링, 보낸 사람 인증 검사를 건너뛰고 메시지를 사용자의 받은 편지 함으로 직접 전송 하는 전체 전자 메일 주소를 가장 합니다. <br/><br/>• 소유 하는 도메인 또는 인기 도메인 (예: `microsoft.com`)을 메일 흐름 규칙의 조건으로 추가 하지 않습니다. 이 방법은 잘못 된 행위자가 메일을 보내 위험을 높일 수 있는 기회를 만들기 때문에 높은 위험으로 간주 됩니다. <br/><br/>• **차단 된 보낸 사람 목록을** 만드는 방법에 대 한 정보가 [여기](create-block-sender-lists-in-office-365.md)에 표시 됩니다.
