...
보안 &  준수 센터의 스푸핑 인텔리전스 인사이트를 사용하여 합법적으로 허용되지 않은 전자 메일(SPF, DKIM 또는 DMARC 검사를 통과하지 않은 도메인의 메시지)을 보내는 스푸핑된 보낸 사람(SPF, DKIM 또는 DMARC 확인을 통과하지 않은 도메인의 메시지)을 빠르게 식별하고 해당 보낸 사람이 이를 수동으로 허용할 수 있습니다.
...
이 문서의 나머지부분에서는 보안 & 준수 센터 및 PowerShell(Microsoft 365 사서함이 있는 Microsoft 365 조직의 경우 Exchange Online PowerShell, Exchange Online 사서함이 없는 조직을 위한 독립 실행형 EOP PowerShell)Exchange Online 에서 스푸핑 인텔리전스 정보를 사용하는 방법에 대해 설명하고 있습니다.
...
2. 주 피싱 **방지 페이지에서** 스푸핑 인텔리전스 인사이트에는 다음 두 가지 모드가 있습니다.
...
스푸핑 인텔리전스  인사이트에서 의심스러운  도메인 또는 의심스러운 도메인을 클릭하여 스푸핑 인텔리전스 인사이트 **페이지로** 이동합니다. 페이지에는 다음 정보가 포함되어 있습니다.

- **스푸핑된 도메인**: 전자 메일 클라이언트의 시작 상자에  표시되는 스푸핑된 사용자의 도메인입니다. 이 주소를 `5322.From` 주소라고도 합니다.
- **인프라:** 보내는 _인프라라고도 합니다._ 이 값은 다음 값 중 하나입니다.
...
- **스푸핑** 허용 : 이 값은 사용자가 통찰력에서  의심스러운 도메인을 클릭하는지 의심스러운 도메인을 클릭하는지 여부에 따라 결정됩니다. 
  - **의심스러운** 도메인 : **스푸핑** 허용 값은 항상 **아니요입니다.** 스푸핑된 도메인과 보내는 인프라의  조합에서 보낸 메시지는 스푸핑 인텔리전스에 의해 잘못된 것으로 표시됩니다. 스푸핑된 메시지에 대해 수행되는 작업은 기본 피싱 방지 정책 또는 사용자 지정 피싱 방지 정책(기본값은 정크 메일 폴더로 메시지 이동)에 의해 **제어됩니다.** 자세한 내용은 [Microsoft Defender에서](configure-atp-anti-phishing-policies.md)피싱 방지 정책 구성을 Office 365.
  - **의심스러운** 도메인이 아닌 도메인: **스푸핑** 허용 값은 항상 **예입니다.** 스푸핑된 도메인과 보내는 인프라의  조합에서 보낸 메시지는 스푸핑 인텔리전스로 잘 표시됩니다.

선택한 열 제목을 클릭하여 결과를 정렬할 수 있습니다.

결과를 필터링하기 위해 다음 옵션을 사용할 수 있습니다.

- **스푸핑된** 도메인 필터링 상자를 사용하여 결과를 필터링하기 위해 COMMA로 구분된 스푸핑된 도메인 값 목록을 입력합니다.
- 필터 인프라 **상자를** 사용하여 결과를 필터링하기 위해 인프라 값의 콤보로 구분된 목록을 입력합니다.
- 필터 **단추를** 클릭하여 스푸핑 유형별로 결과를 **필터링합니다.**

### <a name="view-details-about-spoofed-messages"></a>스푸핑된 메시지에 대한 세부 정보 보기

목록에서 항목을 선택하여 세부 정보를 볼 수 있습니다. 플라이아웃은 다음과 같은 정보와 기능과 함께 표시됩니다.

- **스푸핑 허용:** 이 토글을 사용하여 스푸핑 인텔리전스 판정을 능가합니다.
  - 원래 통찰력에서  의심스러운 도메인을 선택한 경우 **스푸핑** 허용이 해제됩니다. 스푸핑 인텔리전스 인사이트에서 테넌트 허용/차단 목록으로 항목을 스푸핑 허용 항목으로 이동하는 기능을 켜기 위해 토글을 켜기:으로 ![ 전환합니다. 토글 ](../../media/scc-toggle-on.png)
  - 원래 통찰력에서  의심스러우지 않는 도메인을 선택한 경우 **스푸핑** 허용이 켜져 있습니다. 스푸핑 인텔리전스 인사이트에서 테넌트 허용/차단 목록으로 항목을 스푸핑에 대한 차단 항목으로 이동하는 기능을 끄기 위해 토글을 해제: ![ 토글 끄기 ](../../media/scc-toggle-off.png)
...
스푸핑 인텔리전스 인사이트에서 의심하지 않는 도메인의 허용된 스푸핑된 보낸 사람  또는 사용자가 수동으로 스푸핑 허용으로  변경한 의심스러운 도메인의 차단된 보낸 사람으로, 스푸핑된 도메인과 보내는 인프라의 조합에서 보낸 메시지만 허용합니다.   모든 원본에서 스푸핑된 도메인의 전자 메일을 허용하지 않으며 도메인에 대한 보내는 인프라의 전자 메일을 허용하지 않습니다.
