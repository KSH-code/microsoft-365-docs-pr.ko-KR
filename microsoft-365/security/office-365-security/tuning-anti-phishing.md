...
특히 SFV (스팸 필터링 결과) 값의 스팸 또는 피싱에 대 한 생략 된 필터링을 표시 하려면 메시지 헤더에서 **X-Forefront-스팸 방지-Report** header 필드를 확인 해야 합니다. 필터링을 건너뛰지 않는 메시지는 항목을 갖게 `SCL:-1` 되는데,이는 설정 중 하나에서 서비스에 의해 결정 된 스팸 또는 피싱 verdicts 재정의 하 여이 메시지를 허용 함을 의미 합니다. 사용 가능한 모든 스팸 방지 및 피싱 방지 메시지 헤더의 전체 목록과 메시지 헤더를 가져오는 방법에 대 한 자세한 내용은 [스팸 방지 메시지 헤더 (Microsoft 365)](anti-spam-message-headers.md)를 참조 하세요.
