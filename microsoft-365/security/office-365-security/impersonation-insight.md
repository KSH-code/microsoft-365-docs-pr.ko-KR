...
가장 정보를 사용하여 가장 보호를 위해 구성한 가장된 보낸 사람 또는 보낸 사람 도메인의 메시지를 빠르게 식별할 수 있습니다.

## <a name="what-do-you-need-to-know-before-you-begin"></a>시작하기 전에 알아야 할 내용은 무엇인가요?

- <https://protection.office.com/>에서 보안 및 준수 센터를 엽니다. 피싱 방지 페이지에서 가장 인사이트로 직접 이동하기 위해 를  사용하세요. <https://protection.office.com/antiphishing>
...
## <a name="open-the-impersonation-insight-in-the-security--compliance-center"></a>보안 및 준수 센터에서 가장 & 열기

1. 보안 및 & 센터에서 위협 **관리** 정책 피싱 방지 로 이동 \>  \> **합니다.**

2. 기본 **피싱** 방지 페이지에서 가장 인사이트는 다음과 같습니다.

   이 인사이트에는 두 가지 모드가 있습니다.

    - **인사이트** 모드: 피싱 방지 정책에서 가장 보호를 사용하도록 설정하고 구성하는 경우 인사이트는 지난 7일 동안 가장된 보낸 사람이 검색한 메시지 수를 보여줍니다. 이는 모든 피싱 방지 정책에서 검색된 모든 가장된 보낸 사람의 합계입니다.
    - **모드인 경우:** 활성 피싱 방지 정책에서 가장 보호를 사용하도록 설정하고 구성하지 않은 경우 인사이트는 지난 7일 동안 가장 보호 기능에서 검색된 메시지의 수를 보여줍니다. 

   어느 경우든 **가장된** 도메인은 보호된 도메인의 보낸 사람이 보낸 메시지 수를 표시하고, 가장된 사용자는 보호된 사용자의 메시지 수를 보여 주며, 

## <a name="view-information-about-messages-from-senders-in-impersonated-domains"></a>가장된 도메인의 보낸 사람이 보낸 메시지에 대한 정보 보기

가장 인사이트에서 가장된 **도메인을 클릭합니다.** 열 **수 있는** 가장 인사이트 페이지에는 다음 정보가 포함되어 있습니다.
...
- **가장 허용:** 다음 값 중 하나를 사용합니다.
  - **예:** 도메인이 스팸 방지 정책에서 신뢰할 수 있는 도메인(가장 보호에 대한 예외)으로 구성되었습니다. 가장된 도메인의 보낸 사람이 보낸 메시지가 검색되지만 허용되었습니다.
  - **No:** 도메인이 스팸 방지 정책에서 가장 보호를 위해 구성되었습니다. 가장된 도메인의 보낸 사람이 보낸 메시지는 스팸 방지 정책에서 가장된 도메인에 대한 작업을 기반으로 검색되고 처리되었습니다.

선택한 열 제목을 클릭하여 결과를 정렬할 수 있습니다.

결과를 필터링하려면 필터 도메인  상자를 사용하여 콤보로 구분된 값 목록을 입력하여 결과를 필터링할 수 있습니다.

### <a name="view-details-about-messages-from-senders-in-impersonated-domains"></a>가장된 도메인의 보낸 사람이 보낸 메시지에 대한 세부 정보 보기

가장 **인사이트 페이지에서** 사용 가능한 행 중 하나를 선택합니다. 나타나는 세부 정보 플라이아웃에는 다음 정보와 기능이 포함되어 있습니다.
...
가장 인사이트에서 **가장된 사용자를 클릭합니다.** 열 **수 있는** 가장 인사이트 페이지에는 다음 정보가 포함되어 있습니다.
...
- **가장 허용:** 다음 값 중 하나를 사용합니다.
  - **예:** 보낸 사람이 스팸 방지 정책에서 신뢰할 수 있는 사용자(가장 보호에 대한 예외)로 구성되었습니다. 가장된 보낸 사람이 보낸 메시지가 검색되지만 허용되었습니다.
  - **No:** 보낸 사람이 스팸 방지 정책에서 가장 보호를 위해 구성되었습니다. 스팸 방지 정책에서 가장된 사용자에 대한 작업을 기반으로 가장된 보낸 사람이 보낸 메시지가 검색되고 처리되었습니다.
