...
모든 Microsoft 365 조직에서 EOP의 스팸 방지 정책에 있는 ASF(고급 스팸 필터) 설정을 사용하면 관리자가 특정 메시지 속성에 따라 메시지를 스팸으로 표시하도록 할 수 있습니다. ASF는 이러한 속성이 스팸에서 일반적으로 발견되는 것이 흔하기 때문에 이러한 속성을 구체적으로 대상으로 합니다. 속성에 따라 ASF 검색은 메시지를 스팸 또는  높은 신뢰도 스팸으로 **표시합니다.**
