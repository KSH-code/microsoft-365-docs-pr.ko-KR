...
사용을 요구하기 전에 사용자가 MFA에 등록하도록 해야 합니다. E5 보안 추가 Microsoft 365 E5 Microsoft 365 E3, EMS E5를 사용하는 Office 365 또는 개별 Azure AD Premium P2 라이선스가 있는 경우 Azure AD ID 보호와 함께 MFA 등록 정책을 사용하여 사용자가 MFA를 등록할 수 있도록 할 수 있습니다. 선행 [작업으로는](identity-access-prerequisites.md) 모든 사용자를 MFA에 등록하는 것이 포함됩니다.
...
- **Enterprise(수준** 1)를 사용하면 앱이 PIN으로 보호되고 암호화되고 선택적 지우기 작업을 수행할 수 있습니다. Android 장치의 경우 이 수준은 Android 장치 인증의 유효성을 검사합니다. 이는 사서함 정책에서 유사한 데이터 보호 제어를 Exchange Online IT 및 사용자 모집단을 APP에 도입하는 진입 수준 구성입니다.
- **Enterprise(수준** 2)에는 APP 데이터 누출 방지 메커니즘 및 최소 OS 요구 사항이 도입되었습니다. 이 구성은 직장 또는 학교 데이터에 액세스하는 대부분의 모바일 사용자에게 해당됩니다.
- **Enterprise(수준** 3)에는 고급 데이터 보호 메커니즘, 향상된 PIN 구성 및 APP Mobile Threat Defense가 도입되었습니다. 이 구성은 고위험 데이터에 액세스하는 사용자에게 바람직합니다.
...
승인된 앱 및 앱 보호가 필요한 조건부 액세스 정책을 만들려면 시나리오 [1 Microsoft 365: Microsoft 365](/azure/active-directory/conditional-access/app-protection-based-conditional-access#scenario-1-office-365-apps-require-approved-apps-with-app-protection-policies)Outlook 앱에 iOS 및 Android용 앱을 허용하지만 OAuth를 사용할 수 있는 앱 보호 정책을 사용하여 승인된 앱이 필요하지만 Exchange ActiveSync 클라이언트가 앱에 연결할 수 Exchange Online.
...
7. 포함에서 **앱** 선택을 선택하고 > 를 선택한 다음 클라우드 앱 목록에서 원하는 **앱을** 선택합니다.  예를 들어 선택을 Exchange Online. 완료되면 **선택을** 선택합니다.
