...
4. 결과를 필터링하려면 **필터**를 클릭합니다. 사용 가능한 필터:
   ...
   - **격리 이유**:
     ...
     - **대량 전자 메일**
     - **피싱**: 스팸 필터 결과 **피싱 전자 메일** 또는 피싱 방지 보호 기능이 메시지를 격리 했습니다 ([스푸핑 설정](set-up-anti-phishing-policies.md#spoof-settings) 또는 [가장 보호](set-up-anti-phishing-policies.md#impersonation-settings-in-atp-anti-phishing-policies)).
     - **맬웨어**
     - **스팸**
     - **높은 신뢰도 피싱**
...
ATP가 있는 조직에서는 관리자가 SharePoint Online, 비즈니스용 OneDrive 및 Microsoft 팀에서 격리 된 파일을 관리할 수 있습니다. 이러한 파일에 대 한 보호를 사용 하도록 설정 하려면 [SharePoint, OneDrive 및 Microsoft 팀에 대 한 ATP 끄기를](turn-on-atp-for-spo-odb-and-teams.md)참조 하세요.
