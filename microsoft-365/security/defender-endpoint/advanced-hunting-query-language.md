...
고급 헌팅은 [Kusto 쿼리 언어](https://docs.microsoft.com/azure/kusto/query/)를 기반으로 합니다. Kusto 연산자 및 문을 사용하여 특수한 Schema에서 정보를 찾는 [쿼리를 구성할 수 있습니다.](advanced-hunting-schema-reference.md) 이러한 개념을 보다 잘 이해하려면 첫 번째 쿼리를 실행합니다.

## <a name="try-your-first-query"></a>첫 번째 쿼리 시도하기

Microsoft Defender 보안 센터에서 고급 헌팅으로 **이동하여** 첫 번째 쿼리를 실행합니다. 다음 예제를 사용합니다.
...
이러한 연산자의 라이브 예제를 확인하기 위해  고급 헌팅 페이지의 시작 섹션에서 연산자를 실행합니다.
...
| 데이터 형식 | 설명 및 쿼리 의미 |
|--|--|
| `datetime` | 일반적으로 이벤트 타임스탬프를 나타내는 데이터 및 시간 정보입니다. [지원되는 datetime 형식 참조](https://docs.microsoft.com/azure/data-explorer/kusto/query/scalar-data-types/datetime) |
| `string` | UTF-8의 문자열은 단일 따옴표( ) 또는 `'` 따옴표()로 `"` 묶입니다. [문자열에 대한 자세한 내용은](https://docs.microsoft.com/azure/data-explorer/kusto/query/scalar-data-types/string) |
| `bool` | 이 데이터 형식은 지원 `true` 또는 `false` 상태입니다. [지원되는 리터럴 및 연산자 참조](https://docs.microsoft.com/azure/data-explorer/kusto/query/scalar-data-types/bool) |
| `int` | 32비트 정수  |
| `long` | 64비트 정수 |

이러한 데이터 형식에 대한 자세한 내용은 Kusto 스칼라 데이터 형식 [을 읽어 보아야 합니다.](https://docs.microsoft.com/azure/data-explorer/kusto/query/scalar-data-types/)
...
- **[Schema reference](advanced-hunting-schema-reference.md#get-schema-information-in-the-security-center)**—테이블 및 열 설명과 함께 포털 내 참조와 지원되는 이벤트 유형(값) `ActionType` 및 샘플 쿼리
...
![쿼리가 여러 개 있는 고급 헌팅 쿼리 편집기 이미지 ](images/ah-multi-query.png)
...
## <a name="access-comprehensive-query-language-reference"></a>포괄적인 쿼리 언어 참조 액세스

쿼리 언어에 대한 자세한 내용은 [Kusto 쿼리 언어 설명서를 참조하세요.](https://docs.microsoft.com/azure/kusto/query/)
