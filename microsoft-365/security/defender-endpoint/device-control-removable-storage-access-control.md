...
### <a name="property-name-group-id"></a>속성 이름: 그룹 ID

**설명:** 고유한 ID인 GUID는 그룹을 나타내며 정책에 사용됩니다.

### <a name="property-name-descriptoridlist"></a>속성 이름: DescriptorIdList

**설명:** 그룹에서 다루는 데 사용할 장치 속성을 나열합니다.
...
- DeviceId
- HardwareId
- InstancePathId: InstancePathId는 시스템에서 장치를 고유하게 식별하는 문자열입니다(예: USBSTOR\DISK&VEN_GENERIC&PROD_FLASH_DISK&REV_8.07\8735B611&).

끝에 있는 번호(예:&**0)는** 사용 가능한 슬롯을 나타내며 디바이스에서 장치로 변경될 수 있습니다. 최상의 결과를 얻기 위해 끝에 와일드카드를 사용 합니다. 예를 들면 `USBSTOR\DISK&VEN_GENERIC&PROD_FLASH_DISK&REV_8.07\8735B611*`와 같습니다.
...
- PID
- VID_PID
  - 0751_55E0: 이 정확한 VID/PID 쌍과 일치
  - _55E0: PID=55E0과 모든 미디어 일치
  - 0751_: VID=0751과 모든 미디어 일치

### <a name="property-name-matchtype"></a>속성 이름: MatchType

**설명:** DescriptorIDList에서 여러 장치 속성이 사용되는 경우 MatchType은 관계를 정의합니다.
...
### <a name="property-name-policyruleid"></a>속성 이름: PolicyRuleId

**설명:** 고유한 ID인 GUID는 정책을 나타내며 보고 및 문제 해결에 사용됩니다.

### <a name="property-name-includedidlist"></a>속성 이름: IncludedIdList

**설명:** 정책을 적용할 그룹입니다. 여러 그룹이 추가된 경우 정책은 해당 그룹의 모든 미디어에 적용됩니다.

**옵션** 이 인스턴스에서는 그룹 ID/GUID를 사용해야 합니다.

다음 예에서는 GroupID의 사용을 보여 줍니다.

`<IncludedIdList> <GroupId>{EAA4CCE5-F6C9-4760-8BAD-FDCC76A2ACA1}</GroupId> </IncludedIdList>`

### <a name="property-name-excludedidlist"></a>속성 이름: ExcludedIDList

**설명:** 정책이 적용되지 않을 그룹입니다.

**옵션:** 이 인스턴스에서 그룹 ID/GUID를 사용해야 합니다.

### <a name="property-name-entry-id"></a>속성 이름: 항목 ID

**설명:** 하나의 PolicyRule에 여러 항목이 있을 수 있습니다. 고유한 GUID가 있는 각 항목은 장치 제어에 한 가지 제한을 알 수 있습니다.

### <a name="property-name-type"></a>속성 이름: Type

**설명:** IncludedIDList에서 이동식 저장소 그룹에 대한 작업을 정의합니다.

- 적용: 허용 또는 거부
- 감사: AuditAllowed 또는 AuditDenied

**옵션**:

- 허용
- 거부
- AuditAllowed: 액세스가 허용되는 경우 알림 및 이벤트를 정의합니다.
...
### <a name="property-name-options"></a>속성 이름: 옵션

**설명:** 알림을 표시할지 여부를 정의합니다.
...
- 0: nothing
- 4: 이 항목에 **대해 AuditAllowed** 및 **AuditDenied를** 사용하지 않도록 설정 **차단이 발생하고** **AuditDenied가 구성된** 경우에도 시스템에 알림이 표시되지 않습니다.

**AuditAllowed 또는** **AuditDenied** 유형이 선택된 경우:

- 0: nothing
- 1: 알림 표시, AuditDenied에만 작동
- 2: 이벤트 보내기
- 3: 알림을 표시하고 이벤트를 전송합니다. AuditAllowed에 적용하는 경우 보고에 대한 이벤트만 발생하지만 알림은 표시하지 않습니다.

### <a name="property-name-sid"></a>속성 이름: Sid

**설명:** 특정 사용자 또는 사용자 그룹에 이 정책을 적용할지 여부를 정의합니다. 하나의 항목은 최대 하나의 SID를 사용할 수 있으며 SID가 없는 항목은 컴퓨터 위에 정책을 적용하는 것입니다.

### <a name="property-name-computersid"></a>속성 이름: ComputerSid

**설명:** 특정 컴퓨터 또는 컴퓨터 그룹에 이 정책을 적용할지 여부를 정의합니다. 하나의 항목은 최대 하나의 ComputerSID를 사용할 수 있으며 ComputerSID가 없는 항목은 컴퓨터에 정책을 적용하는 것입니다. 특정 사용자 및 특정 컴퓨터에 항목을 적용하려면 동일한 항목에 SID와 ComputerSID를 모두 추가합니다.

### <a name="property-name-accessmask"></a>속성 이름: AccessMask
