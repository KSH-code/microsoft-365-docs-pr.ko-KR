...
## <a name="is-attack-surface-reduction-asr-part-of-windows"></a>ASR(공격 표면 축소) 부분이 Windows?
...
전체 ASR 규칙 및 기능 집합은 전체 ASR 규칙 및 기능 집합에 대한 엔터프라이즈 라이선스가 있는 Windows 10. 엔터프라이즈 라이선스 없이 제한된 수의 규칙이 작동할 수 있습니다. 이 Microsoft 365 Business 경우 Microsoft Defender 바이러스 백신 보안 솔루션으로 설정하고 PowerShell을 통해 규칙을 사용하도록 설정하십시오. 엔터프라이즈 라이선스 없이 ASR을 사용하는 것은 공식적으로 지원되지 않고 ASR의 모든 기능을 사용할 수 없습니다.
...
* [모든 Office 응용 프로그램에서 자식 프로세스를 만들지 차단](attack-surface-reduction.md#block-all-office-applications-from-creating-child-processes)
* [응용 Office 콘텐츠 만들기 차단](attack-surface-reduction.md#block-office-applications-from-creating-executable-content)
* [응용 Office 코드 삽입 차단](attack-surface-reduction.md#block-office-applications-from-injecting-code-into-other-processes)
...
* [매크로에서 Win32 API Office 차단](attack-surface-reduction.md#block-win32-api-calls-from-office-macros)
* [랜섬웨어에 대한 고급 보호 사용](attack-surface-reduction.md#use-advanced-protection-against-ransomware)
* [로컬 보안](attack-surface-reduction.md#block-credential-stealing-from-the-windows-local-security-authority-subsystem) 기관 하위 Windows(lsass.exe) 자격 증명 도용 차단
...
* [통신 Office 자식 프로세스를 만들지 차단](attack-surface-reduction.md#block-office-communication-application-from-creating-child-processes)
...
## <a name="i-applied-a-rule-using-gpo-now-when-i-try-to-check-the-indexing-options-for-the-rule-in-microsoft-outlook-i-get-a-message-stating-access-denied"></a>GPO를 사용하여 규칙을 적용한 경우 이제 Microsoft Outlook 규칙의 인덱싱 옵션을 확인하려고 할 때 '액세스 거부'를 표시하는 메시지가 표시됩니다.

인덱싱 옵션을 직접 열고 Windows 10.
...
## <a name="i-recently-enabled-the-asr-rule-block-credential-stealing-from-the-windows-local-security-authority-subsystem-lsassexe-and-i-am-getting-a-large-number-of-notifications-what-is-going-on"></a>최근에 ASR 규칙, Windows 로컬 보안 기관 하위 lsass.exe(인증 *기관)* 에서 자격 증명 도용 차단을 사용하도록 설정하고 많은 알림을 받고 있습니다. 무슨 일이 일어나고 있나요?
