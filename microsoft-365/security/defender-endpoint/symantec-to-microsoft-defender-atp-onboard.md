...
5. [Symantec을 제거합니다.](#uninstall-symantec)

6. [끝점용 Microsoft Defender가 제대로 작동하고 있는지 확인합니다.](#make-sure-microsoft-defender-for-endpoint-is-working-correctly)
...
|macOS:<p>11.3.1(Big Sur)<p>10.15(카탈로나)<p>10.14(모잡)     |에서 DIY 앱을 다운로드하여 [https://aka.ms/mdatpmacosdiy](https://aka.ms/mdatpmacosdiy) 사용하세요. <p>자세한 내용은 [macOS의 끝점용 Microsoft Defender를 참조하세요.](microsoft-defender-endpoint-mac.md)        |
...
|PowerShell | 1. Windows 디바이스에서 관리자 권한으로 Windows PowerShell 를 니다. <p>2. `Get-MpComputerStatus` cmdlet을 실행합니다. <p>3. 결과 목록에서 **AMRunningMode:** 수동 모드 또는 **AMRunningMode: SxS** 수동 모드 를 검색합니다. |
...
2. Symantec의 제거 암호를 삭제합니다.<br/>
   ...
   1. 사용자 Windows 레지스트리 편집기를 관리자 권한으로 여는 경우
...
3. 장치에서 Symantec을 제거합니다. 이에 대한 도움이 필요한 경우 Broadcom의 설명서를 참조하세요. 다음은 몇 가지 Broadcom 리소스입니다. 
   ...
   - Windows 장치: Endpoint Protection [14개 클라이언트를](https://knowledge.broadcom.com/external/article?articleId=170040) 수동으로 Windows
   ...
   - Linux 장치: Linux의 설치에 [Endpoint Protection 질문과 대답](https://knowledge.broadcom.com/external/article?articleId=162054)

## <a name="make-sure-microsoft-defender-for-endpoint-is-working-correctly"></a>끝점용 Microsoft Defender가 제대로 작동하고 있는지 확인

Symantec을 제거한 후 다음 단계는 끝점용 Defender가 제대로 작동하고 있는지를 확인합니다. 끝점용 Microsoft Defender 데모 시나리오 사이트()를 [https://demo.wd.microsoft.com](https://demo.wd.microsoft.com) 방문하세요. 다음을 포함하여 해당 페이지에서 하나 이상의 데모 시나리오를 시도해 봤습니다.
- 클라우드 제공 보호
