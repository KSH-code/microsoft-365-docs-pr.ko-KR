...
!['역할'을 클릭하고 일반 설정, 사용 권한, API 및 규칙이 포함된 설정 페이지를 열했습니다. 사용 권한을 연 다음 역할을 열 수 있습니다. 모든 역할 표시](../../media/converged-add-role-9.png)
...
보고서는 Microsoft 365 보안 센터에서도 통합됩니다. 관리자는 일반 보안 보고서로 시작하여 끝점, 전자 메일 및 공동 작업과 관련한 특정 보고서로 & 있습니다. 여기에 있는 링크는 작업 구성에 따라 동적으로 생성됩니다.

### <a name="quickly-view-your-microsoft-365-environment"></a>Microsoft 365 환경 빠르게 보기

홈 **페이지에는** 보안 팀에 필요한 많은 공통 카드가 표시됩니다. 카드와 데이터의 구성은 사용자 역할에 따라 달라집니다. Microsoft 365 보안 센터는 역할 기반 액세스 제어를 사용하기 때문에 다양한 역할에 매일 작업에서 더 의미 있는 카드가 표시될 것입니다.  
...
- 엔드포인트용 Microsoft Defender와 Office 365용 Microsoft Defender 사이의 데이터 범위가 개선되어 도메인 전체에서 인시던트 관리, 자동 조사, 수정 및 사전 또는 사후 위협 헌팅이 가능합니다. 
- 끝점용 Microsoft Defender에서 이미 사용할 수 있는 끝점 데이터 외에도 Office 365용 Microsoft Defender의 전자 메일 관련 검색 및 완화 기능
- 경고를 엔드포인트용 Microsoft Defender 및 Office 365용 Microsoft Defender에서 종합적인 공격 사례로 집계하여 작업 큐를 줄이고 조사를 단순화하고 빠르게 하는 위협 관련 인시던트 보기입니다.
- Microsoft 365 Defender 솔루션에서 공격 시도를 감지하고 차단합니다. 추가 노출 및 탄력성 증가 위험을 완화하는 예방 조치를 구동하는 데 사용할 수 있는 데이터도 있습니다. 
...
학습 허브 내에서 Email & Collaboration (Microsoft Defender for Office 365 or MDO) 지침은 Endpoint(Endpoint 또는 MDE용 Microsoft Defender) 및 Microsoft 365 Defender 학습 리소스와 함께 제공됩니다.

학습 허브는 "Microsoft 365 Defender를 사용하여 조사하는 방법"과 같은 주제에 대해 구성한 학습 경로로 열립니다. 및 "Office 365용 Microsoft Defender 모범 사례". 이 섹션은 현재 Microsoft 내부의 보안 제품 그룹에서 큐레이터합니다. 각 학습 경로는 개념을 통과하는 데 소요되는 시간을 반영합니다. 예를 들어 'Office 365용 Microsoft Defender 사용자 계정이 손상될 때 수행해야 하는 단계'는 8분 정도 소요될 것으로 예정된 것이고, 중요한 정보를 제공합니다.
...
3. 다음과 같은 두 가지 옵션이 있습니다.
    1. 아래로 스크롤하여 텍스트 상자 *Office 365에서* 규정 준수를 개선하거나 사용자를 보호하는 방법을 설명하고 *Microsoft 365 보안 센터에 붙여넣습니다.* 결과에서 사용자와 같은 아이디어를 검색하여 투표하거나 새 아이디어 게시 **단추를 사용할 수 있습니다.**
    1. 이 문제가 이미 보고된 것으로 생각되고 해당 프로필을 투표(또는 투표)로  올리고 싶은 경우 UserVoice의 오른쪽에 있는 피드백 보내기 상자를 사용하세요. Microsoft *365* 보안 센터를 검색하고, 문제를 **찾고,** 투표 단추를 사용하여 상태를 올렸다.
...
Microsoft 365 보안 센터의 기능을 계속 탐색합니다.
