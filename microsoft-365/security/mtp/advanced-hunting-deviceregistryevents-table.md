...
고급 `DeviceRegistryEvents` 헌팅 [키의](advanced-hunting-overview.md) 표에는 레지스트리 항목 생성 및 수정에 대한 정보가 포함되어 있습니다. 이 참조를 사용하여 이 표의 정보를 반환하는 쿼리를 생성합니다.
...
| `PreviousRegistryKey` | 문자열 | 레지스트리 값이 수정되기 전의 레지스트리 값의 원래 레지스트리 키 |
| `PreviousRegistryValueName` | 문자열 | 수정되기 전의 레지스트리 값의 원래 이름입니다. |
| `PreviousRegistryValueData` | 문자열 | 레지스트리 값이 수정되기 전의 원래 데이터 |
