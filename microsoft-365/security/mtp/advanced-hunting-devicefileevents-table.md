...
| `RequestProtocol` | 문자열 | 해당되는 경우 활동을 시작하는 데 사용되는 네트워크 프로토콜: 알 수 없음, 로컬, SMB 또는 NFS |
| `ShareName` | 문자열 | 파일이 포함된 공유 폴더의 이름 |
| `RequestSourceIP` | 문자열 | 활동을 시작한 원격 장치의 IPv4 또는 IPv6 주소 |
| `RequestSourcePort` | 문자열 | 활동을 시작한 원격 장치의 원본 포트 |
| `RequestAccountName` | 문자열 | 활동을 원격으로 시작하는 데 사용되는 계정의 사용자 이름 |
| `RequestAccountDomain` | 문자열 | 활동을 원격으로 시작하는 데 사용되는 계정의 도메인 |
| `RequestAccountSid` | 문자열 | 활동을 원격으로 시작하는 데 사용되는 계정의 SID(보안 식별자)입니다. |
...
| `SensitivitySubLabel` | 문자열 | 정보 보호를 위해 분류하기 위해 전자 메일, 파일 또는 기타 콘텐츠에 적용된 하위 분류 민감도 하위 레이블은 민감도 레이블 아래에 그룹화되지만 독립적으로 처리됩니다. |
