...
서비스를 수행하고 응답성을 유지하기 위해 고급 헌팅은 다양한 할당량 및 사용 매개 변수("서비스 제한"이라고도 합니다)를 설정합니다. 이러한 할당량 및 매개 변수는 사용자 지정 검색 규칙에 따라 수동으로 실행된 [쿼리에 적용됩니다.](custom-detection-rules.md) 여러 쿼리를 정기적으로 실행하는 고객은 [](advanced-hunting-best-practices.md) 사용량을 추적하고 최적화 모범 사례를 적용하여 중단을 최소화해야 합니다.

다음 표를 참조하여 기존 할당량 및 사용 매개 변수를 이해합니다.

| 할당량 또는 매개 변수 | 크기 | 새로 고침 주기 | 설명 |
|--|--|--|--|
...
>[!NOTE] 
>별도의 할당량 및 매개 변수 집합은 API를 통해 수행되는 고급 헌팅 쿼리에 적용됩니다. [고급 헌팅 API에 대한 읽기](https://docs.microsoft.com/microsoft-365/security/mtp/api-advanced-hunting)
