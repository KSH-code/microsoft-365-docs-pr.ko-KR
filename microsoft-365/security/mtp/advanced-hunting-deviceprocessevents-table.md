...
| `ProcessIntegrityLevel` | 문자열 | 새로 만든 프로세스의 무결성 수준입니다. Windows는 다운로드한 인터넷에서 시작된 경우와 같은 특정 특성을 기반으로 프로세스에 무결성 수준을 할당합니다. 이러한 무결성 수준은 리소스에 대한 사용 권한에 영향을 미치게 됩니다. |
| `ProcessTokenElevation` | 문자열 | 새로 만든 프로세스에 적용된 UAC(사용자 액세스 제어) 권한 상승의 유무를 나타내는 토큰 형식입니다. |
