...
`summarize` 연산자를 사용 하 여 차트로 표시할 값의 숫자를 가져옵니다. 아래 쿼리는 `summarize` 연산자를 사용 하 여 심각도 별 경고 수를 가져옵니다.
...
또한 `summarize` 연산자를 사용 하 여 여러 필드의 차트 값에 대 한 결과를 준비할 수 있습니다. 예를 들어 여러 운영 체제 (OS)에서 경고 심각도가 분산 되는 방식을 이해할 수 있습니다. 

아래 쿼리는 `join` 연산자를 사용 하 여 `DeviceInfo` 테이블에서 OS 정보를 가져온 후 `summarize` `OSPlatform` 와 `Severity` 열에서 모두 값을 계산 하는 데 사용 됩니다.
...
![OS에의 한 경고에 대 한 누적 된 차트](../../media/advanced-hunting-stacked-chart.jpg)
쿼리 결과로 표시 되는 고급 구하기 쿼리 결과 이미지*및 누적 차트로 표시 되는 심각도*

#### <a name="phishing-emails-across-top-ten-sender-domains"></a>상위 10 개의 보낸 사람 도메인에 걸친 피싱 전자 메일
유한 하지 않은 값의 목록을 처리 하는 경우 `Top` 연산자를 사용 하 여 대부분의 인스턴스가 있는 값만 차트로 만들 수 있습니다. 예를 들어 가장 많이 피싱 전자 메일을 사용 하 여 상위 10 개의 보낸 사람 도메인을 가져오려면 아래 쿼리를 사용 합니다.
...
`bin()` 함수에 `summarize` 연산자를 사용 하 여 시간에 따른 특정 지표와 관련 된 이벤트를 확인할 수 있습니다. 아래 쿼리는 파일 `invoice.doc` 을 30 분 간격으로 포함 하는 이벤트 수를 계산 하 여 해당 파일과 관련 된 작업의 스파이크를 보여 줍니다.
...
```
아래의 꺽은선형 차트는 `invoice.doc`다음과 같은 작업을 포함 하는 시간 기간을 명확 하 게 강조 표시 합니다. 
