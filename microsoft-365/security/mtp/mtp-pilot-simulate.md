...
파일럿 환경을 준비한 후에는 Microsoft 365 Defender 인시던트 관리 및 자동 조사 및 업데이트 기능을 테스트 해야 합니다. Microsoft는 고급 기술을 활용 하 여 검색에서 숨기는 정교한 공격을 시뮬레이트하는 데 도움이 될 것입니다. 이 공격은 도메인 컨트롤러에서 열린 SMB (서버 메시지 블록) 세션을 열거 하 고 사용자 장치의 최근 IP 주소를 검색 합니다. 일반적으로이 유형의 공격에는 처리 되지 않은 장치에 삭제 된 파일이 포함 되지 않으며 메모리에만 발생 합니다. 기존 시스템 및 관리 도구를 사용 하 고 시스템 프로세스에 코드를 주입 하 여 실행을 숨기고, 이러한 동작을 통해 장치를 피하기 감지 하 고 유지할 수 있습니다.
...
시뮬레이션 중에는 공격을 통해 겉보기에 shellcode 코드가 삽입 됩니다. 이 시나리오에서는 notepad.exe을 사용 해야 합니다. 시뮬레이션에이 프로세스를 선택 했지만 공격자가 svchost.exe와 같은 장기 실행 시스템 프로세스를 대상으로 하는 것이 더 좋을 수 있습니다. 그리고 나 서 계속 하는 방법에 대 한 지침을 받으려면 shellcode는 공격자의 C2 (명령 및 제어) 서버에 연락 하 여 이동 합니다. 스크립트에서 DC (도메인 컨트롤러)에 대 한 검사 쿼리를 실행 하려고 합니다. 정찰을 통해 공격자는 최근 사용자 로그인 정보에 대 한 정보를 얻을 수 있습니다. 공격자가 이러한 정보를가지고 있으면 네트워크에서 laterally를 이동 하 여 특정 중요 한 계정에 액세스할 수 있습니다.
...
1.  테 넌 트에서 [Microsoft 365 Defender를 사용 하도록 설정](https://docs.microsoft.com/microsoft-365/security/mtp/mtp-enable#starting-the-service)했는지 확인 합니다.

2.  테스트 도메인 컨트롤러 구성을 확인 합니다.

    - 장치는 Windows Server 2008 R2 이상 버전에서 실행 됩니다.
    - Id 및 [원격 관리](https://docs.microsoft.com/windows-server/administration/server-manager/configure-remote-management-in-server-manager)를 사용 하도록 설정 하 고 [Microsoft Defender에 대 한](https://docs.microsoft.com/azure/security-center/security-center-wdatp) 테스트 도메인 컨트롤러입니다.    
    - [Id 및 Microsoft Cloud App Security 통합용 Microsoft Defender](https://docs.microsoft.com/cloud-app-security/aatp-integration) 가 사용 하도록 설정 되어 있는지 확인 합니다.
...
3.  테스트 장치 구성 확인:
    ...
    1.  테스트 장치가 [등록 용 Microsoft Defender To Endpoint)](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints)인지 확인 합니다.
...
자동 인시던트 및 응답 기능을 작동 하는 것을 확인 하려면 notepad.exe 프로세스를 열어 두십시오. 자동 인시던트 및 응답이 메모장 프로세스를 중지 하는 것을 볼 수 있습니다.
...
Microsoft 365 Defender는 분석을 연관 시키고 서로 다른 제품의 모든 관련 알림과 조사를 하나의 인시던트 엔터티로 집계 합니다. 이를 통해 Microsoft 365 Defender는 보다 폭넓은 공격 영역을 보여 주며, SOC 분석가가 복잡 한 위협에 대처 하 고 대응 하도록 허용 합니다.
...
2.  인시던트 이름 왼쪽에 있는 원으로 클릭 하 여 최신 항목을 선택 합니다. 측면 패널에는 관련 된 모든 경고를 포함 하 여 인시던트에 대 한 추가 정보가 표시 됩니다. 각 인시던트는 포함 된 경고의 특성을 기반으로 하는 고유한 이름을 사용 합니다.
    ...
    대시보드에 표시 되는 경고는 서비스 리소스 (Id 용 Microsoft Defender, Microsoft Cloud App Security, microsoft Defender for Endpoint, Microsoft 365 Defender 및 Microsoft Defender for Office 365)를 기반으로 필터링 할 수 있습니다.  

3.  문제에 대 한 자세한 정보를 보려면 **문제 발생 페이지 열기** 를 선택 합니다.

    **인시던트 페이지에서** 문제와 관련 된 모든 경고 및 정보를 볼 수 있습니다. 이 정보에는 경고에 포함 된 엔터티 및 자산, 경고의 감지 원본 (Id 용 Microsoft Defender, EDR) 및 서로 연결 된 이유가 포함 됩니다. 검토 문제 경고 목록에는 공격 진행 상태가 표시 됩니다. 이 보기에서 개별 알림을 보고 조사할 수 있습니다.
...
>[!NOTE]
>여기서는 시뮬레이트된 공격 중에 생성 되는 몇 가지 경고만 안내 합니다. 테스트 장치에서 실행 되는 Windows 및 Microsoft 365 Defender 제품의 버전에 따라 약간 다른 경고가 표시 될 수 있습니다.

![생성 된 경고 스크린샷](../../media/mtp/fig6.png) 


**경고: 의심 스러운 프로세스 주입이 관찰 됨 (원본: Microsoft Defender for Endpoint EDR)**
...
SOC 분석가가 이러한 advanced 공격을 찾아낼 수 있도록 하기 위해, Microsoft Defender for Endpoint의 깊이 메모리 센서가 클라우드 서비스에 다양 한 프로세스 간 코드 주입 기법에 대 한 전례 없는 가시성을 제공 합니다. 다음 그림에서는 <i>notepad.exe</i>에 대 한 Defender가 검색 되어 코드 삽입 시도에 대 한 알림을 받는 방법을 보여 줍니다.

![잠재적 악성 코드 주입에 대 한 경고 스크린샷](../../media/mtp/fig7.png) 


**경고: 명령줄 인수 없이 실행 된 프로세스에서 관찰 된 예기치 않은 동작 (원본: Microsoft Defender for Endpoint EDR)**

끝점 검색을 위한 Microsoft Defender는 공격 기술에 대 한 가장 일반적인 특성을 대상으로 하는 경우가 많습니다. 이 메서드는 지 수를 확인 하 고 공격자가 새 전술으로 전환 하는 막대를 표시 합니다.

대규모 학습 알고리즘을 사용 하 여 전 세계 조직에서 일반적인 프로세스의 일반적인 동작을 설정 하 고 이러한 프로세스에 비정상 동작이 표시 되는 시점을 조사 합니다. 이러한 비정상 동작은 대개 불필요 한 코드가 도입 되어 다른 신뢰할 수 없는 프로세스에서 실행 되 고 있음을 나타냅니다.
...
IP 주소 세부 정보 페이지를 보려면 경고 프로세스 트리에서 IP 주소를 선택 합니다.
...
**알림: 사용자 및 SMB (IP 주소 검사) (원본: Microsoft Defender for Identity)**
...
### <a name="review-the-device-timeline-microsoft-defender-for-endpoint"></a>장치 시간 표시 막대 검토 [Microsoft Defender for Endpoint]
이 인시던트의 다양 한 경고를 살펴본 후 앞에서 조사 했던 인시던트 페이지로 다시 이동 합니다. 인시던트 페이지에서 **장치** 탭을 선택 하 여 끝점에 대 한 microsoft Defender 및 Id 용 microsoft defender에서 보고 한이 인시던트에 관련 된 장치를 검토 합니다.

공격이 수행 된 장치의 이름을 선택 하 여 해당 장치에 대 한 엔터티 페이지를 엽니다. 이 페이지에서 트리거된 경고 및 관련 이벤트를 볼 수 있습니다.

**시간 표시 막대** 탭을 선택 하 여 장치 시간 표시 막대를 열고, 경고가 발생 한 것으로 표시 된 시간 순으로 장치에서 관찰 된 모든 이벤트 및 동작을 확인 합니다.

![동작이 포함 된 장치 시간 표시 막대 스크린샷](../../media/mtp/fig11.png) 

보다 흥미로운 동작 중 일부를 확장 하면 프로세스 트리와 같은 유용한 정보를 얻을 수도 있습니다.

예를 들어 **의심 스러운 프로세스 주입이 관찰** 될 때까지 아래로 스크롤합니다. 왼쪽 창에 있는 **이벤트 엔터티** 그래프에이 동작에 대 한 전체 프로세스 트리를 표시 하려면 **notepad.exe Process 이벤트에 삽입powershell.exe** 를 선택 합니다. 필요한 경우 검색 창을 사용 하 여 필터링 합니다.
...
인시던트 페이지에서 **사용자** 탭을 선택 하 여 공격과 관련 된 사용자 목록을 표시 합니다. 이 표에서는 각 사용자의 **조사 우선 순위** 점수를 포함 하 여 각 사용자에 대 한 추가 정보를 포함 합니다.

사용자 이름을 선택 하 여 추가 조사가 수행 될 수 있는 사용자 프로필 페이지를 엽니다. [위험한 사용자 조사에 대 한 자세한 내용을 읽으십시오](https://docs.microsoft.com/cloud-app-security/tutorial-ueba#identify).
<br>
...
Microsoft 365 보안 센터 포털의 인시던트로 다시 이동 합니다. **인시던트** 페이지의 **조사** 탭에는 id 용 microsoft defender에서 트리거된 자동화 된 조사 및 끝점에 대 한 microsoft defender가 표시 됩니다. 아래 스크린샷에서는 Defender for Endpoint에 트리거되는 자동화 된 조사가 표시 됩니다. 기본적으로 끝점에 대 한 Defender는 큐에 있는 아티팩트를 자동으로 remediates, 재구성 해야 합니다.

![문제와 관련 된 자동화 된 조사 스크린샷](../../media/mtp/fig14.png)

조사를 트리거한 경고를 선택 하 여 **조사 세부 정보** 페이지를 엽니다. 다음과 같은 세부 정보가 표시 됩니다.
- 자동 조사를 트리거한 경고입니다.
...
자동 조사를 수행 하는 동안 끝점에 대 한 Microsoft Defender가 수정 해야 하는 아티팩트 중 하나로 삽입 된 notepad.exe 프로세스를 확인 했습니다. 끝점에 대 한 Defender는 자동화 된 재구성의 일부로 의심 스러운 프로세스 주입을 자동으로 중지 합니다. 
...
**인시던트 관리** 를 선택 합니다. **인시던트를 해결** 하려면 상태를 설정 하 고 관련 분류를 선택 합니다.
...
<br>
이렇게 하면 문제 관리 및 자동화 된 조사 및 업데이트 시나리오에 대 한 공격 시뮬레이션이 발생 합니다. 다음 시뮬레이션에서는 잠재적으로 유해한 파일에 대 한 사전 예방 위협 요소를 사용 합니다. 
...
2.  전자 메일을 받는 데 사용할 대상 사서함을 식별 합니다.
    a.  이 사서함은 Microsoft Defender for Office 365 b에서 모니터링 해야 합니다.  요구 사항 3에서이 사서함에 액세스 해야 하는 장치
3.  테스트 장치 구성: a를 실행 합니다.  Windows 10 버전 1903 이상 버전을 사용 하 고 있는지 확인 합니다.
    ...
    d.  [끝점에 대 한 Microsoft Defender에 내장](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints)되어 있습니다.
...
6.  다음으로, **EmailAttachmentInfo** 테이블의 **SHA256** 값을 사용 하 여 해당 해시에 대 한 **devicefileevents** (끝점에서 발생 한 파일 작업)를 찾습니다.  여기에는 일반 필드가 첨부 파일의 SHA256 해시가 됩니다.
    ...
    결과 테이블에는 이제 끝점 (Microsoft Defender for Endpoint) (장치 이름, 수행 된 작업 (이 경우에는 FileCreated 이벤트 포함만 필터링), 그리고 파일이 저장 되는 위치에 대 한 세부 정보가 포함 되어 있습니다. 프로세스와 연결 된 계정 이름도 포함 됩니다.
...
7.  기능은 전파, 서명자 및 발급자 정보와 같은 파일에 대 한 추가 TI 데이터를 가져올 수 있게 해 주는 특수 한 유형의 조인입니다.  파일에 대 한 자세한 내용을 보려면 **Fileprofile ()** 함수 향상을 사용 하십시오.
...
5.  알림 규칙의 범위를 선택 합니다. 이 쿼리는 장치를 포함 하므로이 사용자 지정 검색 기능은 끝점 컨텍스트에 대 한 Microsoft Defender에 따라 적합 합니다. 영향을 받는 엔터티로 장치를 포함 하지 않는 사용자 지정 검색을 만드는 경우 범위가 적용 되지 않습니다.  
...
고급 구하기에 대 한 자세한 내용은 다음 웹캐스트에서 Microsoft 365 Defender 내의 고급 검색 기능을 사용 하 여 크로스-기둥 쿼리를 만들고, 엔터티에 대 한 피벗을 수행 하 고, 사용자 지정 검색 및 재구성 작업을 만드는 방법을 안내 합니다.
...
| 에피소드 4: 이번에는 사냥을 사용 합니다. 인시던트 추적에 KQL 적용|일부 공격자 활동을 추적 하는 데 걸리는 시간입니다. 이 에피소드에서는 Microsoft 365 Defender의 KQL 및 고급 구하기에 대 한 향상 된 이해를 활용 하 여 공격을 추적 합니다. Cybersecurity의 ABCs와이를 인시던트 응답에 적용 하는 방법을 비롯 하 여 공격자 활동을 추적 하기 위해 필드에 사용 되는 몇 가지 팁과 트릭을 알아봅니다. | [MP4](https://aka.ms/MTP5AUG20_MP4) | [YouTube](https://youtu.be/2EUxOc_LNd8) | [에피소드 4: Git의 CSL 파일](https://github.com/microsoft/Microsoft-threat-protection-Hunting-Queries/blob/master/Webcasts/TrackingTheAdversary/Episode%204%20-%20Lets%20Hunt.csl) |

## <a name="next-step"></a>다음 단계
|![마감 및 요약 단계](../../media/mtp/close.png) <br>[마감 및 요약 단계](mtp-pilot-close.md) | Microsoft 365 Defender 파일럿 결과를 분석 하 고 관련자에 게 제공 하 고 다음 단계를 수행 합니다.
|:-----|:-----|
