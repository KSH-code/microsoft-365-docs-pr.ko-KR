...
- **보안 운영자** —이 [Azure Active Directory 역할](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-assign-admin-roles#security-administrator) 을 가진 사용자는 알림을 관리 하 고 Microsoft 365 보안 센터의 모든 정보를 포함 하 여 보안 관련 기능에 대 한 전역 읽기 전용 액세스 권한을 갖습니다. 이 역할은 Microsoft Defender for Endpoint에서 RBAC (역할 기반 액세스 제어)가 해제 된 경우에만 사용자 지정 검색을 관리 하기에 충분 합니다. RBAC를 구성한 경우에는 Defender for 끝점에 대 한 **보안 설정 관리** 권한도 필요 합니다.
...
- **설정** 권한 역할의 [microsoft defender 보안 센터](https://securitycenter.windows.com/) 에서 끝점에 대 한 microsoft defender에 대 한 RBAC 설정을 확인  >  **Permissions**  >  **Roles** 합니다. 해당 하는 역할을 선택 하 여 **보안 설정 관리** 권한을 할당 합니다.

> [!NOTE]
> 사용자 지정 검색을 관리 하기 위해 **보안 운영자** 는 RBAC가 설정 된 경우 끝점에 대 한 Microsoft Defender의 **보안 설정 관리** 권한이 필요 합니다.
...
> [!TIP]
> 쿼리 성능을 개선 하려면 규칙의 의도 한 실행 빈도와 일치 하는 시간 필터를 설정 합니다. 가장 자주 실행 되는 시간은 _24 시간 마다_ 다르기 때문에 지난 날 필터링은 모든 새 데이터에 적용 됩니다.
...
- **장치 격리** -Microsoft Defender for Endpoint를 사용 하 여 전체 네트워크 격리를 적용 하 고 장치가 어떤 응용 프로그램 또는 서비스에도 연결 되지 않도록 합니다. [끝점 컴퓨터 격리에 대 한 Microsoft Defender에 대 한 자세한 정보](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/respond-machine-alerts#isolate-devices-from-the-network)
- **수집 조사 패키지** -ZIP 파일의 장치 정보를 수집 합니다. [Microsoft Defender for Endpoint 조사 패키지에 대 한 자세한 정보](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/respond-machine-alerts#collect-investigation-package-from-devices)
- **바이러스 검사 실행** -장치에서 전체 Windows Defender 바이러스 검사를 수행 합니다.
- **시작 조사** -장치에 대 한 [자동화 된 조사](mtp-autoir.md) 시작
- **앱 실행 제한** -Microsoft에서 발급 한 인증서로 서명 된 파일만 실행 하도록 장치에 대 한 제한을 설정 합니다. [끝점에 대 한 Microsoft Defender의 앱 제한 사항에 대해 자세히 알아보기](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/respond-machine-alerts#restrict-app-execution)
...
> [!NOTE]
> 사용자 지정 검색 규칙에 대 한 allow 또는 block 작업은 현재 Microsoft 365 Defender에서 지원 되지 않습니다.
