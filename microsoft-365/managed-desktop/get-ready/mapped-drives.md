...
대부분의 엔터프라이즈 환경에는 사용자 또는 팀이 파일을 공유하고 저장할 수 있도록 매핑된 드라이브에 대한 레거시 요구 사항이 있으며, 이러한 요구 사항은 모든 사용자가 파일을 공유하고 저장할 수 있도록 합니다. Microsoft는 Microsoft Managed Desktop에서 매핑된 드라이브를 사용하지 않는 것이 좋습니다. 대신 다음과 같이 파일 액세스 솔루션을 최신화하는 것이 좋습니다.
...
1. Microsoft [Endpoint Manager로](https://endpoint.microsoft.com/) 이동하여 "문제 해결 + 지원"을 선택한 다음 Microsoft Managed Desktop 섹션에서 "서비스 요청"을 찾아 봐야 합니다.  
