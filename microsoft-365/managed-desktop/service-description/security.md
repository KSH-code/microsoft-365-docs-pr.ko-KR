...
전체 볼륨 암호화 |    Windows BitLocker는 Microsoft Managed Desktop 장치용 볼륨 암호화 솔루션입니다.<br><br>조직이 서비스에 온보드되고 나면 장치가 절전 모드에 있을 때 또는 해제될 때 로컬 데이터에 대한 무단 액세스를 방지하기 위해 TPM(트러스트 플랫폼 모듈)이 기본 제공된 Windows BitLocker를 사용하여 암호화됩니다. 
