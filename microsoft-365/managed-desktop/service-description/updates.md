...
Microsoft에서 출시 되는 업데이트는 누적 된 것으로, 품질이 나 기능 업데이트로 분류 될 수 있습니다.
...
- 배포 중에 Microsoft Managed Desktop은 진단 데이터 신호 및 최종 사용자 지원 시스템을 기반으로 하는 실패 또는 장애의 징후를 모니터링 합니다. 검색 된 경우 모든 현재 및 미래의 그룹에 대 한 배포가 즉시 일시 중지 됩니다.
    ...
    - Microsoft Managed Desktop IT 관리자 포털에 티켓을 저장 하면 호환성 문제가 보고 될 수 있습니다.
...
Windows 참가자 빌드가 포함 된 모든 장치는 테스트 그룹에 저장 되며, sla (서비스 수준 계약)에는 포함 되지 않습니다.

## <a name="bandwidth-management"></a>대역폭 관리

모든 운영 체제 및 드라이버 업데이트에 대해 배달 최적화가 사용 됩니다. 회사 네트워크 내의 피어에서 업데이트를 검색 하 여 WU (Windows Update) 서비스의 다운로드 크기를 최소화 합니다.
