...
사용자는 Microsoft Managed Desktop에서 관리하는 디바이스에서 데이터를 수신, 전송 및 저장할 수 있습니다. 데이터 개인 정보 보호는 예상과 일관된 방식으로만 보호되고 사용된다고 신뢰합니다. 이 문서에서는 Microsoft Managed Desktop에서 개인 데이터를 수집, 저장, 보존, 프로세스, 보안, 공유, 감사 및 내보내는 방법에 대해 설명합니다. 관리자는 개인 데이터를 보고 수정하고 삭제하는 방법도 배우게 됩니다.

Microsoft Managed Desktop은 프로파일링, 광고 또는 마케팅 목적으로 서비스를 제공하는 일부로 수집된 개인 데이터를 사용하지 않습니다.

## <a name="data-collection-of-microsoft-managed-desktop"></a>Microsoft Managed Desktop의 데이터 수집

사용자가 Microsoft Managed Desktop에 회사 장치를 등록하면 Windows 및 Microsoft Intune을 사용하여 기술 계층에서 데이터 수집이 처리됩니다. 이러한 원본은 Microsoft Managed Desktop의 장치 이름과 같이 사용자 장치에 대한 개인 데이터를 수집하여 Microsoft Managed Desktop 환경과 함께 관리 및 제공될 디바이스를 식별할 수 있습니다.
...
Microsoft Managed Desktop은 해당 데이터를 미국에 저장합니다. 개인 데이터는 끝점용 Microsoft Defender에서 수집한 Microsoft Managed Desktop 장치에 대한 경고 데이터를 제외하고 최대 30일 동안 Microsoft Managed Desktop에서 보존됩니다. 개인 데이터를 포함할 수 있는 실제 경고 데이터는 180일 동안 저장됩니다. 개인 데이터가 제거된 알림 데이터는 최대 2년 동안 저장됩니다. Microsoft Managed Desktop은 GDPR(일반 데이터 보호 규정) 및 캘리포니아 소비자 개인 정보 보호법(CCPA)을 준수하여 경고 데이터에 저장된 모든 개인 데이터에 대한 데이터 주체 권리를 준수합니다.

### <a name="staff-location"></a>직원 위치

Microsoft Managed Desktop Operations and Security Operations 팀은 미국과 인도에 있습니다.
...
Microsoft Managed Desktop에서 사용하는 ID 데이터는 Office 365 또는 Azure와 같은 Microsoft 온라인 서비스에 구독할 때 조직에서 제공한 주소를 기반으로 Azure Active Directory에 의해 지리적 위치에 저장됩니다. Azure Active Directory용 데이터 센터를 보여주는 지도는 [Microsoft Azure -내](http://azuredatacentermap.azurewebsites.net/) 고객 데이터는 어디에 있나요?를 참조하세요.

Azure에서 데이터 저장소에 사용하는 지역에 대한 자세한 내용은 [Azure Active Directory–Where is your data located을 참조하세요.](https://msit.powerbi.com/view?r=eyJrIjoiODdjOWViZDctMWRhZS00ODUzLWI4MmQtNWM5NjBkZTBkNjFlIiwidCI6IjcyZjk4OGJmLTg2ZjEtNDFhZi05MWFiLTJkN2NkMDExZGI0NyIsImMiOjV9)
...
보안 관리자는 해당 환경의 Microsoft Managed Desktop 관리 장치에서 끝점 경고에 대한 Microsoft Defender와 관련된 개인 데이터 추출 또는 지우기 기능을 요청할 수 있습니다. 보안 관리자는 Microsoft Managed Desktop [Admin Portal에](https://aka.ms/memadmin) 로그인하고 지원 요청을 제출해야 합니다. 지원 **요청 유형** **변경** **요청,** 보안  범주 및 기타 하위 범주를 선택한 다음 설명에 관련 장치 이름과 함께 데이터 추출 또는 데이터 추출 요청과 함께 제공합니다. 
