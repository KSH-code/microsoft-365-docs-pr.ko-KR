...
ms.locfileid: "37523673"
---
# <a name="customize-the-service"></a>서비스 사용자 지정
...
## <a name="types-of-customizations"></a>사용자 지정 유형
사용자 지정은 Microsoft Managed Desktop 기본 구성에 대 한 추가 또는 변경입니다. 예를 들면 USB 포트 구성에서 새 보안 에이전트를 배포 하기 위한 범위입니다. 다양 한 사용자 지정을 다음과 같이 그룹화 합니다.
...
## <a name="request-a-customization"></a>사용자 지정 요청

변경 요청을 만들어 Microsoft Managed Desktop Administration portal을 통해 요청을 제출 합니다. 다음 정보를 포함 해야 합니다.
-   사용자 지정 유형: 사용자 지정 범주는 무엇입니까? (위의 표 참조)
-   요구 사항: 사용자 지정을 위한 특정 비즈니스 요구 사항은 무엇 인가요?
-   제안: 비즈니스에서 요청 하는 솔루션은 무엇입니까?
-   시간 표시 막대:이 사용자 지정 작업을 마지막으로 얼마나 오래 하시 겠어요? 


## <a name="how-we-assess-a-customization-request"></a>사용자 지정 요청을 평가 하는 방법

사용자 지정 요청을 검토할 때는 다음 순서 대로 이러한 요인을 평가 합니다.
...
3.  Microsoft 기술을 사용 하 여 요구 사항을 충족할 수 있는 경우에는 프로젝트 범위에 따라 3 ~ 12 개월의 마이그레이션 기간에 대 한 요청을 승인할 가능성이 높습니다.
...
다음 조건에 위반 되지 않도록 사용자 지정 내용을 검토 합니다.

-   사용자 지정은 시스템 보안에 악영향을 미치지 않아야 합니다. 
-   사용자 지정을 유지 관리 하는 경우 Microsoft 관리 되는 데스크톱 작업 또는 지원에 상당한 비용이 드는 것은 아닙니다.
-   사용자 지정은 커널 모드를 중단 시키거나 중단 시키는 등 시스템 안정성에는 영향을 주지 않아야 합니다.
...
## <a name="revoking-approval-for-a-customization"></a>사용자 지정에 대 한 승인 해지

요청 된 사용자 지정 내용이 승인 및 배포 된 후에는 변경 내용을 처음으로 승인 했을 때 명확 하지 않은 주요 상황을 위반 하는 문제를 발견할 수 있습니다. 이러한 상황에서는 사용자 지정에 대 한 승인을 취소 해야 할 수 있습니다.
 
이 경우 Microsoft 관리 되는 데스크톱 관리 포털을 사용 하 여 사용자에 게 알림 메시지를 표시 합니다. 사용자에 게 처음 알릴 때 90 일 동안 사용자 지정이 포함 된 장치가 Microsoft Managed Desktop service 수준 계약에 의해 더 이상 바인딩되지 않도록 해야 합니다. 엄격한 시간 표시 막대에 따라 여러 알림을 전송 하지만, 심각한 사고나 위협으로 인해 시간 표시 막대 또는 사용자 지정에 대 한 결정을 변경 해야 할 수 있습니다. 사용자가 동의 하지 않은 경우에는 *제거* 되지 않지만 사용자 지정 내용이 해지 된 모든 장치는 더 이상 서비스 수준 계약에 따라 바인딩되지 않습니다. 다음은 사용자에 게 보낼 알림의 시간 표시 막대입니다.

- **첫 번째 알림:** Microsoft는이를 철회 하는 이유에 대 한 정보, 수행 해야 하는 작업, 해당 작업에 대 한 마감일, 의사 결정을 원하는 경우 따라야 하는 단계를 비롯 하 여 승인 취소 결정에 대 한 첫 번째 알림을 제공 합니다. 이는 모든 장치에서 사용자 지정을 제거 해야 하기 전에 90 일 전에 진행 됩니다. 
...
- **첫 번째 알림 이후 90 일:** Microsoft Managed Desktop 서비스 수준 계약은 사용자 지정 내용이 해지 된 모든 장치에 더 이상 적용 되지 않습니다. 언제 든 지 의사 결정을 시도 하 고 업그레이드, 구성 변경 또는 소프트웨어 변경을 포함 하 여 고려 사항에 대 한 추가 정보를 제공할 수 있습니다. 
