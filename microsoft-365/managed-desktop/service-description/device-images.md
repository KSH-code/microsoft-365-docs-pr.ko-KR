...
승인된 제조업체에서 새 [](device-requirements.md#minimum-requirements)장치를 주문할 때 다음 단계에 따라 올바른 Microsoft Managed Desktop 이미지 및 소프트웨어 구성으로 장치를 배송하는지 확인합니다.

### <a name="dell"></a>Dell
Microsoft Managed Desktop에서 승인한 이미지가 주문을 위해 장치에 적용될 수 있도록 Dell 영업 담당자와 직접 작업합니다. Dell 장치, 이미지 및 주문 프로세스에 대한 자세한 내용은 에 문의 MMD_at_dell@dell.com.

### <a name="hp"></a>HP 
HP에서 새 디바이스를 주문하는 경우 프로그램 디바이스에 표시된 각 모델의 추가 요구 사항 섹션에 나열된 특정 SKU를 [사용하세요.](device-list.md#hp)
...
Microsoft Managed Desktop에서 사용하기 위해 Lenovo에서 디바이스를 주문하는 경우 주문의 일부로 포함된 특정 파트 번호를 지정해야 합니다. Lenovo 영업 담당자 또는 Lenovo 채널 파트너에게 문의하여 장치 요구 사항을 충족하는 시스템을 사용하여 *"* 특수한 경매 모델"을 만들 것을 [요청합니다.](device-requirements.md#minimum-requirements) Microsoft Managed Desktop과 호환되는 미리 로드된 이미지를 포함하려면 영업 담당자에게 "시스템 문서 블록 부분 번호 *SBB0Q94938 – MMD* 사용"을 참조하도록 요청합니다.

다음 제품은 현재 Microsoft Managed Desktop 지원에 사용할 수 있습니다.
...
장치 요구 사항을 충족하는 모든 Microsoft 장치는 Microsoft Managed Desktop에서 작동하는 이미지와 함께 제공합니다. 다른 단계는 필요하지 않습니다.
...
제조업체의 이미지 또는 Microsoft Managed Desktop "유니버설 이미지"를 사용하여 디바이스의 이미지를 다시 제작할 수 있습니다. 적절한 제조업체 이미지를 얻기 위해 다시 [](#new-devices) 사용할 모델의 새 디바이스를 하나 이상 주문할 수 있습니다. 그런 다음 해당 장치에서 이미지를 얻은 후 정확히 동일한 모델의 다른 장치에 적용할 수 있습니다.
...
이러한 이미지는 WinRE(Windows 복구 환경)를 사용하며 이 프로세스는 자동화되지 않은 수동 프로세스입니다. Surface용 USB 복구 드라이브 [만들기 및 사용의 단계를 따릅니다.](https://support.microsoft.com/surface/creating-and-using-a-usb-recovery-drive-for-surface-677852e2-ed34-45cb-40ef-398fc7d62c07)


### <a name="universal-image"></a>유니버설 이미지
Microsoft Managed Desktop은 Microsoft Managed Desktop에서 사용할 수 있는 Windows 10 Pro 및 엔터프라이즈용 Microsoft 365 앱을 포함하는 이미지를 생성했습니다. 그러나 가능하면 사용자가 로그인한 후 업데이트해야 하는 이전 Windows 버전을 의미하는 경우에도 가능하면 제조업체에서 제공하는 Microsoft Managed Desktop에 적합한 이미지를 사용하는 것이 좋습니다. Microsoft Managed Desktop 유니버설 이미지를 사용하는 것이 최종 옵션입니다.

- 30~60일마다 최신 Windows 월별 품질 업데이트와 엔터프라이즈용 Microsoft 365 앱 업데이트로 이미지를 업데이트합니다.
- 이 이미지에는 Windows 복구 시나리오에 따라 엔터프라이즈용 Microsoft 365 앱이 복원되도록 하는 복구 프로비저닝 패키지가 포함되어 있습니다.
...
- USB 드라이브에서 배포하는 동안 드라이버 및 품질 업데이트가 Windows에 추가됩니다.
